...
class ShortestRouteEnv(gym.Env):
    ...
    def _make_horizons(self):
        self.__make_horizons(True)

    def __make_horizons(self, is_shortest):
        self.horizon_states = self.graph.horizon_states
        self.horizon_acts = self.graph.horizon_acts
        self.horizon_rews = self.horizon_acts.copy()
        ...
        for i, epoch in enumerate(self.horizon_acts):
            for j, state_acts in enumerate(epoch):
                for k, act in enumerate(state_acts):
                    ...
                    self.horizon_rews[i][j][k] = (-1 if is_shortest else 1) * rew

class LongestRouteEnv(ShortestRouteEnv):
    def _make_R(self):
        self.__make_R(False)
    def _make_horizons(self):
        self.__make_horizons(False)
