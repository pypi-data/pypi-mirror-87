...
def test_rabbit_consume():
    sd.log_init_config('core', '/tmp/')
    consumer = sd.RabbitConsumer('lk_relay', 'rabbit_88')
    consumer.execute_safely(rabbit_consume, prefetch_count=1)
...
def test_mq_delay_produce():
    from sense_core import RabbitProducer, now_time
    import json
    produce = RabbitProducer('rabbit_88')
    import pdb
    pdb.set_trace()
    # produce = RabbitProducer('rabbit_88', delay=1, dlx_queue='lk_alg_meta', dlx_exchange='lk_alg_exchange', dlx_routing='lk_alg_meta', queue_ttl=35 * 60000)
    produce.send_safely(topic='lk_alg_de', message=json.dumps({'name': 'abc'}))
    produce.close_safely()
    for _k in range(1, 20):
        st = {'index': _k, 'time': now_time(), 'q_ttl': 3 * 60000, 'msg_ttl': 30000}
        if _k % 2 == 0:
            produce.send_safely(topic='lk_alg_delay_queue', message=json.dumps(st), msg_ttl=str(0))
        else:
            # 数据推送到topic中，ttl到期后通过dlx_exchange推送到dlx_queue队列，（dlx routing可以不写，方便按routing分到不同队列）
            produce.send_safely(topic='lk_alg_delay_queue', message=json.dumps(st))
