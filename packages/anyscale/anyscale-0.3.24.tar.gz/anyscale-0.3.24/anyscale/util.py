...
def wait_for_session_start(
    ...
    while True:
        ...
        if len(sessions) > 0:
            ...
            elif session.state and (
                "Errored" in session.state or session.state in {"Terminated", "Stopped"}
            ):
                raise click.ClickException(
                    f"Error while starting session {session_name}: Session was stopped due to an error."
                )
        else:
            raise click.ClickException(
