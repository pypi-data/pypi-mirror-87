...
from polyaxon_sdk import V1ModelVersion, V1ModelRegistry
...
def get_info(model: str = None, version: str = None):
    ...
    try:
        owner, model_registry, model_version = get_model_info(entity=entity, entity_name=entity_name, is_cli=True)
...
@registry.command()
@click.option(
    "--name", type=str, help="The model registry name, e.g. 'model-A' or 'acme/model-A:latest'."
)
...
def create(name, description, tags, public):
    ...
    try:
        ...
        _registry = polyaxon_client.model_registry_v1.create_model_registry(owner, registry_config)
...
@click.option("--run-uid", "-uid", type=str, help="The run to link to this model version.")
@clean_outputs
def push(name, description, tags, run_uid):
    ...
    polyaxon_client = PolyaxonClient()
    try:
        polyaxon_client.model_registry_v1.get_model_version(owner, registry_name, version)
        to_update = True
    ...
    if to_update:
        if not click.confirm(
            ...
            "Do you want to push force this version?".format(owner, registry_name, version)
        ):
    ...
    click.echo(
        "You can view this model version on Polyaxon UI: {}".format(
            get_dashboard_url(
                subpath="{}/registry/{}/versions/{}".format(owner, registry_name, _version.name)
            )
        )
    )
...
def delete(model, version):
    ...
    try:
        ...
        if is_version:
            ...
            polyaxon_client.model_registry_v1.delete_model_registry(owner, model_registry)
        Printer.print_success(
...
def dashboard(model, version, yes, url):
    ...
    owner, model_registry, model_version, is_version = get_info(model, version)
    subpath = (
        "{}/registry/{}/versions?version={}".format(owner, model_registry, model_version)
        if is_version
