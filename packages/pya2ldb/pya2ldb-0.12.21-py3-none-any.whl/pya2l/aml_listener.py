...
def createEnumeration(name, enumerators, is_referrer = False):
    class Enumeration(BaseType):
        def __init__(self, name, enumerators, is_referrer):
            self.name = name
            self.enumerators = enumerators
            self._renumber_constants()
            self.tags = frozenset(e.tag for e in enumerators)
            self.is_referrer = is_referrer

        def __repr__(self):
            return "Enumeration(name = {}, enumerators = {} is_referrer = {})".format(
                self.name, self.enumerators, self.is_referrer
            )
    ...
    res = Enumeration(name, enumerators, is_referrer)
    return res
...
def createTaggedUnion(name, members, is_referrer = False):
    class TaggedUnion(BaseType):
        def __init__(self, name, members, is_referrer):
            self.name = name
            self.members = members
            self.is_referrer = is_referrer

        def __repr__(self):
            return "TaggedUnion(name = {}, members = {} is_referrer = {})".format(
                self.name, self.members, self.is_referrer
            )

    res = TaggedUnion(name, members, is_referrer)
    return res
...
def createStructType(name, members, is_referrer = False):
    class StructType(BaseType):
        ...
        def __repr__(self):
            return "StructType(name = {}, members = {} is_referrer = {})".format(self.name, self.members, self.is_referrer)

    res = StructType(name, members, is_referrer)
    return res
...
def createTaggedStructType(name, members, is_referrer = False):
    class TaggedStructType(BaseType):
        ...
        def __repr__(self):
            return "TaggedStructType(name = {}, members = {} is_referrer = {})".format(
                self.name, self.members, self.is_referrer
            )

    res = TaggedStructType(name, members, is_referrer)
    return res
...
class AMLListener(antlr4.ParseTreeListener):
    ...
    def exitStruct_type_name(self, ctx):
        ...
        value = createStructType(name, members, is_referrer)
        ctx.value = value
        if not is_referrer:
    ...
    def exitTaggedstruct_type_name(self, ctx):
        ...
        if ctx.t0:
            ...
            name = None
        members = [l.value for l in ctx.l]
        ctx.value = createTaggedStructType(name, members, is_referrer)
        if not is_referrer:
    ...
    def exitEnum_type_name(self, ctx):
        ...
        if ctx.t0:
            ...
            name = None
        ctx.value = createEnumeration(name, elements, is_referrer)
    ...
    def exitTaggedunion_type_name(self, ctx):
        ...
        if ctx.t0:
            ...
            name = None
        members = [l.value for l in ctx.l]
        ctx.value = createTaggedUnion(name, members, is_referrer)
        if not is_referrer:
