...
def delete_file(record_id, file_id):
    ...
    Will delete the file specified by the given *file_id* from the record specified by
    the given *record_id*.
    """
    record = Record.query.get_active_or_404(record_id)
...
def delete_upload(record_id, upload_id):
    """Delete an upload from a record.

    Will delete the upload specified by the given *upload_id* from the record specified
    by the given *record_id*. Only uploads owned by the current user can be deleted.
    """
    record = Record.query.get_active_or_404(record_id)
