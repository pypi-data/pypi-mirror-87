...
class FuncThread(threading.Thread):
 def __init__(self,func,params=JUlCw):
...
class RequestHandler(BaseHTTPRequestHandler):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if JUlCe(result,JUlCG)else result
  return result
 def read_content(self):
  if JUlCr(self,'data_bytes'):
   return
  content_length=self.headers.get('Content-Length')
  self.data_bytes=self.rfile.read(JUlCV(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get('file_name')or 's3file.%s'%JUlCb(uuid.uuid4()))
...
def start_ssh_forward_proxy(options):
 ...
 host=LOCAL_BIND_ADDRESS_PATTERN.replace('*',JUlCb(JUlCD(USED_BIND_ADDRESSES)+2))
...
def create_network_interface_alias(address,interface=JUlCw):
 sudo_cmd='sudo'
 if is_mac_os():
 ...
 if is_linux():
  ...
  for interface in interfaces:
   try:
    ...
    pass
 raise JUlCd('Unable to create network interface')
def run_shell_cmd(command):
 try:
  return{'result':run_cmd(command)}
 except JUlCd as e:
  error_string=JUlCb(e)
  if JUlCe(e,subprocess.CalledProcessError):
...
def run_cmd(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=JUlCt))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output('uname -a',shell=JUlCt))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode('utf-8')if JUlCe(obj,JUlCb)else obj
def to_str(obj):
 return obj.decode('utf-8')if JUlCe(obj,JUlCE)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if JUlCD(sys.argv)>1 else 'main'
...
def start_in_background():
 ...
 run(cmd,outfile=log_file,asynchronous=JUlCt)
