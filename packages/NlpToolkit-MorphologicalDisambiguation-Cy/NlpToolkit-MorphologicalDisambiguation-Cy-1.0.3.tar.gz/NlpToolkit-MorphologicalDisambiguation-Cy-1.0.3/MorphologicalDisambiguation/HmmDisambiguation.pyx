...
cdef class HmmDisambiguation(NaiveDisambiguation):
    ...
    cpdef list disambiguate(self, list fsmParses):
        ...
        list
            List of FsmParses.
        """
        cdef int i, j, bestIndex, k, t
        cdef list correctFsmParses, probabilities, best
        cdef FsmParse currentParse, previousParse
        cdef Word w1, ig1, w2, ig2
        cdef double probability, bestProbability
        if len(fsmParses) == 0:
            return None
