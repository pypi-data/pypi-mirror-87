    ...
    for existing_open_event in existing_open_events:
        for i, new_event in enumerate(new_events):
            ...
            if can_new_event_close_open_event(event_end, new_event, min_length_of_alert, merge_period):
                ...
                meta[EVENT_END] = str(event_end)
                existing_open_event.metadata = meta
                # should string be still excluded?
                start_time: int = existing_open_event.start_time or 0
                if not is_string_truthy(meta.get("show")) and event_end - start_time > min_length_of_alert:
                    meta["show"] = "True"
                    existing_open_event.metadata = meta
            else:
        ...
        meta = existing_open_event.metadata or {}
        final_event_end = int(meta.get(EVENT_END, "0"))
...
def merge_time_intervals(
    intervals: Union[List[Union[List[int], Event]], EventList], min_length_of_alert: int = 0, merge_period: int = 0
) -> List[List[int]]:
    ...
    for higher in intervals_sorted:
        if higher[1] - higher[0] < min_length_of_alert - 1:
...
def remove_duplicates(existing_events: List[Event], new_events: List[List[int]]) -> Union[List[int], List]:
    ...
    for i, event in enumerate(new_events):
        event_is_duplicated = any(ev.start_time <= event[0] and ev.end_time >= event[1] for ev in existing_events)
...
def send_notification(start_time: int, end_time: int, oldest_alert_time_point: int, min_length_of_alert: int) -> bool:
    is_event_fresh = end_time >= oldest_alert_time_point
