...
# For this, to get the file to download like how the user anticipates it to download, we are passing up the data (ascii lines) to the django view. 
# From there, we are creating a file to send over to CSnap Pro - Andrew
...
# Making ASCII value true since we are sending the data to CSnap Pro. Avoids having to encode / decode from Python 3. -Andrew
def numpy2stl(A, fn, scale=0.1, mask_val=None, ascii=True,
              max_width=235.,
              max_depth=140.,
              max_height=150.,
              solid=False,
