...
def main_options(parser, endpoint_args=True):
    ...
    parser.add_argument(
        '--version', action='version', version=('%(prog)s ' + str(VERSION))
    )
    parser.add_argument(
        '--timeout',
        type=int,
...
def qq_main(argz=None, argv=None):
    ...
    try:
        ...
        # In python2 if no subcommand is specified the above line will print the
        # usage message and exit.  In python 3 a Namespace object is still
        # returned but it is missing the subcommand attribute. To resolve this
        # we check if the subcommand attribute exists and print a message if it
        # does not.
        # see https://bugs.python.org/issue9253
        if not hasattr(argz, 'subcommand'):
            parzer.print_usage()
            print('qq: error: too few arguments')
            return 1
        main(argz)
