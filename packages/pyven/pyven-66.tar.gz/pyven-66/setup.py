...
setuptools.setup(
        ...
        install_requires = ['aridity>=48', 'diapyr>=18', 'lagoon>=17', 'setuptools>=44.1.1', 'twine>=1.15.0'],
        package_data = {'': ['*.pxd', '*.pyx', '*.pyxbld', '*.arid', '*.aridt']},
        scripts = ['bootstrap', 'foreignsyms'],
        entry_points = {'console_scripts': ['tests=pyven.checks:main_tests', 'drmake=pyven.drmake:main_drmake', 'gclean=pyven.gclean:main_gclean', 'initopt=pyven.initopt:main_initopt', 'pipify=pyven.pipify:main_pipify', 'minreqs=pyven.projectinfo:main_minreqs', 'release=pyven.release:main_release', 'tasks=pyven.tasks:main_tasks', 'travis-ci=pyven.travis_ci:main_travis_ci', 'tryinstall=pyven.tryinstall:main_tryinstall']},
        **ext_modules())
