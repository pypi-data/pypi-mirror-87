...
def erode_box_slice_by_slice(source : Image, destination : Image = None):
    ...
    execute(__file__, 'erode_box_slice_by_slice_' + str(len(destination.shape)) + 'd_x.cl', 'erode_box_slice_by_slice_' + str(len(destination.shape)) + 'd', destination.shape, parameters)
    return destination
