...
def RunRFTestSuite(project_dir, ts_name, ts_subfolders, reports_folder):
    """
    ...
    test suite folders (parameter: ts_subfolders) and test suite name (parameter: ts_name)
    Note:  Based on the Robot Framework template structure
    
    Use Example:   Run RF Test Suite    ${EXECDIR}  TS001_TC001_Register_Your_Organization   Test_Suites\\TS001_Register_Your_Organization    TS001_Register_Your_Organization
    """
    ...
    a = "robot -d "
    b = "Reports" + "\\" + reports_folder + "\\" + ts_name + "-" + timestamp + " --timestampoutputs -r "
    c = "_reports.html -o "
    d = "_output.xml -l "
    e = "_log.html "
    f = ts_subfolders + "\\" + ts_name + ".robot"

    cmds = a + b + ts_name + c + ts_name + d + ts_name + e + f
...
def ContainsOnlyDigits(str_value):
    """
     Returns the count of the number of times a string value appears in a string
...
def GetValueInString(Value, str): 
    """
     Returns the string (str) passed in if it is contained within the Value or Returns FAIL if the string
       is not contained within the Value.
    """
...
def GetStringPosition(str,fnd): 
    """
    Returns the index of the positive of a string (fnd) contained in the string value passed in (str)
    """
    pos = 0
    ind = str.find(fnd)
    pos += str.find(fnd)
...
def GetUniqueItems(list_values):
  """
  Returns the unique list of values from argument list_values
  """
