    ...
    def get_closest_match(x, other, cutoff, report):
        ...
        if len(matches) > 0:
            if x != matches[0]:
                ...
                print('   found EXACT match for ', x, ': ', matches)
                return x
        else:
            print('    found NO matches for ' , x)
            # report[x] = None
            return x

    df2['original_right'] = df2[right_on]
    df2_copy = df2.copy()

    # Apply the get_closest_match for each x in df2 merge column.
    # This is VERY unoptimized but meh for now.
    # Save it in a new left_on column in the df2 df.
    df2_copy[left_on] = [get_closest_match(x, df1[left_on], cutoff, report)
                         for x in df2_copy[right_on]]
    # print('report', report)
    # print('df2_copy', df2_copy)
    # print('left_on', left_on)

    # Return the merged output along with a report on what happened.
    return df1.merge(df2_copy, on=left_on, how=how), report
...
def make_path_global_pyramid(input_path, output_path=None, make_overviews=True, overwrite_overviews=False,
                             calculate_stats=True, overwrite_stats=False,
                             clean_temporary_files=False, raise_exception=False, make_overviews_external=True, set_ndv_below_value=None, write_unique_values_list=False, verbose=False):
