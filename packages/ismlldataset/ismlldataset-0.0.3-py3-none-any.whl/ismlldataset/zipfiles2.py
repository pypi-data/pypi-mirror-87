...
mffile = "/home/hsjomaa/dataset2vec-tf2/metafeatures"
os.makedirs(temp,exist_ok=True)
maps = {}
ss = [_ for _ in os.listdir(path) if _!="trains"]
for dataset_id,folder in _map.items():
    ...
    pd.DataFrame(pd.read_csv(os.path.join(mffile,"meta-features-split-0.csv"),header=0,index_col=0).loc[folder]).to_csv(os.path.join(temp,"metafeatures.dat"),index=False)
    # break
