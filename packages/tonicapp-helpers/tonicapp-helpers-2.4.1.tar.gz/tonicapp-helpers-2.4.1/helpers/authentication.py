...
class TonicAuthentication():
    ...
    def decode_token(self, token, prefix):
        ...
        """
        if prefix != 'JWT':
            if 'CUSTOM_WEB_TOKEN' in settings_auth.keys() and token.decode('utf-8') in settings_auth["CUSTOM_WEB_TOKEN"].keys():
                return settings_auth['CUSTOM_WEB_TOKEN'][token.decode('utf-8')]

            elif token.decode('utf-8') in settings_auth['TONIC_WEB_TOKEN']:
                return {
                    'name': 'Tonic Admin',
                    'is_admin': True,
                    'user_id': '9999999',
