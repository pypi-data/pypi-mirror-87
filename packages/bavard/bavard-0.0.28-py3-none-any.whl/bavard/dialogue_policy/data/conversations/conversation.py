...
class Conversation(Encodable, Jsonable):
    def __init__(self, turns: List[Union[AgentDialogueTurn, UserDialogueTurn]]):
        ...
        self.is_last_turn_user = self.turns[-1].actor == Actor.USER
    ...
    def make_validation_pairs(self) -> Tuple[List["Conversation"], List[str]]:
        ...
        for i in range(len(self.turns) - 1):
            if self.turns[i].actor == Actor.USER and self.turns[i + 1].actor == Actor.AGENT:
                # This is a user turn and the next turn is an agent turn, so this is
                # a good splitting point to make a validation conversation.
                convs.append(cls(self.turns[:i + 1]))
                next_actions.append(self.turns[i + 1].action.action_name)
        return convs, next_actions
