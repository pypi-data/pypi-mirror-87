...
def get_az_cli_tokens(tenant: str) -> (str, str):
    ...
    if tokens is not None and len(tokens) > 0:
        datalakeToken = next((entry for entry in tokens if entry['resource'].startswith(DatalakeResourceUrl)), None)
        if datalakeToken is not None:
            return datalakeToken['accessToken'], datalakeToken['refreshToken']
        else:
            mgmtToken = next((entry for entry in tokens if entry['resource'].startswith(ResourceMgmtUrl) and entry['isMRRT']), None)
