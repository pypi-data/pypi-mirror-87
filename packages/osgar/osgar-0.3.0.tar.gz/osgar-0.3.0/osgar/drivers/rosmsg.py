...
#            return
 ...
 #       print(frame_id)
        ...
        #    import pdb
        #    pdb.set_trace()
        if frame_id.endswith(b'/camera_front') or frame_id.endswith(b'/camera_down') or frame_id.endswith(b'/camera_color_optical_frame') or frame_id.endswith(b'/depth_camera_front') or frame_id.endswith(b'/rgbd_front'):
            # used to be self.topic_type == 'sensor_msgs/CompressedImage'
            self.bus.publish('image', parse_jpeg_image(packet))
        elif frame_id.endswith(b'/front_laser') or frame_id.endswith(b'/laser_front') or frame_id.endswith(b'/lidar_front'):  # self.topic_type == 'sensor_msgs/LaserScan':
            self.count += 1
            if self.count % self.downsample != 0:
                return
