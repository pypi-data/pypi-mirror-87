...
class UserUttered(Event):
    ...
    def __init__(
        ...
        self.asr_confidence = asrConfidence if asrConfidence is not None else 1
        self.recording_id = recordingId if recordingId else None
        self.missed_intent = missed_intent

        if parse_data:
            self.parse_data = parse_data
            self.parse_data['asrConfidence'] = self.asr_confidence
            self.parse_data['recordingId'] = self.recording_id
            self.parse_data['missed_intent'] = self.missed_intent
            self.parse_data['message_id'] = self.message_id
        else:
            self.parse_data = {
                "intent": self.intent,
                "entities": self.entities,
                "text": text,
                "asrConfidence": self.asr_confidence,
                "recordingId": self.recording_id,
                "missed_intent": self.missed_intent,
...
class BotUttered(Event):
    ...
    def as_dict(self):
        d = super().as_dict()
        d.update({"text": self.text, "data": self.data, "metadata": self.metadata, "message_id": self.message_id})
        return d
...
class SlotSet(Event):
    ...
    def as_dict(self):
        d = super().as_dict()
        d.update({"name": self.key, "value": self.value, "message_id": self.message_id})
        return d
