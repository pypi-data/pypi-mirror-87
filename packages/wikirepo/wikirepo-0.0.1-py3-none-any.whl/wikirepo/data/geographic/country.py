...
# This script is used to derive occupied and break away regions by 
# finding those regions with a 'P17' (country) different than itself
# 
# The purpose is to allow the user to include these disputed regions 
# or not, and through this to allow for the analysis of "countries"
#
# Contents
# --------
#   0. No Class
...
def query_country(dir_name=None,
                  ents_dict=None,
                  depth=None,
                  locations=None, 
                  time_lvl=None, 
                  timespan=None):
    """
    Queries the country for given location(s) over the given time_lvl and timespan
    """
    ...
    fixes = [['Aruba', 'Netherlands'], # Territory
             ...
             ['Nauru', 'Nauru'], # Fix
             ['Republic of Artsakh', 'Azerbaijan'], # Recognized by most states as a part of Azerbaijan
             ...
             ['Turkish Republic of Northern Cyprus', 'Cyprus'], # Recognized by most states as a part of Cyprus
             ['Western Sahara', 'Morocco']] # 80% is controlled by Morocco

    for f in fixes:
        if f[0] in list(df[lctn_utils.depth_to_col_name(depth)]):
            df.loc[df.loc[df[lctn_utils.depth_to_col_name(depth)] == f[0]].index, col_name] = f[1]
