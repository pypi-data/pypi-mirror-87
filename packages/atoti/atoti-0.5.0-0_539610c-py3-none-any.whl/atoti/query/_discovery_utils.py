...
def _create_cube(discovery_cube: DiscoveryCube, session: QuerySession,) -> QueryCube:
    hierarchies = QueryHierarchies(
        {
            hierarchy["name"]: _create_hierarchy(dimension["name"], hierarchy)
...
def create_cubes_from_discovery(
    discovery: Discovery, session: QuerySession,
) -> QueryCubes:
