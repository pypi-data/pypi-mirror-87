...
def read_csv(path, sep=',', delimiter=None, names=None,  index_col=None,
    ...
    if engine == 'python' or engine == 'c':
        ...
        raise ValueError(f'Unknown engine: {engine} (valid options are "c", "python", or "dolphindb")')
...
def save_table(df, db_path, table_name, ignore_index=True, session=default_session()):
...
def pivot_table(data, values=None, index=None, columns=None, aggfunc='mean', fill_value=None, margins=False, dropna=True, margins_name='All', observed=False):
    ...
    return data.pivot_table(values, index, columns, aggfunc, fill_value, margins, dropna, margins_name, observed)
