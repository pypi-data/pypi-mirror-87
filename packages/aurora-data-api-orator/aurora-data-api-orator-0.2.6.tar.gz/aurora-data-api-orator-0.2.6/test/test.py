...
import aurora_data_api_orator  # noqa
from aurora_data_api_orator.mysql_error_codes import MySQLErrorCodes  # noqa

logging.basicConfig(level=logging.INFO)
logging.getLogger("aurora_data_api_orator").setLevel(logging.DEBUG)
...
class TestAuroraDataAPI(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with aurora_data_api_orator.connect(database=cls.db_name) as conn, conn.cursor() as cur:
            try:
                cur.execute("""
            ...
            except aurora_data_api_orator.DatabaseError as e:
    ...
    def test_invalid_statements(self):
        with aurora_data_api_orator.connect(database=self.db_name) as conn, conn.cursor() as cur:
            with self.assertRaisesRegex(aurora_data_api_orator.DatabaseError, "syntax"):
