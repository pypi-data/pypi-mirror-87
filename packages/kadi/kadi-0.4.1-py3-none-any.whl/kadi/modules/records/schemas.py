...
from .files import get_preview_type
...
class RecordSchema(KadiSchema):
    ...
    license = fields.Pluck(LicenseSchema, "name")
    ...
    def _deserialize_extras(self, value):
        return ExtraSchema(many=True).load(value)
    ...
    def _generate_links(self, obj):
        ...
        if self._internal:
            links["view"] = url_for("records.view_record", id=obj.id, _external=True)
...
class FileSchema(KadiSchema):
    ...
    def _generate_links(self, obj):
        ...
        if get_preview_type(obj) is not None:
            links["preview"] = url_for(
                "api.get_file_preview", record_id=obj.record_id, file_id=obj.id
            )
