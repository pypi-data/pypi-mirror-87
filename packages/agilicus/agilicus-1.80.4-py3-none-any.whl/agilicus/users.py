...
def add_user_role(ctx, user_id, application, roles, org_id=None):
    token = context.get_token(ctx)
...
def format_user_application_access_info_as_text(info):
    columns = [
...
def format_user_requests_as_text(info):
    columns = [
        metadata_column("id"),
        spec_column("org_id"),
        spec_column("user_id"),
...
def update_user_request(ctx, user_request_id, **kwargs):
    ...
    model = _get_user_request(apiclient, user_request_id, user_id=user_id)
...
def action_user_request(ctx, user_request_id, state, **kwargs):
    ...
    model = _get_user_request(apiclient, user_request_id, org_id=org_id)
    kwargs["state"] = state
    model.spec = build_updated_model(agilicus.UserRequestInfoSpec, model.spec, kwargs)
...
def show_user_request(ctx, user_request_id, **kwargs):
    apiclient = context.get_apiclient_from_ctx(ctx)
    user_id = input_helpers.get_user_id_from_input_or_ctx(ctx, **kwargs)
    return _get_user_request(apiclient, user_request_id, user_id=user_id).to_dict()
...
def format_user_metadata_as_text(info):
    columns = [
        metadata_column("id"),
        spec_column("name"),
        spec_column("user_id"),
...
def format_service_account_as_text(info):
    columns = [
        metadata_column("id"),
        spec_column("name"),
        spec_column("org_id"),
