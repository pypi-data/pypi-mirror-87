...
class Request(object):
    ...
    def get_header(self, name, default=None):
        ...
        return self.environ.get(
            "HTTP_" + name.upper().replace("-", "_"), default
        )
    ...
    def path_info(self, environ_to_str=environ_to_str) -> str:  # type: ignore
        ...
        try:
            return environ_to_str(
                self.environ.get("PATH_INFO", ""), self.charset
            )
        except UnicodeDecodeError:
            raise exceptions.BadRequest
    ...
    def script_name(self):
        ...
        try:
            return environ_to_str(
                self.environ.get("SCRIPT_NAME", ""), self.charset
            )
        except UnicodeDecodeError:
            raise exceptions.BadRequest
    ...
    def query_string(  # type: ignore
        self, environ_to_str=environ_to_str,
    ) -> Optional[str]:
        """\
