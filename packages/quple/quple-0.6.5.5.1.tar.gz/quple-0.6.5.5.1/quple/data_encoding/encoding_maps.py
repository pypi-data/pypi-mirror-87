...
class trial_2_cmr_qiskit:
    def __init__(self, degree=1):
        self.degree = degree
    def __call__(self, x: np.ndarray):
        if len(x) == 1:
            coeff = x[0]**self.degree/sp.pi**2
        elif len(x) == 2:
            coeff = (x[0]+x[1])**self.degree/sp.pi**3
        elif len(x) == 3:
            coeff = (x[0]+x[1]+x[2])**self.degree/sp.pi**4
        elif len(x) == 4:
            coeff = (x[0]+x[1]+x[2]+x[3])**self.degree/sp.pi**5
        else:
