'''
引用该文件from BasicLibrary.identity_random.identity import IdNumber
生成身份证的方法是：IdNumber.generate_id(sex=0, count=3)  sex = 0表示女性，sex = 1表示男性，count表示生成数据条数
返回数据格式为：['', '', ''] 格式
'''
...
# 导入常量并重命名
from BasicLibrary.identity_random import constant as const
...
class IdNumber(str):
    ...
    @classmethod
    def generate_id(cls, sex=0, count=1):
        ...
        for i in range(count):
            ...
            # 限定出生日期范围(8位数)
            start, end = datetime.strptime("1960-01-01", "%Y-%m-%d"), datetime.strptime("2019-12-30", "%Y-%m-%d")
            ...
            # 校验码(1位数)
            random_id.append(id_number + str(cls(id_number).get_check_digit()))
        return random_id


if __name__ == '__main__':
    random_sex = random.randint(0, 1)  # 随机生成男(1)或女(0)
    print(IdNumber.generate_id(random_sex,3))  # 随机生成身份证号
