...
def run_example(device_id, do_plot=False, scope_stream_rate=12, scope_inputselects=[16, 32]):
    ...
    if "DEMODS" in node_branches:
        ...
        exp_setting.append(["/%s/demods/%d/timeconstant" % (device, demod_channel), 0.0])
    ...
    while n < num_scope_samples:
        ...
        print(
            ...
            f"blocks processed {num_blocks}, samples acquired {n}.\r"
        )
        for b, block in enumerate(data[stream_nodepath]):
            if block["flags"] & 1:
                message = f"Block {b} from poll indicates dataloss (flags: {block['flags']})"
                warnings.warn(message)
                continue
            if block["flags"] & 2:
                # This should not happen.
                message = f"Block {b} from poll indicates missed trigger (flags: {block['flags']})"
                warnings.warn(message)
                continue
            if block["flags"] & 3:
                message = f"Block {b} from poll indicates transfer failure  (flags: {block['flags']})"
                warnings.warn(message)
            assert (
    ...
    print()
    print(f"Total blocks processed {num_blocks}, samples acquired {n}.")
    ...
    for c, channel_samples in enumerate(scope_samples):
        ...
        if nan_count:
            ...
            warnings.warn(
                ...
                % (c, int(nan_count), len(scope_samples[c]["value"]), nan_index[0])
            )
        if zero_count:
            warnings.warn("Scope channel %d timestamps contain %d entries equal to 0." % (c, int(zero_count)))
    ...
    if do_plot:
        ...
        for c, channel_samples in enumerate(scope_samples):
            ...
            ax.plot(t * 1e6, channel_samples["value"][0:n])
        ...
        ax.set_title(r"Scope streaming data portion (total duration acquired %.1f s)" % dt)
        plt.show()
