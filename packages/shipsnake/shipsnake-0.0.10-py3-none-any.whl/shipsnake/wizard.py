...
def main():
	"""
	The setup wizard for shipsnake
	"""
		...
		if 'github' not in data['build']:
			...
			if not ghrepo:
				...
				if create:
					...
					os.system(f'git init >> build/log.txt;git config user.name "{data["author"]}";git config user.email "{data["email"]}";git add .;git commit -m ":rocket: Initial Commit!" >> build/log.txt;git remote add origin git@github.com:{ghusername}/{data["short_name"]}.git >> build/log.txt;git push -u origin master;')
				else:
	...
	print('\n\n\033[1;31m`shipsnake.toml` has been created. Please edit that for further changes and advanced options.\033[0m')