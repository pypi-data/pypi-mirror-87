...
class fpa:
    ...
    def __init__(self, df=pd.DataFrame()):
        ...
        self.analysis = []  # General container for functions
        self.records = []  # container for make_records
    ...
    def import_custom_xl(
        ...
        ws = wb[ws_name]
        dim_names = [] if dim_names is None else dim_names.copy()
        if dim_names_coords.__ne__(None) and dim_names_coords.__ne__([None]):
            for item in dim_names_coords:
                dim_name_from_ws = ws[item].value
                dim_names.append(dim_name_from_ws)
        dim_values = [] if dim_values is None else dim_values.copy()
        ...
        if dim_values.__ne__([None]) and dim_names.__ne__([None]):
            self.add_dimensions(dim_names.copy(), dim_values.copy(), data_obj='block')
        #Add meta data
        meta_df = pd.DataFrame(
            index=[data_block_value],
            columns=[
    ...
    def import_xl_directory(
        ...
        if not table_coords_d:
            table_coords_in = table_coords_d
        else:
            table_coords_in  = table_coords_d.copy() if isinstance(table_coords_d, list) else [table_coords_d]
        if not dim_names_d:
            dim_names_in = dim_names_d
        else:
            dim_names_in = dim_names_d.copy() if isinstance(dim_names_d, list) else [dim_names_d]
        if not dim_vals_d:
            dim_vals_in = dim_vals_d
        else:
            dim_vals_in = dim_vals_d.copy() if isinstance(dim_vals_d, list) else [dim_vals_d]
        if not dim_names_coords_d:
            dim_names_coords_in = dim_names_coords_d
        else:
            dim_names_coords_in = dim_names_coords_d.copy() if isinstance(dim_names_coords_d, list) else [dim_names_coords_d]
        if not dim_coords_d:
            dim_coords_in = dim_coords_d
        else:
            dim_coords_in = dim_coords_d.copy() if isinstance(dim_coords_d, list) else [dim_coords_d]

        file_list = pd.Series(os.listdir(dir_path))
        file_list = file_list[~file_list.str.contains('~', case=False, na=False)]
        file_list = file_list[file_list.str.contains('.xlsx', case=False, na=False) |
                              file_list.str.contains('.xls', case=False, na=False)]
        if xl_id != None:
    ...
    def update_custom_xl(
        ...
        :param dim_coords: Excel references to get dimension values from the Excel.  i.e. ['B3', 'B4'].
        """
        if isinstance(f_path, pd.DataFrame):
            if 'Data_Block' in f_path.index:
                f_path.drop('Data_Block', inplace=True)
            self.block = f_path
        else:
        ...
        for dir in dir_list:
            ...
            for file in file_list:
                ...
                if table_coords is None:
                    for wsheet in wb_sheets:
                        self.import_xl(f_path, wsheet)
                        self.add_dimensions(["Work_Sheet"], [wsheet], 'block')
                        self.add_block_to_data()
                else:
                    for wsheet in wb_sheets:
                        self.update_custom_xl(
    ...
    def add_dimensions(
        ...
        :param data_obj: Which data object you want o effect.  Available - 'block', 'slice', 'consolidation', 'analysis', 'data'
        :return: data_obj
        """
        ...
        if data_obj == "block":
            dfi_add = self.block.index.to_frame()
            for dim_name, dim_value in zip(new_dimensions, dim_values_to_add):
                dfi_add.insert(col_num, dim_name, dim_value)
            self.block.index = pd.MultiIndex.from_frame(dfi_add)
        elif data_obj == "slice":
            dfi_add = self.slice.index.to_frame()
            for dim_name, dim_value in zip(new_dimensions, dim_values_to_add):
                dfi_add.insert(col_num, dim_name, dim_value)
            self.slice.index = pd.MultiIndex.from_frame(dfi_add)
        elif data_obj == "consolidation":
            dfi_add = self.consolidation.index.to_frame()
            for dim_name, dim_value in zip(new_dimensions, dim_values_to_add):
                dfi_add.insert(col_num, dim_name, dim_value)
            self.consolidation.index = pd.MultiIndex.from_frame(dfi_add)
        elif data_obj == "analysis":
            dfi_add = self.analysis.index.to_frame()
            for dim_name, dim_value in zip(new_dimensions, dim_values_to_add):
                dfi_add.insert(col_num, dim_name, dim_value)
            self.analysis.index = pd.MultiIndex.from_frame(dfi_add)
        else:
            if (
                len(new_dimensions) == len(dim_values_to_add)
                and isinstance(dim_values_to_add[0], list) is False
            ):
                dfi_add = self.data.index.to_frame()
                for dim_name, dim_value in zip(new_dimensions, dim_values_to_add):
                    dfi_add.insert(col_num, dim_name, dim_value)
                self.data.index = pd.MultiIndex.from_frame(dfi_add)
            else:
                dfi_add = self.data.index.to_frame()
                index_counts = self.data._count_level(0).iloc[:, 0]
                index_counts = index_counts.reindex(
                    dfi_add.index.get_level_values("Data_Block").unique()
                )
                for new_dim_name in new_dimensions:
                    ...
                    dfi_add.insert(1, new_dim_name, dim_value_list.values)
            self.data.index = pd.MultiIndex.from_frame(dfi_add)
    ...
    def merge_dim_from_accts(self, dim_set, base_dim, new_dims, data_obj="data"):
        ...
        if data_obj == "block":
            ...
            dfi = self.analysis.index.to_frame()
            dfi.reset_index(drop=True, inplace=True)
            new_dims.append(base_dim)
            dim_df = dim_df.loc[:, new_dims]
            dim_df.set_index(base_dim, inplace=True)
            dfi = dfi.join(dim_df, base_dim)
            self.analysis.index = pd.MultiIndex.from_frame(dfi)
        else:
    ...
    def reorder_dimensions(self, new_order, data_obj="data"):
        ...
        if data_obj == "block":
            ...
            self.analysis= self.analysis.reorder_levels(new_order)
        elif data_obj == "variance":
    ...
    def rename_dimensions(self, dim_list, data_obj="data"):
        ...
        elif data_obj == "records":
            self.records.index.names = dim_list
        elif data_obj == "consolidation":
            self.consolidation.index.names = dim_list
        else:
            self.analysis.index.names = dim_list
    ...
    def rename_dim_item(self, dim, old, new, data_obj="data"):
        ...
        if data_obj == "data":
            ...
            idx = self.analysis.index.to_frame()
            idx.fillna("nval", inplace=True)
            idx[dim] = idx[dim].str.replace(old, new)
            self.analysis.index = pd.MultiIndex.from_frame(idx)
        elif data_obj == "consolidation":
    ...
    def reorder_index_dim(
        ...
        if data_obj == "data":
            ...
            index_items = list(set(self.analysis.index.get_level_values(dim)))
            for item_no in new_order:
                index_items.remove(item_no)
            new_order.reverse()
            for item_no in new_order:
                index_items.insert(0, item_no)
            self.analysis= self.analysis.reindex(
                index_items, axis=axis_target, level=dim, fill_value="nval"
            )
        elif data_obj == "consolidation":
    ...
    def move_dims_to_col(self, dims, data_obj="data"):
        ...
        if data_obj == "data":
            ...
            self.analysis.reset_index(dims, inplace=True)
    ...
    def move_col_to_dims(self, dims, data_obj="data"):
        ...
        if data_obj == "data":
            ...
            self.analysis.set_index(dims, append=True, inplace=True)

    def make_records(self, data_obj="data"):
        """
        Change the data object to a records format rather than a table.  Works bes on single level columns.  Result in
        .records object

        :param data_obj: Which data object you want o effect. Default: 'data'.  Available - 'block', 'data', 'slice',
         'consolidation', 'analysis', 'variance'
        """
        if data_obj == "data":
            self.records = self.data.copy().stack()
            if isinstance(self.records, pd.Series) and not self.records.name:
                self.records.name = 'Amount'
        elif data_obj == "slice":
            self.records = self.slice.copy().stack()
            if isinstance(self.records, pd.Series) and not self.records.name:
                self.records.name = 'Amount'
        elif data_obj == "block":
            self.records = self.block.copy().stack()
            if isinstance(self.records, pd.Series) and not self.records.name:
                self.records.name = 'Amount'
        elif data_obj == "variance":
            self.records = self.variance.copy().stack()
            if isinstance(self.records, pd.Series) and not self.records.name:
                self.records.name = 'Amount'
        elif data_obj == "consolidation":
            self.records = self.consolidation.copy().stack()
            if isinstance(self.records, pd.Series) and not self.records.name:
                self.records.name = 'Amount'
        else:
            print("no object")

        namer = list(self.records.index.names)
        count = 1
        for item in namer:
            if item == None:
                idx_item = namer.index(item)
                namer[idx_item] = 'Col' + str(count)
                count += 1
        self.rename_dimensions(namer, 'records')
    ...
    def drop_dimension(self, dimension_drop, data_obj="data"):
        ...
        if data_obj == "data":
            ...
            self.analysis.index = self.analysis.index.droplevel(dimension_drop)
    ...
    def consol_dimension(self, dims="Data_Block", data_obj="data"):
        ...
        if data_obj == "data":
            ...
            self.consolidation = self.analysis.groupby(level=dims, sort=False).sum()
        elif data_obj == "block":
    ...
    def keyword_slice(self, keywords, dims=None, data_obj="data", append_to=False):
        """
    ...
    def variance_analysis(self, dim_name, dim1, dim2, data_obj="data"):
        ...
        :param data_obj: Which data object you want o effect.  Default: 'data'. Available - 'data', 'slice', 'consolidation'
        ...
        if data_obj == "data":
            ...
            df1 = self.consolidation.xs(dim1, level=dim_name)
            df1 = df1.droplevel("Data_Block")
            df2 = self.slice.xs(dim2, level=dim_name)
            df2 = df2.droplevel("Data_Block")
        amt_var = df1 - df2
    ...
    def plot_object(self,
        ...
        if data_obj == 'data':
            df = self.data.copy()
            # if the data is a table format
            if table_format is True:
                df = df.stack()
                if isinstance(df, pd.Series) and not df.name:
                    df.name = 'Amount'
            df = df.reset_index()
        elif data_obj == 'slice':
            df = self.slice.copy()
        ...
        if expanded is True:
            ...
            figsize_w = input('Width of Chart [default = 800]:')
            figsize_w = 800 if figsize_w == '' else int(figsize_w)
            figsize_h = input('Height of Chart [default = 500]:')
            figsize_h = 500 if figsize_h == '' else int(figsize_h)
            num_fmt = input('Hover Number Format [default = 0,0]:')
            num_fmt = "0,0" if num_fmt == '' else num_fmt
        #Actual Charting
        output_file(fpath)
        if kind == 'bar':
            df.plot_bokeh.bar(
            ...
            df.plot_bokeh.scatter(x=x_axis,
                                  y=y_axis,
                                  category=scat_cat,
                                  xlabel=xlabel,
                                  ylabel=ylabel,
    ...
    def multiply_dim(
        ...
        if not isinstance(calc_name, list):
            calc_name = [calc_name]
        self.add_dimensions(dim_name, calc_name, data_obj="analysis")
        self.import_xl(self.analysis.copy())
        return self.analysis
    ...
    def get_duplicates(self, based_on=None, data_obj="data"):
        ...
        if data_obj == "data":
            ...
            self.analysis.reset_index(level=lx, inplace=True)
            if based_on == None:
                based_on = list(self.analysis.columns)
            if isinstance(based_on, str):
                based_on = [based_on]
            try:
                self.analysis= pd.concat(
                    z for _, z in self.analysis.groupby(based_on) if len(z) > 1
                )
            except ValueError as e:
                if str(e) == "No objects to concatenate":
                    print("No duplicates.")
                    pass
            self.analysis.set_index(lx, append=True, inplace=True)
            self.reorder_dimensions(index_order, "analysis")
            return self.analysis

    def _align_indicies(self):
        #Pull columns from block that are in the index for data.
        col_x = list(self.block.columns)
        idx_y = list(self.data.index.names)
        move_cols = [x for x in col_x if x in idx_y]
        self.move_col_to_dims(move_cols, 'block')
        ...
        for item in w:
            if item[1] == False:
                self.add_dimensions([item[0]], ["nval"], data_obj='block')
        z = [item in y for item in x]
        w = zip(x, z)
        for item in w:
            if item[1] == False:
                self.add_dimensions([item[0]], ["nval"], 1, "data")
