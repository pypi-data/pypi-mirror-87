...
from atoti._measures.extrema_member_measures import MinMaxMemberMeasure
...
from ._measures.restricted_measure import RestrictedMeasure
...
def _stop(measure: Measure, *at: Level) -> Measure:
    ...
    return RestrictedMeasure(measure, set(at))
...
def max_member(measure: MeasureOrMeasureConvertible, level: Level) -> Measure:
    ...
    return MinMaxMemberMeasure(measure, level, True)
...
def min_member(measure: MeasureOrMeasureConvertible, level: Level):
    if not isinstance(measure, Measure):
        measure = measure._to_measure()
    return MinMaxMemberMeasure(measure, level, False)


@doc(_SCOPE_DOC)
def sum_product(
    *factors: MeasureOrMeasureConvertible, scope: Optional[Scope] = None,
) -> Measure:
