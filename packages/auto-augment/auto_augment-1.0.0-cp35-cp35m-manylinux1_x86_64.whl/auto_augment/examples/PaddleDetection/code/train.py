...
# autoaumgent modify
import _init_paths
from ppdet_utils.hub_utils import create_reader, DetReaderStatus
...
def main():
    ...
    for it, data in enumerate(train_loader()):
        ...
        for i in range(129806):
            if i == 129804:
                import pdb
                pdb.set_trace()
                pass
            logger.info("test get it {} data".format(i))
            outs = exe.run(compiled_train_prog, fetch_list=train_values, feed=data)
            logger.info("after on_batch_begin")

        # get it 129804 data ,then die
        logger.info("get it {} data".format(it))
        it = it + start_iter
        #reader_status.on_batch_begin(it)
        logger.info("after on_batch_begin")
        ...
        outs = exe.run(compiled_train_prog, fetch_list=train_values, feed=data)
        logger.info("it {} exe.run done".format(it))
