...
def _defineSymbols():
    ...
    if _java_type_ is None:
        ...
        _java_type_ = jpy.get_type("com.illumon.iris.db.util.config.TableInputHandler")
        _Enum_Values_ = jpy.get_type("com.illumon.iris.db.util.config.TableInputHandler$EnumValues")
...
@_passThrough
def c(name, type, *attributes):
    """
    Creates a column configuration for a "normal" input table column.
    
    :param name: (java.lang.String) - column name
    :param type: (java.lang.Class) - column type
    :param attributes: (com.illumon.iris.db.util.config.TableInputHandler.ColumnConfigAttribute...) - column attributes
    :return: (com.illumon.iris.db.util.config.TableInputHandler.ColumnConfig) column configuration
    """
    
    return _custom_c(name, type, *attributes)


@_passThrough
def cKey(name, type, *attributes):
    """
    ...
    return _custom_cKey(name, type, *attributes)


@_passThrough
def getRestrictedColumnNames():
    """
    Gets the set of special column names that may not be used with input tables.
    
    :return: (java.util.Set<java.lang.String>) the set of special column names that may not be used with input tables.
    """
    
    return _custom_getRestrictedColumnNames()
