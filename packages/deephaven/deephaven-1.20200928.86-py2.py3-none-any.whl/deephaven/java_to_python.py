...
__remoteTableProxy__ = 'com.illumon.iris.db.tables.remote.RemoteTableHandle$TableProxy'
...
def _fillRectangular(javaArray, shape, basicType, convertNulls):
    ...
    def fillValuesIn(dimension, ndElement, arrElement):
        ...
        if dimension < len(shape)-1:
            ...
            if basicType.startswith('com.illumon.iris.db.tables.dbarrays.Db'):
                # convert each leaf
                for i, leafElement in enumerate(arrElement):
                    ndElement[i] = convertJavaArray(leafElement.toArray(), convertNulls=convertNulls)
            elif basicType == 'com.illumon.iris.db.tables.utils.DBDateTime':
                # get long array
...
def freezeTable(table):
    ...
    # if table is remote proxy, then the emptyTable must be known by the proxy
    maybeRemote = jpy.cast(table, jpy.get_type(__remoteTableProxy__))
    if isinstance(maybeRemote, jpy.get_type(__remoteTableProxy__)):
        return maybeRemote.getRemoteTableHandle().getTableManager().getProcessorConnection().getRemoteDatabase() \
          .makeRemote(emptyTable(0)).snapshot(table, True)
    else:
...
def columnToSeries(table, columnName, convertNulls=NULL_CONVERSION.ERROR):
    ...
    if columnType == 'com.illumon.iris.db.tables.utils.DBDateTime':
