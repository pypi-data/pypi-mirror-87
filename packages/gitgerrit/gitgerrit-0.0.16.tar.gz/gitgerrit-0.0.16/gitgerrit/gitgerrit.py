...
def get_changed_files_in_commit(repo, commit):
    base_commit = repo.commit(commit)
    parent_commit = repo.commit(f"{commit}^")
    changed_files = []
    for diff in base_commit.diff(parent_commit):
        changed_files.append(diff.a_blob.path)
        if diff.b_blob:
            changed_files.append(diff.b_blob.path)
    return list(set(changed_files))

def is_part_of_patch(repo, commit, filename):
    files = get_changed_files_in_commit(repo, commit)
    LOGGER.debug(f"Commit {commit} as following changed files: {files}")
...
def parse_args(gerrit_config):
    ...
    review_parser.add_argument(
        ...
        metavar="N",
        default="Unspecified",
        help="Run ID shown in the gerrit message"
    )
...
def _get_payload(git_repo, commit, payload_json, path_prefixes, robot_id, robot_run_id):
    ...
    for key in comments:
        ...
        if not isFileInRepo(git_repo, new_key):
            LOGGER.debug(f"File {key} contains issues reported by static analysis but its not part of the git repository at {git_repo.working_dir} - Skipping!")
            continue

        if not is_part_of_patch(git_repo, commit, new_key):
            LOGGER.debug(f"File {new_key} contains issues reported by static analysis but its not part of the currently processed commit {commit} - Skipping!")
            continue
        ...
        for item in comments[key]:
            ...
            item["robot_run_id"] = robot_run_id
            issues = issues + 1
    ...
    extra = ""
    report_url = os.environ.get("CC_REPORT_URL", None)
    if report_url:
        extra = f"See {report_url} for more details"

    payload["message"] = f"Jenkins found {issues} issue(s) within this pull request. Some issues might be filtered. {extra}"
...
def review(rest, git_repo, args, gerrit_config):
    ...
    payload = _get_payload(git_repo, args.commit, args.payload, args.path_prefixes, args.robot_id, args.robot_run_id)
