...
def match (n, l, aux, aux_param, inf_amp=None, param_amp=None) :
 ...
 if np.any ((aux[:, 0]==str(n)) & (aux[:, 1]==str(l))) : 
   ...
   if np.any ( (aux[:,0]==str(n+incr)) & (aux[:,1]==0) & (aux_param[:,2]==0) ) :
     l_ref = 0
     ref_ratio = 1.
   else :
     l_ref = 1
     ref_ratio = 1.8
   value = aux_param[(aux[:,0]==str(n+incr)) & (aux[:,1]==0) & (aux_param[:,2]==0), 0][0] / ref_ratio
   value_err = aux_param[cond2, 1][0]
     
 elif (np.any (aux[:, 0]==str(n+incr))) :
...
def fill_param (pkb, name, inf_a2z, param_a2z) :
  ...
  for ii in range (pkb.shape[0]) :
      v, verr = match (int(pkb[ii, 0]), int (pkb[ii, 1]), aux, aux_param, inf_amp=inf_amp, param_amp=param_amp)
      pkb[ii, ind] = v
      pkb[ii, ind+1] = verr
...
def a2z_to_pkb_nopandas (n, l, inf_a2z, param_a2z) :
  ...
  if np.any (inf_a2z[:, 2]=='split') :
    ...
    pkb[(pkb[:,1]==4)|(pkb[:,1]==5), :] = 0.4 #fixing splitting for high degrees modes 
