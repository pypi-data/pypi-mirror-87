...
class WeiboCrawl():
    ...
    def weibo_parse(self, web_content):
        ...
        for item in list_item:
            try:
                ...
                dianzan = None
                try:
                    zhuanfa = lis[1].find_all('em')[1].text.replace('转发','0').strip()
                    pinglun = lis[2].find_all('em')[1].text.replace('评论','0').strip()
                    dianzan = lis[3].find_all('em')[1].text.replace('赞','0').strip()
                except:
                    pass
    ...
    # 搜索微博
    def search_weibo2(self, keyword, day=-365*50, pn=None):
        self.keyword = keyword
        result=None
        try:
            self.df_weibo = pd.DataFrame(columns=['mid', 'user_id', 'user_name', 'idendity', 'member', 'weibo_stamp', 'weibo_date', 'weibo_time', 'source', 'original', 'content', 'retweet_count', 'comment_count', 'like_count', 'topics', 'at_users', 'retweet'])
            # self.param['weibo']['id'] = '100606'+keyword
            self.urls['weibo2']=self.urls['base'].format(keyword)
            self.param['weibo2']['page'] = 0

            # for index in range(1, pn+1):
            self.weibo_next2(keyword, day=day, pn=pn)
    ...
    # 微博翻页
    def weibo_next2(self, keyword, day=-365*50, pn=None):
        result=None
        try:
            if pn is not None:
    ...
    def go(self):
        self.update_info()
        df=self.search_weibo2('5140353859', pn=1)
        print(df)
        df.to_excel('weibo.xlsx')
        #
        # df=self.search_keywords('杭州女子失踪案', pn=12)
        # print(df)
        # df.to_excel('杭州女子失踪案.xlsx')

        # df = self.search_topics('#鲍毓明发文十问涉事女孩#', pn=50)
        # # df=self.search_topics('https://s.weibo.com/weibo?q=%23%E9%B2%8D%E6%9F%90%E6%9F%90%E6%B6%89%E5%AB%8C%E6%80%A7%E4%BE%B5%E6%A1%88%E8%B0%83%E6%9F%A5%E6%83%85%E5%86%B5%23', pn=50)
        # print(df)
        # df.to_excel('#鲍毓明发文十问涉事女孩#_topic.xlsx')
