...
AXIS_STATUS = (
    ...
    'IFOV Enabled',
    'Not Virtual',
    'Calibration Enabled 1D',
    'Calibration Enabled 2D',
    'Master/Slave Control',
    'Joystick Control',
    'Backlash Active',
    'Gain Mapping Enabled',
    'Stability0',
    'Motion Blocked',
    'Move Done',
    'Motion Clamped',
...
DRIVE_STATUS = (
    'Enabled',
    'CW End Of Travel Limit Input Level',
    'CCW End Of Travel Limit Input Level',
    'Home Limit Input Level',
    'Marker Input Level',
    'Hall A Input Level',
    'Hall B Input Level',
    'Hall C Input Level',
    'Sine Encoder Input Error',
    'Cosine Encoder Input Error',
...
def decode_response(response, response_values):
    '''
    This function evaluates an integer value representing a bitmask against
    a tuple object that contains sorted corresponding entries
    Args:
        response (int): Integer value that serves as a bitmask
        response_values (tuple): Tuple that is masked by response integer
    Returns:
        list: List of values that are marked true by the masking integer
    '''
    mask = [bool((1 << i) & response) for i in range(len(response_values))]
    status = [val for val, bit in zip(response_values, mask) if bit]
    return status
    ...
    def validate_axes(original_function):
        ...
        def wrapper_function(self, axes, *args, **kwargs):
            ...
            if axes_valid:
                ...
                raise ValueError('Axes not valid')
        return wrapper_function
...
class TcpIpSocketDummy:
    ...
    def send(self, message, blocking=True):
        ...
        if 'AxisStatus' in message:
            return Message(f'%{2**(len(AXIS_STATUS) + 1) - 1}\n')
        elif 'DriveStatus' in message:
            return Message(f'%{2**(len(DRIVE_STATUS) + 1) - 1}\n')
        return Message('%Test\n')
...
class A3200Controller:
    ...
    def get_task_state(self):
        ...
        status = [TASK_STATE[val] for val in ret]
        return status

    def PSO_arm(self, axis):
        self.send_command(f'PSOCONTROL {axis} ARM')
    
    def PSO_off(self, axis):
        self.send_command(f'PSOCONTROL {axis} OFF')
    
    def PSO_reset(self, axis):
        self.send_command(f'PSOCONTROL {axis} RESET')
    
    def PSO_track_input(self, axis, input_):
        self.send_command(f'PSOTRACK {axis} INPUT {input_}')
    
    def PSO_window_input(self, axis, input_):
        self.send_command(f'PSOWINDOW {axis} 1 INPUT {input_}')
    
    def PSO_output_control(self, axis):
        self.send_command(f'PSOOUTPUT {axis} CONTROL 0 1')
    
    def PSO_pulse(self, axis, time_on=1, time_off=1, cycles=1):
        self.send_command(f'PSOPULSE {axis} TIME {time_on},{time_off} \
                          CYCLES {cycles}')
    
    def PSO_window_load(self, axis, value):
        self.send_command(f'PSOWINDOW {axis} 1 LOAD {value}')
    
    def PSO_distance_fixed(self, axis, distance):
        self.send_command(f'PSODISTANCE {axis} FIXED UNITSTOCOUNTS({axis}, \
                          {distance})')
    
    def PSO_window_range(self, axis, lower, upper):
        self.send_command(f'PSOWINDOW {axis} 1 RANGE UNITSTOCOUNTS({axis}, \
                          {lower}), UNITSTOCOUNTS({axis}, {upper})')
    
    def PSO_output_pulse_window_mask(self, axis):
        self.send_command(f'PSOOUTPUT {axis} PULSE WINDOW MASK')
    
    def init_PSO(self, axis):
        self.PSO_reset(axis)
        self.PSO_track_input(axis, 0)
        self.PSO_window_input(axis, 0)
        self.PSO_output_control(axis)
        self.PSO_pulse(axis)
        
    def load_PSO_window(self, axis, pulse_sep, lower, upper):
        self.PSO_window_load(axis, 0)
        self.PSO_distance_fixed(axis, pulse_sep)
        self.PSO_window_range(axis, lower, upper)
        self.PSO_output_pulse_window_mask(axis)
