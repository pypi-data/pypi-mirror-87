...
def edit_config(model_selected, input_config_dir, output_config_dir, num_steps, label_map_path, record_dir, eval_number, annotation_type, 
        ...
        for v in summary.summary.value:
            if not 'image' in v.tag:
                if v.tag in log_dict.keys():
                    ...
                    log_dict[v.tag]["step"].append(str(len(log_dict[v.tag]["step"])+1))
                    log_dict[v.tag]["value"].append(str(tf.cast(decoded, tf.float32).numpy()[0]))
                else:
                    decoded = tf.compat.v1.decode_raw(v.tensor.tensor_content, tf.float32)
                    if type=='train':
                        scalar_dict = {"step": [0], "value": [str(tf.cast(decoded, tf.float32).numpy()[0])]}
...
def export_graph(ckpt_dir, exported_model_dir, pipeline_config_dir,
    ...
    '''
    pipeline_config_path = os.path.join(pipeline_config_dir,"pipeline.config")
