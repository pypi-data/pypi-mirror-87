        ...
        for grid_item in self.processed_grids:
            ...
            for measure_type in self.common_measure_types:
                ...
                if measure is None:
                    ...
                    logger.debug("Updating the order of the given measure {} :\
                            was {} becomes {}".format(measure.label, measure.order,
                            measure_type.order))
...
""".format(len(grids)/12)
    ...
    def _process_common_measures(self):
        ...
        """
        logger.debug(
            u"  + Processing specific datas for child {}".format(self.__class__.__name__)
        )
...
def _collect_operations(upload_id, data_type, upload_date):
    ...
    closure_day = 0
    closure_month = 0
    if(config.get("accounting_closure_day") != None and
            config.get("accounting_closure_month") != None):
    ...
    current_exercice_begining_date = date(current_year, begining_month,
            begining_day)
    current_exercice_end_date = date(current_year, closure_month,
            closure_day)
    ...
    current_exercice_operations = AccountingOperation.query().filter(
        AccountingOperation.date.between(current_exercice_begining_date,
            current_exercice_end_date)
    ).all()
    ...
    if not closure_is_done:
        ...
        last_exercice_operations = AccountingOperation.query().filter(
            AccountingOperation.date.between(last_exercice_begining_date,
                last_exercice_end_date)
        ).all()
...
def compile_measures_task(self, upload_id, grid_type=None):
    ...
    if grid_type == "treasury":
        ...
        operations = _collect_operations(upload_id, upload.filetype,
...
def scheduled_compile_measure_task(self, force=False):
    ...
    Eg:
        ...
        task = endi_celery.tasks.accounting_measure_compute.scheduled_compile_measure_task
        type = crontab
