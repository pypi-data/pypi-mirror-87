class botInfo:
    def __init__(self, data):
        ...
        try: self.certified: bool = data["certified"]
        except (KeyError, TypeError): self.certified: bool = False
...
class botReviews:
    ...
    def botReviews(self):
        for comment in self.json:
            try: self.id.append(comment["id"])
            except (KeyError, TypeError): self.id.append('None')
            try: self.botId.append(comment["botId"])
            except (KeyError, TypeError): self.botId.append('None')
            try: self.reviewerId.append(comment["reviewerId"])
            except (KeyError, TypeError): self.reviewerId.append('None')
            try: self.isPositive.append(comment["positive"])
            except (KeyError, TypeError): self.isPositive.append(True)
            try: self.review.append(comment["review"])
            except (KeyError, TypeError): self.review.append('None')
            try: self.reply.append(comment["reply"])
            except (KeyError, TypeError): self.reply.append('None')
