...
class rx:
    ...
    interesting = re.compile("""
        ^ \s*
         ...
         (ErrorLog | CustomLog | ForensicLog | (Error)?LogFormat |                  # log directivess
...
class vhost:
        ...
        for dir,args  in rx.configline.findall(src):    # or .finditer()? to record positions right away?
            ...
            # .cfg option
                ...
                if not dir in self.cfg:
                    ...
                    pass # log.debug(f"Duplicate directive '{dir}' found")
            # .wrap state
            elif dir.startswith("</"):
                if self.wrap:
                    self.wrap.pop(0)
    ...
    def errorlog(self, args):
        fn = self.var_sub(args[0])
        self.logs.append(fn)
        tmp.log_map[fn] = "error"
    def customlog(self, args):
        ...
        if ty.find("%") >= 0:  # turn literal placeholder format into temporary name
            ...
            self.log_format(ty, fmt)
        tmp.log_map[fn] = ty
    def forensiclog(self, args):
        fn = self.var_sub(args[0])
        self.logs.append(fn)
        tmp.log_map[fn] = "forensic"
    def servername(self, args):
        if not self.name:
            self.name = args[0]
    def logformat(self, args):
        tmp.log_formats[args[1]] = args[0]
...
class secrule:
    ...
    def assign(self, pfx, action, value):
        if action == "id":
            ...
            print("  WHATNOW? ", [pfx, action, value]) #log.warn
