...
def _ver_to_cal_info(vnfo):
    return version.V1CalendarInfo(vnfo.year, vnfo.quarter, vnfo.month, vnfo
        .dom, vnfo.doy, vnfo.iso_week, vnfo.us_week)
...
def cal_info(date=None):
    ...
    if date is None:
        date = version.TODAY
    kwargs = {'year': date.year, 'quarter': version.quarter_from_month(date
        .month), 'month': date.month, 'dom': date.day, 'doy': int(date.
        strftime('%j'), base=10), 'iso_week': int(date.strftime('%W'), base
        =10), 'us_week': int(date.strftime('%U'), base=10)}
...
def _parse_field_values(field_values):
    ...
    return version.V1VersionInfo(year=year, quarter=quarter, month=month,
        dom=dom, doy=doy, iso_week=iso_week, us_week=us_week, major=major,
        minor=minor, patch=patch, bid=bid, tag=tag)


def _is_calver(cinfo):
    ...
    """
    for field in version.V1CalendarInfo._fields:
        maybe_val = getattr(cinfo, field, None)
...
def _parse_pattern_groups(pattern_groups):
    for part_name in pattern_groups.keys():
        is_valid_part_name = (part_name in v1patterns.
            COMPOSITE_PART_PATTERNS or part_name in v1patterns.
            PATTERN_PART_FIELDS)
    ...
    field_value_items = [(field_name, pattern_groups[part_name]) for 
        part_name, field_name in v1patterns.PATTERN_PART_FIELDS.items() if 
        part_name in pattern_groups.keys()]
...
def parse_version_info(version_str, raw_pattern='{pycalver}'):
...
def is_valid(version_str, raw_pattern='{pycalver}'):
...
def incr(old_version, raw_pattern='{pycalver}', **kwargs):
    ...
    if tag_num:
        raise NotImplementedError(
            '--tag-num not supported for old style patterns')
    if tag:
