...
class SONG(BaseEstimator):
    ...
    def __init__(self, n_components=2, n_neighbors=1,
                 ...
                 online_portion=.0, fvc_growth=0.5, non_so_rate = 0, low_memory = False, scale_growth=False):
    ...
    def fit(self, X):
        ...
        thresh_g = -np.log(X.shape[1]) * np.log(self.sf) * (scale if self.scale_growth else 1.)
