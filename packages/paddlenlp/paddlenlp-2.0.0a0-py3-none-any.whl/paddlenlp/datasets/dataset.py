...
@classmethod
def get_datasets(cls, configs=None):
    """
    Get muitiple datasets like train, valid and test of current data.

    Args:
        configs (list|tuple, optional): The list|tuple of argument(s) to init each dataset. Default: None
    Returns:
        tuple: The result list of datasets.
    Example:
        .. code-block:: python
            from paddlenlp.datasets import GlueQNLI
            train_dataset, dev_dataset, test_dataset = GlueQNLI.get_datasets(['train', 'dev', 'test'])
            train_dataset, dev_dataset, test_dataset = GlueQNLI.get_datasets([['train'], ['dev'], ['test']])
            train_dataset, dev_dataset, test_dataset = GlueQNLI.get_datasets([{'segment':'train'}, {'segment':'dev'}, {'segment':'test'}])
    """
    if not configs:
        try:
            configs = cls.SEGMENTS.keys()
        except:
            raise AttributeError(
                'Dataset must have SEGMENTS attridute to use get_dataset if configs is None.'
            )
    if isinstance(configs[0], str):
        datasets = tuple(MapDatasetWrapper(cls(config)) for config in configs)
    elif isinstance(configs[0], list):
        datasets = tuple(MapDatasetWrapper(cls(*config)) for config in configs)
    elif isinstance(configs[0], dict):
        datasets = tuple(MapDatasetWrapper(cls(**config)) for config in configs)
    else:
        raise TypeError('Elements of configs must be string, list or dict.')
    return datasets
