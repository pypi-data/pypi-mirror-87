...
from azureml.interpret.common.constants import ExplainParams, ExplainType, History, BackCompat, IO, \
...
from azureml.interpret.common.exceptions import SamplesExceededException, ExplanationNotFoundException, \
...
class ExplanationClient(object):
    ...
    def upload_model_explanation(self,
        ...
        error_msg = 'The number of points supplied, {}, is greater than the supported maximum of {}. Please reduce'
        is_viz_turned_on = True
        index_getter = None
        has_local_explanation = LocalExplanation._does_quack(explanation)
        sparse_eval_data = False
        if isinstance(visualization_points, int):
            if visualization_points > VISUALIZATION_MAXIMUM:
                raise SamplesExceededException(error_msg.format(visualization_points, VISUALIZATION_MAXIMUM))
            ...
            if len(visualization_points) > VISUALIZATION_MAXIMUM:
                raise SamplesExceededException(error_msg.format(len(visualization_points), VISUALIZATION_MAXIMUM))
            elif len(visualization_points) == 0:
                is_viz_turned_on = False
            else:
                if has_local_explanation and not explanation.is_local_sparse:
                    num_viz_points = self._get_num_viz_points(len(visualization_points), explanation)
        ...
        if LocalExplanation._does_quack(explanation):
            ...
            if explanation.is_local_sparse:
                ...
                single_artifact_list.append((History.LOCAL_IMPORTANCE_VALUES,
                                             explanation.local_importance_values,
                                             None, IO.JSON))
        ...
        if classification and explanation.classes is not None:
            ...
            if classes is not None and num_classes is not None and len(classes) != num_classes:
                raise DimensionMismatchException('Class length mismatch')
            if LocalExplanation._does_quack(explanation) and num_classes != len(explanation._local_importance_values):
                raise DimensionMismatchException('Class length mismatch')
        ...
        viz_dict = {}
        # keep track of whether eval data for viz is uploaded, if not then we shouldn't upload ys either
        eval_data_viz_uploaded = False
        if index_getter is not None:
            viz_dict[History.INDICES] = num_viz_points
        if is_viz_turned_on and hasattr(explanation, History.EVAL_DATA):
            eval_data_attr = getattr(explanation, History.EVAL_DATA)
            eval_data = None
            if isinstance(eval_data_attr, pd.DataFrame):
                eval_data = eval_data_attr.values.tolist()
            elif isinstance(eval_data_attr, np.ndarray):
                eval_data = eval_data_attr.tolist()
            elif isinstance(eval_data_attr, list):
                eval_data = eval_data_attr
            elif sp.sparse.issparse(eval_data_attr):
            ...
            if eval_data is not None:
                ...
                assert isinstance(eval_data_for_viz, list)
                uploader._upload_artifact(upload_dir, 'eval_data_viz', eval_data_for_viz)
        ...
        if true_ys is not None and is_viz_turned_on and eval_data_viz_uploaded:
            ys_data = None
            ...
            if isinstance(true_ys[0], list):
                raise DimensionMismatchException('The true_ys input should be a one-dimensional array.')
            if ys_data is not None:
                if index_getter is None:
                    ...
                    true_ys_for_viz = list(index_getter(ys_data))
                mismatch_message = ('Mismatch in length of true ys for visualization and samples'
                                    'in evaluation data for visualization. {} does not match {}')
                if len(true_ys_for_viz) != len(eval_data_for_viz):
                    raise DimensionMismatchException(mismatch_message.format(len(true_ys_for_viz),
                                                                             len(eval_data_for_viz)))
                uploader._upload_artifact(upload_dir, 'true_ys_viz', true_ys_for_viz)
                temp_path = os.path.normpath('{}/{}/{}/{}'.format(RUN_ORIGIN, self._run.id, upload_dir,
                                                                  'true_ys_viz'))
                artifact_list.append({History.PREFIX: temp_path})
        ...
        if is_viz_turned_on and has_eval_y_predicted and eval_data_viz_uploaded:
            ...
            if index_getter is None:
                ...
                ys_pred_for_viz = list(index_getter(y_pred))
            uploader._upload_artifact(upload_dir, 'ys_pred_viz', ys_pred_for_viz)
            temp_path = os.path.normpath('{}/{}/{}/{}'.format(RUN_ORIGIN, self._run.id, upload_dir,
                                                              'ys_pred_viz'))
            artifact_list.append({History.PREFIX: temp_path})
        ...
        if is_viz_turned_on and has_eval_y_predicted_proba and eval_data_viz_uploaded:
            y_pred_proba = explanation.eval_y_predicted_proba
            if len(y_pred_proba[0]) != explanation.num_classes:
                error = 'Dimension mismatch between model.predict_proba() output and classes passed in.'
                raise DimensionMismatchException(error)
            ...
            if index_getter is None:
                ...
                ys_pred_proba_for_viz = list(index_getter(y_pred_proba))
            uploader._upload_artifact(upload_dir, 'ys_pred_proba_viz', ys_pred_proba_for_viz)
            temp_path = os.path.normpath('{}/{}/{}/{}'.format(RUN_ORIGIN, self._run.id, upload_dir,
                                                              'ys_pred_proba_viz'))
            artifact_list.append({History.PREFIX: temp_path})
            viz_dict[VizData.PROBABILITY_Y] = temp_path

        is_local_attr = hasattr(explanation, History.LOCAL_IMPORTANCE_VALUES)
        ...
        if is_local_attr:
            enable_local_viz = explanation.local_importance_values is not None and not explanation.is_local_sparse
        if is_viz_turned_on:
            if enable_local_viz:
                if index_getter is None:
                    temp_path = None
                    ...
                    uploader._upload_artifact(upload_dir, 'local_importance_viz', local_vals_for_viz)
                    temp_path = os.path.normpath('{}/{}/{}/{}'.format(RUN_ORIGIN, self._run.id, upload_dir,
                                                                      'local_importance_viz'))
                    artifact_list.append({History.PREFIX: temp_path})
                viz_dict[VizData.PRECOMPUTED_LOCAL_FEATURE_IMPORTANCE_SCORES] = temp_path
            else:
                temp_path = None
                for prefix_dict in artifact_list:
                    if History.GLOBAL_VALUES in prefix_dict[History.PREFIX]:
                        temp_path = prefix_dict[History.PREFIX]
                        break
        ...
        try:
            ...
            try:
                ...
                raise NoExperimentNameOrIdException('The experiment used has no ID or name.')
    ...
    def _get_local_importance_vals_for_viz(self, index_getter, explanation):
        ...
        """
        if ClassesMixin._does_quack(explanation):
            local_vals = explanation.local_importance_values
            local_vals_for_viz = [list(index_getter(class_vals)) for class_vals in local_vals]
        else:
            local_vals_for_viz = list(index_getter(explanation.local_importance_values))
    ...
    def _download_explanation_as_batch(self, explanation_id=None, comment=None, raw=None, engineered=None):
        ...
        if explanation_id is not None:
            ...
            if len(explanation_list) > 0:
                ...
                raise ExplanationNotFoundException(error_string)
            ...
            if len(explanation_list) == 0:
                ...
                if len(explanation_assets) == 0:
                    error_msg = 'Did not find any explanations matching comment or raw filters.'
                    raise ExplanationNotFoundException(error_msg)
            else:
    ...
    def _download_viz_data(self, raw=False):
        ...
        with open(os.path.join(download_dir, 'visualization_dict.interpret.json'), 'rb') as f:
            file_string = f.read()
            viz_data_dict = json.loads(file_string.decode('utf-8'))

        # Load the individual data in viz
        eval_data_viz = None
        if VizData.TEST_DATA in viz_data_dict:
            with open(os.path.join(download_dir, 'eval_data_viz.interpret.json'), 'rb') as f:
                file_string = f.read()
                eval_data_viz = json.loads(file_string.decode('utf-8'))
