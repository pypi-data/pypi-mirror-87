...
import robot
from SeleniumLibrary import LibraryComponent, JavaScriptKeywords, SelectElementKeywords
...
class FacileToolkit2:
    ...
    def checkpoint(self, locator, timeout=None, error=None):
        self.set_selenium_library()
        self._wait_until(
            lambda: self.sl.find_element(locator).is_displayed() and self.sl.execute_javascript(
                "return document.readyState=='complete'"),
            "Element '%s' or their parents did not contain class in <TIMEOUT>." % locator, timeout, error
        )
    ...
    def select_from_list_random_index_optional(self, *locators, start_index=None, end_index=None, timeout=None):
        ...
        for locator in locators:
            ...
            if lista_presente:
                self.select_from_list_random_index(locator=locator, start_index=start_index, end_index=end_index,
                                                   timeout=timeout)
    ...
    def i_land_on_page(self, url, timeout="60s"):
        ...
        try:
            ...
            self.dom_is_loaded(timeout=timeout)
        finally:
    ...
    def i_see(self, *locators):
        ...
        if not_visible_elements:
            ...
            raise AssertionError("Following elements are not visible: %s" % errors)
    ...
    def i_dont_see(self, *locators):
        ...
        if visible_elements:
            ...
            raise AssertionError("Following elements are visible: %s" % errors)
    ...
    def select_first_autocomplete_option(self,
        ...
        """
        self.set_selenium_library()
        self.sl.wait_until_element_is_visible(locator_option)
    ...
    @keyword
    def error_message_should_be(self, message, error_locator="//span[contains(@class, 'err') or "
                                                             "contains(@class, 'wrong') and "
                                                             "normalize-space(.)]", timeout=None,
        ...
        self.set_selenium_library()
        try:
            self.sl.wait_until_element_is_visible(error_locator, timeout)
    ...
    @keyword
    def wait_ajax(self, timeout="60s", wait_active="400ms"):
        self.set_selenium_library()
        BuiltIn().run_keyword_and_ignore_error("wait_ajax_activation", wait_active)
        self.sl.wait_for_condition('if(typeof(jQuery) == "function") {return jQuery.active == 0} return false', timeout)

    @keyword
    def wait_ajax_activation(self, timeout="400ms"):
        self.set_selenium_library()
    ...
    def fields_should_be_verified(self, *locators, timeout=None, verified_class="verified", error=None):
        for locator in locators:
            BuiltIn().run_keyword_and_continue_on_failure("field_or_parents_should_be_verified", locator, timeout, verified_class, error)

    @keyword
    def fields_should_contain_error(self, *locators, timeout=None, error_class="wrong", error=None):
        for locator in locators:
            BuiltIn().run_keyword_and_continue_on_failure("field_or_parents_should_contain_error", locator, timeout, error_class, error)
    ...
    @keyword
    def select_checkboxes(self, *locators):
        for locator in locators:
            if BuiltIn().run_keyword_and_return_status("checkbox_should_not_be_selected", locator):
                self.click_js(locator)
        self.should_be_checked(*locators)
    ...
    def should_be_checked(self, *locators):
        self.set_selenium_library()
        for locator in locators:
            is_checked = self.sl.driver.execute_script("return arguments[0].checked || arguments[0].classList.contains('checked');", self.sl.find_element(locator))
            BuiltIn().run_keyword_and_continue_on_failure("should_be_true", is_checked, "Locator %s is not checked" % locator)
    ...
    def should_not_be_checked(self, *locators):
        ...
        for locator in locators:
            is_checked = self.sl.driver.execute_script("return arguments[0].checked || arguments[0].classList.contains('checked');", self.sl.find_element(locator))
            BuiltIn().run_keyword_and_continue_on_failure("should_not_be_true", is_checked, "Locator %s is checked" % locator)
    ...
    def click_label_from_field_locator(self, locator):
        self.set_selenium_library()
        id = self.sl.get_element_attribute(locator, "id")
        self.click_js("css: label[for='%s']" % id)
    ...
    def wait_for_condition_extended(self, *condition, timeout=None, error=None):
        ...
        js_code, js_args = jk._get_javascript_to_execute(condition)
    ...
    def i_see_elements_label(self, *locators):
        self.set_selenium_library()
        for locator in locators:
            BuiltIn().run_keyword_and_continue_on_failure("element_should_be_visible", "css:label[for='%s']" % self.sl.get_element_attribute(locator, "id"))

    @keyword
    def close_symfony_toolbar(self):
        self.set_selenium_library()
        if BuiltIn().run_keyword_and_return_status("wait_until_element_is_visible", "body > .sf-toolbar"):
    ...
    def _wait_until_worker(self, condition, timeout, error):
        ...
        while time.time() < max_time:
            try:
                ...
                self.sl.info('Suppressing StaleElementReferenceException from Selenium.')
                not_found = err
            else:
                not_found = None
            time.sleep(0.2)
...
if __name__ == "__main__":
    robot.run('../atests/keyword_tests.robot')
