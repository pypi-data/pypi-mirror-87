...
def create_gdal_virtual_raster_using_file(file_paths_list, output_tif_path, warp=True, bands='all', vrt_extent_shift_match_path=None, extent_shift_match_path=None, remove_generator_files=True,
    ...
    print('extent_shift_match_path', extent_shift_match_path)
    ...
    if remove_generator_files and warp:
...
def get_rank_array_and_keys(input_array, nan_mask=None, highest_first=True, ndv=None):
    ...
    unraveled_keys = np.unravel_index(input_array.argsort(axis=None), shape=input_array.shape)  # returns TWO arrays in a tuple of all the rows, then all the cols keys
...
def reclassify_flex(input_flex, rules, output_path=None, output_data_type=None, array_threshold=100000, match_path=None, verbose=False):
    ...
    if isinstance(rules, dict):
        ...
        if max_key < array_threshold and max_value < array_threshold:
            ...
            if max_key <= 255 and max_value <= 255:
                ...
                rules = np.zeros(max_key + 1, dtype=hb.gdal_number_to_numpy_type[output_data_type])
                # rules = np.zeros(max_key + 1, dtype=np.uint8)
                for k, v in old_rules.items():
                    try:
                        rules[int(k)] = v
                    except:
                        L.info('failed to put into array for ', k, v, 'which maaaaay be okay but it will default to ndv in the output then.')
            else:
                rules = np.zeros(max_key + 1, dtype=hb.gdal_number_to_numpy_type[output_data_type])
                for k, v in old_rules.items():
                    try:
    ...
    if treat_input_as_arrayframe: # Then it has a path and needs to be done with raster calculation (but is memory safe)
        if isinstance(rules, dict):
            ...
            if input_flex.data_type == 1:
                ...
                raise NameError('reclassify unable to handle floats yet. Unimplemented. ' + str(input_flex))
...
def extract_global_netcdf_to_geotiff(input_nc_path, output_dir, vars_to_extract=None, time_indices_to_extract=None, ndv=None):
    ncfile = netCDF4.Dataset(input_nc_path, 'r')
    ...
    if time_indices_to_extract is not None:
        ...
        time_indices_to_extract = range(len(ncfile.variables['time']))
    else:
    ...
    res = (360.0) / len(ncfile.variables['lon'])
    ...
    for var_name, var in ncfile.variables.items():
        ...
        for time_index in time_indices_to_extract:
            # Not sure what this rules out but I'm going to keep it.
            if 'standard_name' in var.__dict__:
                ...
                if var_name not in dim_names:
                    ...
                    output_geotiff_uri = os.path.join(output_dir, var_name + '_' + str(time_index) + '.tif')
                    hb.save_array_as_geotiff(array, output_geotiff_uri, ndv=ndv, data_type=7, compress=True, n_cols=array.shape[1], n_rows=array.shape[0],
    ...
    for var_name, var in ncfile.variables.items():
        if 'standard_name' in var.__dict__:
            ...
            if var_name not in dim_names:
                ...
                hb.save_array_as_geotiff(array, output_geotiff_uri, ndv=no_data_value, data_type=7, compress=True, n_cols=array.shape[1], n_rows=array.shape[0],
                                         geotransform_override=geotransform, projection_override=projection)
...
def add_overviews_to_path(input_path, raise_exception_on_fail=False, specific_overviews_to_add=None, overview_resampling_algorithm='nearest'):
