...
class RPCClient(object):
    ...
    def remote_address(self):
        return f"{self.__ws.host}:{self.__ws.port}"
            ...
            if self.is_connected:
                try:
                    ...
                    loggers.get(LOGGER_NAME).debug(
                        f"C({self.address}) << S({self.remote_address}): \
{ws_data}")
                except Exception as e:
    ...
    async def send(self, method: str, params: dict = {}) -> Any:
        ...
        loggers.get(LOGGER_NAME).debug(
            f"C({self.address}) >> S({self.remote_address}): {payload}")
    ...
    async def notify(self, method: str, data: Any) -> Any:
        ...
        loggers.get(LOGGER_NAME).debug(
            f"C({self.address}) >> S({self.remote_address}): {rpc_packet}")
