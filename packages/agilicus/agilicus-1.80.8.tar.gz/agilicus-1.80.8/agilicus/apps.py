...
def query(ctx, org_id=None, maintained=None, updated_since=None, **kwargs):
...
def format_application_summaries_as_text(summaries):
    columns = [
...
def format_secure_agent_as_text(info):
    app_service_columns = [
        column("id"),
        column("hostname"),
        column("port"),
    ...
    columns = [
        metadata_column("id"),
        spec_column("name"),
        subtable("application_services", app_service_columns, subobject_name="status"),
        subtable("connector_info", connector_info_columns, subobject_name="status"),
    ]
