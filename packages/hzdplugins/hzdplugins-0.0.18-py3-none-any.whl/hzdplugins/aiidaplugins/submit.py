...
def qePwContinueSubmit(results, pk, codename='', add_parameters={}, del_parameters={}, kpoints=[], pseudo_family='', cluster_options={}, metadata={}):
    ...
    if len(cluster_options) > 0:
        ...
        restart_builder.metadata.options.resources = slurm_options[computer]['resources'] # in here machine = node
        restart_builder.metadata.options.max_wallclock_seconds = slurm_options[computer]['max_wallclock_seconds'] # in here machine = node
        restart_builder.metadata.options.account = slurm_options[computer]['account'] # in here machine = node
        restart_builder.metadata.options.scheduler_stderr = slurm_options[computer]['scheduler_stderr']
        restart_builder.metadata.options.scheduler_stdout = slurm_options[computer]['scheduler_stderr']
        restart_builder.metadata.options.queue_name = slurm_options[computer]['queue_name']
