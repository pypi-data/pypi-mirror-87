...
class DotDict(AlignedDict):
    """AlignedDict that *also* supports attribute (dot) access.
    ...
    # TODO: should include DotDict in memo somehow?
    #       I believe the problem with builtin deepcopy
    #       is that the recursion check prevents the
    #       self.__dict__ = self that is being done in __init__.
    #       However, the current implementation includes no
    #       recusion check, so could be fatal.
    # Also see: https://stackoverflow.com/a/15774013/38281
    # TODO: test
