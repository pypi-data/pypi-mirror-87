...
class GPGTool(Command):
	...
	def _gpgcheck(self):
		...
		#enc = self.enstdo('%s -r %s -e'%(cmd, gpgkey), inputs='test')
		#o, e, n = self.oerc('%s -d'%cmd, inputs=str(enc))
		#if o == 'test':
		#	self._gpgselfcheck = True
		#	return True
		#yesno = getin(quest)
		#if yesno and str(yesno).lower() in ('true', 'y', ''):
		#	return self._gpgcheck()
		#return False
		sig = self.sig
		self.sig = False
		enc = self.encrypt(' ~^ test ^~ ')
		self.sig = sig
		plain, ret = self.decrypt(enc)
		if str(plain) == ' ~^ test ^~ ':
			return True
		else:
			if getin(quest):
	...
	def encrypt(self, data, **kwargs):
		...
		fingers = []
		for rec in recvs:
			key = self.findkey(rec, typ='e')
			if key:
				finger = self._fingered(key, 'e')
				if finger:
					#print(finger)
