...
import requests
from ibm_watson_machine_learning.utils.autoai.utils import  preprocess_request_json, prepare_auto_ai_model_to_publish_normal_scenario, load_file_from_file_system,init_cos_client
    ...
    def _publish_from_training(self, model_uid, meta_props, training_data=None, training_target=None,version=False,artifactId=None,subtrainingId=None):
        ...
        if self._client.CLOUD_PLATFORM_SPACES or self._client.ICP_35:
           ...
           # print(model_content)
            ...
            if self._client.ICP_PLATFORM_SPACES:
                #asset_url = details['entity']['results_reference']['location']['assets_path'] + "/" + model_uid + "/resources/wml_model/request.json"
                request_str = load_file_from_file_system(wml_client=self._client, file_path=asset_url).read().decode()
            else:
    ...
    def _store_tf21_model(self, model, meta_props):
        ...
        if "tensorflow.python.keras.engine.training.Model" in str(type(model)):
            if save_formats is not None and save_formats == 'h5':
                raise WMLClientError("Invalid save format value provided in tf_model_params to save model, provide 'tf' as the value. ")
