...
def _parse_response(*, response: httpx.Response) -> Optional[Union[AutomationInputGenerator, BadRequestError]]:
    if response.status_code == 200:
        return AutomationInputGenerator.from_dict(cast(Dict[str, Any], response.json()))
    if response.status_code == 400:
        return BadRequestError.from_dict(cast(Dict[str, Any], response.json()))
    return None


def _build_response(*, response: httpx.Response) -> Response[Union[AutomationInputGenerator, BadRequestError]]:
    return Response(
