        ...
        def render_scene(script_and_scene):  # the part below controls TCL/VMD rendering
            ...
            if scn.run_vmd:
                ...
                with open('just_an_empty_file.vmd', 'w') as out:
                    out.write('')
                if os.name == 'posix':
                    call_moly('{} {} -e script_{}.tcl -startup just_an_empty_file.vmd 2>&1 | grep -v "Rendering '
                              'Progress" | tee {}_vmdlog.moly | grep "[05].dat"'.format(script.vmd, ddev, scn.alias,
                                                                                        scn.alias), log=self.log)
                else:
...
class Action:
    ...
    allowed_params = {'do_nothing': {'t'},
                      ...
                      'add_overlay': {'figure', 't', 'origin', 'relative_size', 'dataframes', 'dataframes_from_file',
                                      ...
                                      'movie', 'from', 'length', 'seaborn', 'mode', 'alias'},
                      ...
                      'fit_trajectory': {'selection', 't', 'axis', 'invert', 'abruptness', 'molecules'},
    ...
    def parse(self, command, ignore=()):
        ...
        try:
            new_dict = OrderedDict({prm.split('=')[0]: prm.split('=')[1].strip("'\"") for prm in spl[1:]
        ...
        if not isinstance(self, SimultaneousAction):
            ...
            if spl[0] == 'translate':
                self.transl = OrderedDict({'translate': self.parameters})
...
class SimultaneousAction(Action):
    ...
    def parse_many(self, directive, actions_dict, keyword):
        ...
        try:
            prm_dict = OrderedDict({prm.split('=')[0]: '='.join(prm.split('=')[1:]).strip("'\"") for prm in spl[1:]})
        except IndexError:
            raise RuntimeError("Line '{}' is not formatted properly; action name should be followed by keyword=value "
