...
from fibomat.backend import backendbasemeta
from fibomat.rasterizedpattern import RasterizedPattern

from fibomat import layout


class ShapeNotSupportedError(Exception):
...
class BackendBase(metaclass=backendbasemeta.BackendBaseMeta):
    ...
    (e.g. :meth:`fibomat.backend.backendbase.BackendBase.rect`) is called, e.g. ::
        ...
        def rect(self, ptn: pattern.Pattern):
            super().rect(pattern)
            # ... do your stuff here
    ...
    def process_pattern(self, ptn: pattern.Pattern) -> None:
        ...
        def dispatch(extracted_ptn):
            try:
                method = self.implemented_shape_methods[type(extracted_ptn.dim_shape.obj)]
                return method(self, extracted_ptn)
            except KeyError:
                # try bases classes
                for base in inspect.getmro(extracted_ptn.dim_shape.obj.__class__):
                    try:
        ...
        if isinstance(ptn.dim_shape.obj, layout.LayoutBase):
            # ptn.dim_shape[0]: layout.LayoutBase
            for extracted_shape in ptn.dim_shape.obj.layout_elements():
                ...
                # else:
                dispatch(
                    pattern.Pattern((extracted_shape, ptn.dim_shape.unit), ptn.mill, ptn.raster_style, **ptn.kwargs, description=ptn.description)
                )
        else:
            dispatch(ptn)
    ...
    def process_unknown(self, ptn: pattern.Pattern) -> None:
        ...
        Args:
            ptn (pattern.Pattern):  pattern to be added.

        Returns:
            None
        """
        raise ShapeNotSupportedError(f'Shape type = {ptn.dim_shape.obj.__class__}')
    ...
    def process_site(self, new_site: site.Site) -> None:
        """
    ...
    def parametric_curve(self, ptn: pattern.Pattern[shapes.ParametricCurve]) -> None:
        """
