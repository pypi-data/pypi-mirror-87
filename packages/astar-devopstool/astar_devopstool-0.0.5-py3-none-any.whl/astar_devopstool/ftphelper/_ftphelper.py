...
class FTPTool:
    ...
    def __init__(self, host, port=21, user='', passwd='', acct='',
                 timeout=socket._GLOBAL_DEFAULT_TIMEOUT, source_address=None,
                 encoding='gbk',
                 log_file='', log_flag=False):
        """
        ...
        if log_flag:
            if isinstance(log_file, (str, bytes)):
                self.log_file = open(log_file, "a")
            else:
                self.log_file = log_file
        self.file_list = []

    def login(self, username, password, acct=''):
        """ 初始化 FTP 客户端
            参数:
                  username: 用户名
        ...
        try:
            ...
            self.deal_error("FTP 连接失败 ，错误描述为：%s" % err)
            self.ftp.quit()
    ...
    def upload_file(self, local_file, remote_file, buf_size=1024):
        """从本地上传文件到ftp
