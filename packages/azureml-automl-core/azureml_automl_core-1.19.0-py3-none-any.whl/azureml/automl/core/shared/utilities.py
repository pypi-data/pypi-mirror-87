...
def interpret_exception(exception: BaseException, is_aml_compute: bool = True) -> AzureMLException:
    """
    ...
    (e.g. OSErrors, networking errors may need to be interpreted differently based on the run type)
    ...
    if isinstance(exception, MemoryError):
        result = ResourceException._with_error(
            AzureMLError.create(InsufficientMemory), inner_exception=exception
        ).with_traceback(safe_traceback)
    elif isinstance(exception, KeyboardInterrupt):
        result = ResourceException._with_error(
            AzureMLError.create(RunInterrupted), inner_exception=exception
        ).with_traceback(safe_traceback)
        ...
        if is_aml_compute:
            ...
            result = ClientException._with_error(
                AzureMLError.create(Service, error_details=str(exception), reference_code=ref_code),
                inner_exception=exception,
            ).with_traceback(safe_traceback)
        else:
            result = UserException._with_error(
                AzureMLError.create(HttpConnectionFailure, error_details=str(exception), reference_code=ref_code),
                inner_exception=exception,
            ).with_traceback(safe_traceback)
    else:
        # This was an unhandled exception, interpreting it as a system error
        result = ClientException._with_error(
            AzureMLError.create(AutoMLInternal, error_details=str(exception)), inner_exception=exception
        ).with_traceback(safe_traceback)
