...
class Operators(object):
    def __init__(self):
        ...
        for period in self._ts_period:
            self._ts_mutation_function_list += [Function(f, 1, FunctionType.time_series, period) for f in self._time_series_mutation_list]
            self._ts_crossover_function_list += [Function(f, 2, FunctionType.time_series, period) for f in self._time_series_crossover_list]
