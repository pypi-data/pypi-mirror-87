...
class Model(ctypes.Structure):
	...
	def sample(self, observed = None, num_samples = None, sampler = SamplerType.gibbs, progress_hook = 0, iterations = None, perturbation = 0.1, burn = 100, inference = InferenceType.belief_propagation):
		if self.destroyed:
			return numpy.array([])
	...
	with numpy.nditer(data, flags = ['multi_index']) as it:
		while not it.finished:
			...
			if data[row,col] == MISSING_VALUE:
				...
				R[row, col] = val * M[col].sdev + M[col].mean
			it.iternext()
