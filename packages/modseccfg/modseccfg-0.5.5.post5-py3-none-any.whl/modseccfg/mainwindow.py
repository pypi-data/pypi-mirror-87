...
from modseccfg import utils, icons, vhosts, logs, writer, editor, ruleinfo, recipe, install, scripts
...
vhosts.scan_all()
print(vhosts.tmp.log_formats)
print(vhosts.tmp.log_map)
...
class gui_event_handler:
   ...
   # add to *win_map{} event loop
    ...
    def test(self, data):
        ...
        w = sg.Window("HTML", layout=[[]], size=(800,700))
        w.read(timeout=1)
        root = w.TKroot.pack_slaves()[0]  # .!toplevel2.!frame 
        log.debug(dir(root))
#            p.destroy()
        frame = HtmlFrame(root, horizontal_scrollbar="auto")
        frame.configure(width=1000, height=800)
        frame.set_content("""<html><h3>hello</h3><br> <a href="http://google.com/new.html">new.html</a> <img src="./logo.png"></html>""")
        frame.pack()
        w.read()
