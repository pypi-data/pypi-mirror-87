...
from ._logging import ALREADY_IMPORTED_MODULES
...
from .convert_utils import convert_to_scalar
...
class BaseExperiment(CommonExperiment):
    ...
    def __init__(
        ...
        # Get parameters:
        self.log_code = self.config.get_bool(
    ...
    def end(self):
        ...
        """
        if _in_ipython_environment() and self.log_code:
            source_code = get_ipython_source_code()
            if source_code == "":
                # We might be running script directly:
                source_code = get_caller_source_code()
            if source_code:
    ...
    def log_model(
        ...
        # type: (str, Union[str, dict], Optional[str], Optional[bool], Optional[dict], Optional[bool]) -> Any
        """
        ...
        if filename:
            if code is not None:
                ...
                try:
                    ...
                    LOGGER.warning(
                        "unable to set code from filename %r; ignoring",
                        filename,
                        exc_info=True,
                    )
                    return
    ...
    def _set_code(self, code, overwrite=False, framework=None):
        if self.alive and code is not None:
            ...
            self._summary.increment_section(
                "uploads", "code", size=len(code), framework=framework
            )
