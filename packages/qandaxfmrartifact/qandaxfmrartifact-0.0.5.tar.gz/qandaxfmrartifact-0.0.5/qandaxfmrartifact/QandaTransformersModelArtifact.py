...
class QandaTransformersModelArtifact(BentoServiceArtifact):
    ...
    def _load_from_directory(self, path, opts):
        ...
        embedder = hub.load(path)

        self._model = {'model': transformers_model, 'tokenizer': tokenizer, 'embedder': embedder, 'opts': opts}

    def _load_from_dict(self, model, opts):
    ...
    def save(self, dst):
        ...
        self._save_package_opts(path, self._model.get('opts'))
        return path
