import logging
from typing import List, Text, Optional, Dict, Any, Union
...
from ragex.community.database.admin import Role, Permission, Project, User
from ragex.community.database.service import DbService
from ragex.community.services.user_service import ADMIN, ANNOTATOR, TESTER, UserService
...
PERMISSIONS = {
    ...
    "basic": {
        "view": {"user.get", "chatToken.get", "statistics.get", "telemetry.get"},
        "modify": {
            "user.update",
    ...
    "test_conversation": {
        "view": {
            ...
            "clientMessages.list",
            "metadata.get",
            # special case falling into view being a create:
            "messages.create",
            "actionPrediction.get",
        }
    },
    "conversations": {
        "view": {
    ...
    "responses": {
        "view": {"responseTemplates.list", "responses.list", "response.get", "response.getIntent"},
        "modify": {
    ...
    "git": {
        "view": {"repositories.list", "repositories.get", "repository_status.get"},
        "modify": {
            "repositories.create",
            "repositories.update",
            "repositories.delete",
...
DEFAULT_ROLES = {
    ...
    TESTER: ["basic.*", "test_conversation.*"],
}
...
class RoleService(DbService):
    ...
    def init_roles(
        self, overwrite: bool = False, project_id: Text = config.project_name
    ):
        ...
        for role in [*DEFAULT_ROLES]:
            ...
            is_default = role == config.saml_default_role
    ...
    def get_role_permissions(
        self, role: Text, project_id: Text = config.project_name
    ) -> List[Text]:
    ...
    def _permissions_for(
        self, user: Dict, project_id: Text = config.project_name
    ) -> List[Text]:
    ...
    def count_role_users(self, role: Text) -> int:
        """Retrieve number of all existing users for role."""

        return self.query(User).filter(User.roles.any(Role.role == role)).count()
