...
class TestModel(unittest.TestCase):
    ...
    def testOnebyoneSeq2seqTime(self):
        ...
        once_model = tfkit.model.onebyone.Model(tokenizer, pretrained, maxlen=maxlen)

        # test exceed 512
        start = timeit.default_timer()
        result, model_dict = seq2seq_model.predict(input="T " * 540)
        self.assertTrue(isinstance(result, list))
        print(result[0])
        print("exceed max len", len(result[0]))
        stop = timeit.default_timer()
        print('Seq2Seq Time: ', stop - start)

        # test exceed 512
        start = timeit.default_timer()
        result, model_dict = once_model.predict(input="T " * 540)
        self.assertTrue(isinstance(result, list))
        print(result[0])
        print("exceed max len", len(result[0]))
        stop = timeit.default_timer()
