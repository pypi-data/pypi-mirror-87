...
from modseccfg.utils import srvroot, conf, inject
...
#-- widget structure
menu = [
    ["File", ["Edit conf/vhost file (F4)", "---", "Settings (F12)", "SecEngine options", "CoreRuleSet options", "---", "Rescan configs", "Test", "---", "Exit"]],
    ["Rule", ["Info (F1)", "Disable", "Enable", "Modify", "<Wrap>", "Masquerade"]],
    ["Recipe", recipe.ls()],
    ["Log", ["Advise", "Log View (F3)", "Preprocess", ["---"], "Report", ["---"], "Reread log"]],
    ["Help", ["About", "FAQ", "Debug"]]
]
layout = [
    [
        sg.Column([
            ...
            [
                ...
                sg.T(" " * 18),
                sg.Button("Filter ↓", key="filter_log", button_color=("white","gray"), tooltip="Apply filter phrase to current log"),
                sg.Combo(values=["", "robots", "500|429", "/wp-"], size=(20,1), key="log_filter", enable_events=True, tooltip="Regex to filter with")
            ],
            [sg.T("  "*63+"↓")],
            # comboboxes
...
class gui_event_handler:
   ...
   # add to *win_map{} event loop
    ...
    # Help: Debug
    def debug(self, data):
        print(sg.show_debugger_window())  # we need the window handle for the event loop
        #._DebugWin.debug_window
        #self.win_register(sg._Debugger.debugger.watcher_window, lambda *x: None)
