...
contains
  ...
  subroutine Mll_blocks(lmax, well, nwell, mll_TT_TT, ier)
    integer, intent(in)      :: lmax
    integer, intent(in)      :: nwell
    real(real64), intent(in) :: well(nwell)
    real(real64), intent(out), dimension(0:lmax, 0:lmax) :: mll_TT_TT
    integer, intent(out) :: ier

    real(real64), parameter  :: one_fourpi = 1.d0 / (atan(1.d0) * 16.d0)
    real(real64), dimension(2*lmax+1)  :: wigner0
    real(real64), dimension(0:nwell-1) :: well_TT, thewell, ell
    real(real64) :: rl1, rl2, rl1min, rl1max, wig00, coef, sum_TT
    ...
    !$omp& shared(ier, lmax, nwell, well_tt) &
    !$omp& shared(mll_TT_TT)
    do l1=0, lmax
      rl1 = real(l1, real64)
      do l2=0, lmax
        rl2 = real(l2, real64)
