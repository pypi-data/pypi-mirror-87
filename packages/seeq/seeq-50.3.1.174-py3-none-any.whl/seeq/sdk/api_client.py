...
class ApiClient(object):
    ...
    def __init__(self, host=None, header_name=None, header_value=None, cookie=None):
        ...
        self.user_agent = 'Swagger-Codegen/50.3.1/python'
    ...
    def __call_api(self, resource_path, method,
        ...
        # We have to use a dict here as a "wrapper" object for the closure
        retry_timeout = dict()

        def _reraise_possibly(force=False):
            if 'start' not in retry_timeout:
                retry_timeout['start'] = time.time()
            if force or time.time() - retry_timeout['start'] > config.retry_timeout_in_seconds:
        ...
        while True:
            try:
                ...
                _reraise_possibly(e.status != 504)
    ...
    def sanitize_for_serialization(self, obj):
        ...
        if obj is None:
            return None
        elif isinstance(obj, (binary_type, text_type)):
            # SEEQ-NOTE:
            # We use (six.binary_type, six.text_type) as an abstraction for (str, unicode) in Python 2 and
            # and (bytes, str) in Python 3. (In Python 3, str is now unicode.)
            #
            # Python 3 urllib and json libraries expect to recieve text_type objects and encode them correctly
            # to utf-8 by default.  Python 2 libraries expect to recieve binary_type objects that have already
            # been encoded using the desired encoding. Therefore:
            #  - For Python 3 we return text_type (assuming utf-8) which will be encoded to binary in urllib and json.
            #  - For Python 2 we return binary_type encoded using utf-8 (assuming binary objects are already utf-8)
            if PY3:
                return obj if isinstance(obj, text_type) else text_type(obj, 'utf-8', errors='replace')
            else:
