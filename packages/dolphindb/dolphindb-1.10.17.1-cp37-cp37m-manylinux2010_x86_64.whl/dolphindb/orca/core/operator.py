...
from .utils import (ORCA_INDEX_NAME_FORMAT, _infer_axis, _infer_level,
                    ...
                    is_datetimelike, is_dolphindb_scalar,
                    is_dolphindb_uploadable, is_dolphindb_vector, sql_select,
                    to_dolphindb_literal, to_dolphindb_type_string)
...
def _binary_op_compute(session, left, right, func, axis, ExpressionType=None):
    left_obj = left.compute()
    right_obj = right.compute()
    return left_obj._binary_op_on_different_indices(right_obj, func, axis, ExpressionType)
...
def _get_expr_with_binary_op(ExpressionType, left, right, func):
    ...
    def is_temporal_type(obj):
        if obj._is_series_like:
            try:
                return is_datetimelike(obj._ddb_dtypes[obj._data_columns[0]])
            except:
                return False
        else:
            return False
    ...
    if (isinstance(left, (pd.DataFrame, pd.Series, pd.Index))
        ...
        if is_temporal_type(right) and isinstance(left, (str, pd.Timestamp)):
            try:
        ...
        if is_temporal_type(left) and isinstance(right, (str, pd.Timestamp)):
            try:
    ...
    elif isinstance(left, Series) and isinstance(right, np.datetime64):
        left_obj, right_obj = left, Series([right]*left.__len__(), session=right._session)
        axis = _default_axis(left)
        return _binary_op_compute(left._session, left_obj, right_obj, func, axis, ExpressionType)
    elif isinstance(right, Series) and isinstance(left, np.datetime64):
        right_obj, left_obj= left, Series([left] * right.__len__(), session=right._session)
        axis = _default_axis(right)
        return _binary_op_compute(right._session, left_obj, right_obj, func, axis, ExpressionType)
        ...
        axis = _default_axis(left)
        return _binary_op_compute(left._session, left, right, func, axis, ExpressionType)
    else:
        left_obj, right_obj = left, right
        axis = _default_axis(left)
...
def _check_rank_arguments(axis, method, na_option, ascending, pct, rank_from_zero, group_num):
    ...
    ignore_na = "true"
    if (na_option == "top" and ascending) or (na_option == "bottom" and not ascending):
        ignore_na = "false"
    elif na_option != "keep":
        raise ValueError("if ascending is True, na_option must be 'keep' or 'top'. if ascending is not True, "
                         "na_option must be 'keep' or 'bottom' ")
    if pct:
    ...
    if rank_from_zero:
        if group_num is not None:
            func = f"rank{{,{asc},{group_num},{ignore_na}}}"
        else:
            func = f"rank{{,{asc},,{ignore_na}}}"
    else:
        if group_num is not None:
            func = f"(x->rank(x,{asc},{group_num},{ignore_na})+1)"
        else:
            func = f"(x->rank(x,{asc},,{ignore_na})+1)"
    return func
...
class StatOpsMixin(metaclass=abc.ABCMeta):
    ...
    reciprocal = _orca_unary_op("reciprocal", numeric_only=True)
    expm1 = _orca_unary_op("expm1", numeric_only=True)
    exp2 = _orca_unary_op("exp2", numeric_only=True)
    log2 = _orca_unary_op("log2", numeric_only=True)
    log10 = _orca_unary_op("log10", numeric_only=True)
    log1p = _orca_unary_op("log1p", numeric_only=True)
    cbrt = _orca_unary_op("cbrt", numeric_only=True)
    square = _orca_unary_op("square", numeric_only=True)
    ...
    mad = _orca_unary_agg_op("mad", numeric_only=True)
    ...
    def astype(self, dtype, copy=True, errors="raise"):
        ...
        if self._is_series_like:
            ...
            typestr = to_dolphindb_type_string(dtype, to_lower=True)
            if copy:
                return self._unary_op(typestr, numeric_only=False).compute()
            elif self._ddb_dtypestr[self._data_columns[0]] == typestr:
                return self
            else:
                return self._unary_op(typestr, numeric_only=False)
        else:
            if isinstance(dtype, dict):
                ...
                typestr = {col: (to_dolphindb_type_string(dtype[col], to_lower=True)
                                 if col in dtype
                                 else "")
                           for col in self._data_columns}
                if copy:
                    return self._unary_op(typestr, numeric_only=False).compute()
                elif all(self._ddb_dtypestr[col] == typestr[col] for col in dtype):
                    return self
                else:
                    return self._unary_op(typestr, numeric_only=False)
            else:
                ...
                if copy:
                    return self._unary_op(typestr, numeric_only=False).compute()
                elif all(self._ddb_dtypestr[col] == typestr for col in self._data_columns):
                    return self
                else:
                    return self._unary_op(typestr, numeric_only=False)
    ...
    def rank(self, axis=0, method='min', numeric_only=None, na_option='keep', ascending=True, pct=False, rank_from_zero=False, group_num=None):
...
class IOOpsMixin(metaclass=abc.ABCMeta):
    ...
    if pd.__version__.startswith("1"):
        to_markdown = _orca_io_op("to_markdown")
...
class BaseExpression(_InternalAccessor):
    ...
    def __bool__(self):
        klass = "Series" if self._is_series_like else "DataFrame"
        raise ValueError(f"ValueError: The truth value of a {klass} is ambiguous. Use a.empty, a.bool(), a.item(), a.any() or a.all().")
...
class BooleanExpression(BaseExpression, ArithOpsMixin, LogicalOpsMixin, StatOpsMixin, IOOpsMixin):
    ...
    def __init__(self, left, right, func, axis, infered_ddb_dtypestr=None):
        ...
        if isinstance(other_obj, (_ConstantSP, Timestamp)):
            ...
            self._internal = core_obj._internal
            if isinstance(left, _ConstantSP) and axis == 0:
                self._data_select_list = [f"{left._var_name} {func} {script}"
                                          for script in right._get_data_select_list()]
            elif isinstance(left, Timestamp) and axis == 0:
                if self._infered_ddb_dtypestr is not None:
                    typestrs = [self._infered_ddb_dtypestr]
                    ...
                    typestrs = (right._ddb_dtypestr[col] for col in right._data_columns)
                self._data_select_list = [f"{typestr}({left._var_name}) {func} {script}"
                                          for typestr, script in zip(typestrs, right._get_data_select_list())]
            elif isinstance(left, _ConstantSP) and axis == 1:
                self._data_select_list = [f"{left._var_name}[{i}] {func} {script}"
                                          for i, script in enumerate(right._get_data_select_list())]
            elif isinstance(right, _ConstantSP) and axis == 0:
                self._data_select_list = [f"{script} {func} {right._var_name}"
                                          for script in left._get_data_select_list()]
            elif isinstance(right, Timestamp) and axis == 0:
                if self._infered_ddb_dtypestr is not None:
                    typestrs = [self._infered_ddb_dtypestr]
                    ...
                    typestrs = (left._ddb_dtypestr[col] for col in left._data_columns)
                self._data_select_list = [f"{script} {func} {typestr}({right._var_name})"
                                          for typestr, script in zip(typestrs, left._get_data_select_list())]
            elif isinstance(right, _ConstantSP) and axis == 1:
                self._data_select_list = [f"{script} {func} {right._var_name}[{i}]"
                                          for i, script in enumerate(left._get_data_select_list())]
        elif right is None:
            self._internal = left._internal
            self._is_index_like = left._is_index_like
            ...
            if isinstance(func, dict):
                left_data_columns = left._data_columns
                left_scripts = left._get_data_select_list()
                self._data_select_list = [f"{func[col]}({left_script})" for col, left_script in
                                          zip(left_data_columns, left_scripts)]
            elif func.startswith("isDuplicated"):
                self._is_index_like = False
                self._is_series_like = True
                self._is_dataframe_like = False
            ...
            if left._is_series_like and right._is_series_like:
                left_script = left._get_data_select_list()[0]
                right_script = right._get_data_select_list()[0]
                self._data_select_list = [f"{left_script} {func} {right_script}"]
            elif left._is_series_like and right._is_dataframe_like:
                left_script = left._get_data_select_list()[0]
                right_scripts = right._get_data_select_list()
                self._data_select_list = [f"{left_script} {func} {right_script}"
