...
from kadi.lib.web import contains_query_param
from kadi.lib.web import get_locale
from kadi.lib.web import IdentifierConverter
from kadi.lib.web import modify_query_params
...
def _init_plugins(app):
    ...
    with app.app_context():
        ...
        # Register plugin OAuth2 providers.
...
def _init_celery(app):
    ...
    class ContextTask(TaskBase):
        ...
        Wraps Celery's default Task class. Note that any pre- or postrun handlers might
        not necessarily run inside this application context as well.
        """
...
def _setup_jinja(app):
    # Provide access to some useful builtins in the templates.
    builtins = [
        abs,
        all,
        any,
    ...
    app.jinja_env.globals.update(
        {
            "_l": lazy_gettext,
            "contains_query_param": contains_query_param,
...
def _setup_shell_context(app):
    ...
    def _shell_context():
        # The listed objects will be automatically accessible when running "flask shell"
        # or "kadi shell" without having to import them.
        return {
            "db": db,
            "es": es,
