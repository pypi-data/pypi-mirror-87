...
def _convert_pipeline_to_api_format(pipeline: dict) -> dict:
    """ Takes the simplified yaml format for the development environment and
    converts to the same format provided by Sermos' admin API
    """
    formatted_resp = {
        'config': pipeline,
    }
...
def get_schedules(deployment_id: str):
    ...
    try:
        ScheduleConfigSchema().load(schedules)
...
def get_pipelines(deployment_id: str):
    ...
    # Transform into what we expect from Admin API server. The local
    # pipelines.yaml file format is for convenience.
    retval = []
    for pipeline in pipelines['pipelines']:
        retval.append(_convert_pipeline_to_api_format(pipeline))
...
def get_pipeline(deployment_id: str, pipeline_id: str):
    ...
    if pipeline is not None:
        return jsonify(_convert_pipeline_to_api_format(pipeline))
    return jsonify({}), 404


@api.route(PREFIX + '/auth', methods=['GET'])
