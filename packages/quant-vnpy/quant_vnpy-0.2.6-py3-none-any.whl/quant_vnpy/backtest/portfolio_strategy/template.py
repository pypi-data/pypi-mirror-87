    ...
    def _set_strategy_status(self, status: StrategyStatusEnum):
        ...
        if status == StrategyStatusEnum.RunPending and self._strategy_status not in (
            ...
            try:
                ...
                self.write_log(f"策略 {self.strategy_name}{self.vt_symbols} 状态 {status.name} 被远程启动")
            finally:
                if self._lock is not None:
                    self._lock.release()
            ...
            try:
                ...
                self._strategy_status = status
                self.write_log(f"策略 {self.strategy_name}{self.vt_symbols} 状态 {status.name} 被远程停止")
            finally:
                if self._lock is not None:
                    self._lock.release()
            ...
            self.write_log(f"策略 {self.strategy_name}{self.vt_symbols} 状态 {status.name}")
            self._strategy_status = status
