...
class DNN(nn.Module):
    ...
    def __init__(self, n_input, n_output, n_hidden_list, activation_list):
        ...
        if type(activation_list) is not list:
            activation_list = [activation_list]*(len(n_hidden_list)+1)
        assert len(activation_list)==len(n_hidden_list)+1, 'length of layers and activations must match. If you want no activation, use nn.Identity'
        ...
        # Output layer
        layers.extend([nn.Linear(n_hidden_list[-1], n_output), activation_list[-1]])
...
if __name__ == '__main__':
    ...
    model = DNN(n_input, n_output, n_hidden_list, activation_lisy)
    print(model)
