# We heavily borrowed utility functions and implementations of CRF and Deep Biaffine from stanza.
import os
import json
import copy
import torch
from .train import config as master_config
...
class Pipeline:
    def __init__(self, language, cache_dir=None):
        ...
        if cache_dir is None:
            print('`cache_dir` has not been set.\nUsing `resources/merino` as default...')
            master_config.cache_dir = 'resources/merino'
        else:
        ...
        self.embedding_layers = EmbeddingModel(self.config)
    ...
    def add_language(self, lang):
        if lang in self.lang_list:
            ...
            with open(os.path.join(self.config.cache_dir,
                                   '{}/{}.vocabs.json'.format(treebank2lang[treebank_name], treebank2lang[treebank_name]))) as f:
                vocabs = json.load(f)
