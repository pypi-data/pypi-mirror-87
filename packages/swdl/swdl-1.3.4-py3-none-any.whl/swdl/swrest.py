...
class DataService:
    ...
    def pull_labels(
        self, match, time_from=None, virtual_camera_id="-1", with_events=True
    ):
        """
        ...
        # Get events
        if with_events:
            self.pull_events(match)

        match_id = match.match_id
        label = match.labels

        # Get positions
        pos_unsorted = list(self.get_positions(match_id, time_from, virtual_camera_id))
        bot_name = "bari@soccerwatch.tv"
        label_resolution = 250
        if pos_unsorted is not None and len(pos_unsorted) > 0:
            positions = sorted(pos_unsorted, key=lambda k: k["timestamp"])
            highest_timestamp = int(positions[-1]["timestamp"])
            label_rows = int(highest_timestamp / label_resolution) + 1
            if label_rows > label.positions.shape[0]:
                label.positions.resize(
                    (label_rows, label.positions_dim), refcheck=False
                )
            ...
            for p in positions:
                ...
                y = float(p["y"])
                zoom = float(p["zoom"])
                bot_created = float(p["username"] == bot_name)
                temp = [timestamp, x, y, zoom, x, y, zoom, bot_created]
                label.positions[int(timestamp / label_resolution)] = temp
