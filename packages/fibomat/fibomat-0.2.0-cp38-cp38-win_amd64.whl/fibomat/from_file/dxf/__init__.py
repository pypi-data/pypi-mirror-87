...
# from fibomat.utils.math import norm


def _is_rect(vertices):
    if len(vertices) == 4:
        # check if the diagonals are equal
        if math.isclose(norm(vertices[0]-vertices[2]),
...
def _rect_from_vertices(vertices):
    ...
    width = norm(vertices[base_start_vertex_i]-vertices[base_end_vertex_i])
    height = norm(vertices[base_end_vertex_i]-vertices[(base_end_vertex_i+1) % 4])
...
def _polyline(ocs, vertices_raw, closed):
    ...
    for i, vertex in enumerate(vertices_raw):
        vertices[i] = ocs.to_wcs(vertex)[:2]
...
def shapes_from_dxf(file_path: pathlib.Path) -> List[Shape]:
    """
