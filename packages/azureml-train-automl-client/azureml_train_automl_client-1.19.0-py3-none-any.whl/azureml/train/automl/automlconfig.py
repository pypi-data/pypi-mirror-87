...
from azureml.train.automl import _azureautomlclient, _azureautomlsettings
...
def _start_execution(
    ...
    if is_managed:
        """
        Local Managed is disabled at this time.
        msg = "Tried submitting a local conda or local docker run, but local managed scenario is disabled. " \
              "Falling back to running a local run in the current environment. To submit a local run to " \
              "a conda or docker environment, please set enable_local_managed=True in AutoMLConfig."
        logger.warning(msg)
        logging.warning(msg)
        """
...
class AutoMLConfig(object):
    ...
    .. remarks::
        ...
        https://github.com/Azure/MachineLearningNotebooks/blob/master/how-to-use-azureml/automated-machine-learning
        /regression/auto-ml-regression.ipynb
    ...
    :param drop_column_names:
        ...
        and regression tasks, use the ``featurization`` parameter. This setting is being deprecated.
    ...
    def __init__(self,
        ...
        if enable_tf:
            ...
            if not _azureautomlclient.AzureAutoMLClient._is_tensorflow_module_present():
                enable_tf = False
        ...
        if enable_local_managed:
            logging.warning("Local managed runs are currently disabled even if the \"enable_local_managed\" flag is "
                            "set. Falling back to running in the current environment.")

        if task in constants.Tasks.ALL_IMAGE:
            self._validate_and_default_image_tasks(task)
        ...
        try:
            ...
            logging.warning("scenario is an internal parameter that should not be used for regular experiments.")
            self.user_settings[EnvironmentSettings.SCENARIO] = scenario
        except KeyError:
            if is_prod() or force_curated_environment:
                self.user_settings[EnvironmentSettings.SCENARIO] = constants.Scenarios.SDK_COMPATIBLE
            else:
                self.user_settings[EnvironmentSettings.SCENARIO] = constants.Scenarios._NON_PROD
                self.user_settings["environment_label"] = os.environ.get(EnvironmentSettings.ENVIRONMENT_LABEL)
    ...
    def _get_fit_params(
        self,
        func: 'Callable[..., Optional[Any]]' = _azureautomlclient.AzureAutoMLClient.fit
    ) -> Dict[str, Any]:
        """
    ...
    def _validate_config_settings(self, workspace: Optional[Workspace] = None) -> None:
        ...
        if compute_target != constants.ComputeTargets.LOCAL:
            ...
            if not data_script_provided and \
                ...
                raise ConfigException._with_error(
                    AzureMLError.create(
                        InvalidInputDatatype, target="training_data", input_type="Unknown",
                        supported_types=", ".join(SupportedInputDatatypes.REMOTE_RUN_SCENARIO)
                    )
                )
        ...
        if self.user_settings['task_type'] not in constants.Tasks.ALL_IMAGE:
    ...
    def _validate_and_default_image_tasks(self, task):
        ...
        if self.user_settings.get('label_column_name', None):
            logging.warning("Parameter 'label_column_name' is ignored for image task types.")
        else:
