...
def get_module_from_choice(connection, powerOnDevice, report_dict, return_val, is_qps=False):
    ...
    if str_pos != -1:
        ...
        if powerOnDevice:
            powerOnDrive(my_qps_device, outputMode=output_mode_value)
        ...
        # Making an auto-power up -- Could be changed to specify whether it is to be powered up or not.
        if powerOnDevice:
            powerOnDrive(my_quarch_device, is_qps=is_qps)
    ...
    report_dict["quarch_module"] = my_quarch_device.ConString
    report_dict["module_firmware"] = my_quarch_device.sendCommand("version")
    return return_val


def powerOnDrive(myQuarchDevice, outputMode=None, is_qps=True):
...
def chooseQuarchModuleQPS(moduleName, myQps=None):
    ...
    #     comms.newNotifyChoiceOption("qpsmodule", selection[1].replace("::", ":"), connectionWithoutConType,
    #                                 outputMode=outMode)
    ...
    def activate_remote_server(self):
        ...
        try:
            ...
            try:
                ...
                print("Hit an expection during tls conn")
                print(e)
                return
    ...
    def parse_request(self, xml_tree):
        ...
        if xml_tree.find('function'):
            ...
            if sub_elem.find('function_call').text == "init_test":
                ...
                # from quarchpy.disk_test.test_test import PowerVsPerformance
                # self.test = PowerVsPerformance()
                # from quarchpy.disk_test.test_test2 import Full_Range_HotPlugTest
                # self.test = Full_Range_HotPlugTest()
            ...
            try:
                ...
                if self.server_name is None:
                    try:
                        ...
                        self.server_name = "no-name-server"
