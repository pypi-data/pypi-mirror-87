...
class ResG(dock.Craft):
    def __init__(self, in_shape, latent_dim, base_chs, norm_fn=BN, attention=False,
        ...
        self.min_res = 16
        ichs = {128: [base_chs*c for c in (8, 4, 2)]}
        ochs = {128: [base_chs*c for c in (4, 2, 1)]}
        ...
        for i in range(len(self.ochs)):
            setattr(self, 'blk_%d'%i, SimpleRes((self.min_res * 2**i, self.min_res * 2**i, self.ichs[i]),
...
class ResD(dock.Craft):
    def __init__(self, in_shape, base_chs, norm_fn=BN, attention=False, spec_norm=False, w_init=XAVIER_NORM, scope='RESD'):
        super(ResD, self).__init__(scope)
        H, W, C = in_shape
        min_size = min(H, W)
        ichs = {128: [base_chs*c for c in (1, 2, 4)]}
        ochs = {128: [base_chs*c for c in (2, 4, 8)]}
        self.ichs = ichs[min_size]
        self.ochs = ochs[min_size]
        ...
        for i in range(len(self.ichs)):
            setattr(self, 'blk_%d'%i,
                    SimpleRes((H // 2**i, W / 2**i, self.ichs[i]), self.ichs[i]//2, self.ochs[i],
