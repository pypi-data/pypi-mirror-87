...
def gru_lstm_network(dl_inputs, model_name, train_dc, valid_dc=False, test_dc=False):
    ...
    if dl_inputs["gru_lstm"]["pooling_mode"] in ["hstates_cosine"]:
        input_loss = nn.MSELoss()
        #input_loss = nn.CrossEntropyLoss()
    else:
        input_loss = nn.NLLLoss(weight=torch.tensor([1, 1], 
                                                     dtype=torch.float32, 
    ...
    for epoch in tnrange(epochs):
        if train_dl:
            ...
            for x1, len1, x2, len2, y, train_indxs in t_train:
                ...
                if pooling_mode in ["hstates_cosine"]:
                    loss = loss_fn(pred[:, 1], y.to(torch.float32))
...
def test_model(model, test_dl, eval_mode='test', valid_desc=None,
    ...
    if pooling_mode in ["hstates_cosine"]:
        ...
        loss_fn = nn.NLLLoss(weight=torch.tensor([1, 1], 
                                                 dtype=torch.float32, 
                                                 device=device), 
                             reduction="mean")  # The negative log likelihood loss
        ...
        with torch.no_grad():
            ...
            if map_flag:
                ...
                if pooling_mode in ["hstates_cosine"]:
                    ...
                    y_score_test += pred.cpu().data.numpy()[:, 1].tolist()
                else:
            ...
            if output_preds:
                if pooling_mode in ["hstates_cosine"]:
                    y_score_output = pred.T.cpu().data.numpy()
                else:
                    y_score_output = torch.exp(pred).T.cpu().data.numpy()
...
class two_parallel_rnns(nn.Module):
    ...
    def forward(self, x1_seq, len1, x2_seq, len2, pooling_mode='hstates', device="cpu", output_state_vectors=False, evaluation=False):
        ...
        if pooling_mode in ['attention']:
            ...
            return torch.tensor(torch.stack([1- hstates_cosine_sim, hstates_cosine_sim])).T
            #return hstates_cosine_sim
