...
def list():
    ...
    dataset_list = client.dataset_list(context.project.workspace.slug)
    if len(dataset_list) == 0:
        typer.echo(f'No datasets found in \'{context.project.workspace.slug}\' workspace.')
    else:
        table = AsciiTable([
            ['NAME', 'SOURCE TYPE', 'WORKSPACE'],
            *[[d.slug, d.source.type, d.workspace.slug] for d in dataset_list],
        ])
