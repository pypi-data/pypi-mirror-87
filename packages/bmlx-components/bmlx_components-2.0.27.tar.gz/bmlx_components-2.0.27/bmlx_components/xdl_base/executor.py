...
class XdlExecutor(Executor):
    ...
    def init_ps_or_scheduler_env(
        ...
        if exec_properties["need_ps"]:
            args.extend([
                "--ps_num",
                str(exec_properties["runtime_configs"]
                    ["resources"]["ps"]["instances"].as_number()),
                "--ps_memory_m",
                "%.f" % exec_properties["runtime_configs"]["resources"]["ps"]["memory"].as_sunit(
                ).to_mega_i(),
                "--ps_cpu_cores",
                str(exec_properties["runtime_configs"]
                    ["resources"]["ps"]["cpu"].as_number()),
            ])
            ...
            def __call__(self, ts, metrics, sample_ts, step):
                ...
                for name, value in metrics:
                    if isinstance(value, np.ndarray):
                        for i, subvalue in enumerate(value.tolist()):
                            summary = tf.Summary(value=[tf.Summary.Value(tag="%s_%d" % (name, i), simple_value=subvalue)])
                            self.fw.add_summary(summary=summary, global_step=step)
                    elif isinstance(value, (int, float)):
                        summary = tf.Summary(value=[tf.Summary.value(tag=name, simple_value=value)])
                        self.fw.add_summary(summary=summary, global_step=step)
                    else:
                        raise RuntimeError("unknown metrics type: %s" % (type(value)))
                self.fw.flush()

            def close(self):
                base_dir = "s3://mlpipeline/metrics/" # TODO，这个后续一起归一化一次
                fp = os.path.join(base_dir, str(exec_properties["pipeline_execution_id"]), str(exec_properties["component_id"]))
