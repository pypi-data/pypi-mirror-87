...
class DShap(object):
    ...
    def _load_dataset(self, data_dir):
     ...
     #   print("PRINTING YFFFFFFFFFFFFFFF",data_dic['y_test'])
        self.y_test = data_dic['y_test']
     #   print("++CHE++++++CKNG=================", np.unique(self.y_test))
        self.X = data_dic['X'] 
        self.y = data_dic['y']
        ...
        while tmc_run or g_run:
            ...
            if tmc_run:
                if error(self.mem_tmc) < err:
                    ...
                    #print("TMC VALUES",self.vals_tmc)
            if self.directory is not None:
                self.save_results()
        ...
        for number in numbers:
           ...
           # print(samples_dir)
            ...
            if not len(dic['mem_{}'.format(key)]):
                ...
                continue
            #print("WE ARE CHECKING:",mem.shape)
            #print(mem)
            #print("AGAIN CHECKING:", dic['mem_{}'.format(key)].shape)
