...
class CANMessage:
    def __init__(self, sid: int, dlc: int,  timestamp: int, 
            data16:  List[int] = [],
            data64 = -1):
    ...
    def encode_bstring(self):
        ...
        data = self.get_data16()
        return bytes(
            str(self.sid)+","+str(self.dlc)+","+str(data[0])+","+str(data[1])+","+str(data[2])+","+str(data[3])+","+str(self.timestamp)+"\n",
            "utf-8",
        )
    ...
    def decode_json(j) -> CANMessage:
        ...
        msg = CANMessage(d.get("sid"),d.get("dlc"),d.get("timestamp"),data16=d.get("data"))
    ...
    def decode_kvaser_csv(csv, start_time):
        try:
            ...
            dlc = int(line[4])
            data8 = line[5:13]
            data64 = sum([(0 if d == '' else int(d)) << (i*8) for i, d in enumerate(data8)])
            data = []
            for i in range(4):
...
def test_decode_kvaser_csv():
    msg = CANMessage.decode_kvaser_csv("0.31947,1,1099,2,8,20,178,20,178,61,74,0,0,1", datetime.datetime(year=2019, month=9, day=3))
