...
class Bottleneck(dock.Craft):
    def __init__(self, in_shape, neck_chs, stride=1, width_multp=4, dsample=False, scope='BOTTLENECK'):
        super(Bottleneck, self).__init__(scope)
        H, W, C = in_shape
        self.body_chs = neck_chs * width_multp
...
class ResBlock(dock.Craft):
    def __init__(self, in_shape, nchs, nblock, stride=1, scope='RESBLOCK'):
        super(ResBlock, self).__init__(scope)
        H, W, C = in_shape
        self.layers = []
        self.layers.append(Bottleneck(in_shape, nchs, stride, dsample=True))
        self.C = self.layers[-1].body_chs
        for i in range(1, nblock):
            self.layers.append(Bottleneck((H, W, self.C), nchs))
    ...
    def __init__(self, in_shape, nblocks, scope='RESNET_V2'):
        ...
        for i, nblk in enumerate(nblocks):
            if i == 0:
                ...
                stride = 2
                ds_power += 2
            self.blocks.append(ResBlock((ceil(H/2**ds_power), ceil(W/2**ds_power), ichs), nchs, nblk, stride))
            ichs = self.blocks[-1].C
            nchs *= 2
