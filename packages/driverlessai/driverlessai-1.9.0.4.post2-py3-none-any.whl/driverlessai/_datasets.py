...
class Dataset(_utils.ServerObject):
    ...
    def __init__(self, client: "_core.Client", info: Any) -> None:
        super().__init__(client=client, key=info.key)
        self._columns = [c.name for c in info.columns]
        self._logical_types = {
            "categorical": "cat",
            "date": "date",
            "datetime": "datetime",
        ...
        self._shape = (info.row_count, len(info.columns))
    ...
    def data_source(self) -> str:
        ...
        return self._get_raw_info().data_source

    @property
    def file_path(self) -> str:
        """Path to dataset bin file on the server."""
        return self._get_raw_info().bin_file_path

    @property
    def file_size(self) -> int:
        """Size in bytes of dataset bin file on the server."""
        return self._get_raw_info().file_size
    ...
    def _update(self) -> None:
        self._set_raw_info(self._client._backend.get_dataset_job(self.key).entity)
    ...
    def download(
        ...
        return self._client._download(path, dst_dir, dst_file, overwrite)
...
class DatasetColumnSummaryCollection:
    ...
    def _update(self) -> None:
        ...
        self._column_summaries: Dict[str, Dict[str, Union[float, int, str]]] = {
            ...
            for c in self._dataset._get_raw_info().columns
...
class DatasetJob(_utils.ServerJob):
    ...
    def result(self, silent: bool = False) -> "Dataset":
        ...
        return self._client.datasets.get(self._get_raw_info().entity.key)
