...
class TotalRevenueTTMFactor(BasicFactor):
    ...
    def calc(self):
        self._factor = get_factor_from_stock_data('get_em_stock_fin_fac', columns=['income_statement_83'], processor_after_pivot=lambda x: x.apply(calc_ttm, axis=1, whole_df=x))
...
class EBITDATTMFactor(BasicFactor):
    ...
    def calc(self):
        self._factor = get_factor_from_stock_data('get_em_stock_fin_fac', columns=['ebitda'], processor_after_pivot=lambda x: x.apply(calc_ttm, axis=1, whole_df=x))
...
class CashFlowTTMFactor(BasicFactor):
    ...
    def calc(self):
        self._factor = get_factor_from_stock_data('get_em_stock_fin_fac', columns=['cashflow_statement_39'], processor_after_pivot=lambda x: x.apply(calc_ttm, axis=1, whole_df=x))
