...
class TestExperimentObservable:
    ...
    def test_follow_handles_exceptions(self, capsys):
        def mp_websocket_closed(*args, **kwargs):
            raise websockets.exceptions.ConnectionClosedError(code=1,
                                                              reason='test')

        with monkeypatched(self.grid.client, 'subscribe', mp_websocket_closed):
            obsevable = experiment_observable.Experiment(
                client=self.grid.client, identifier='test-run')
        ...
        # with monkeypatched(self.grid.client, 'subscribe', mp_websocket_connection_closed_ok):
        ...
        # with monkeypatched(self.grid.client, 'subscribe', mp_execute):
        #     obsevable = observables.Experiment(client=self.grid.client,
        #                                    identifier='test-run')
        #     with pytest.raises(click.ClickException):
        #         obsevable.follow()
