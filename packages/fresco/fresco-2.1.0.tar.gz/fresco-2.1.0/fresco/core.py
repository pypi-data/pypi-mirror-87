...
class FrescoApp(RouteCollection):
    ...
    def __str__(self):
        ...
        return "<%s %s>" % (
            clsname,
            ("\n" + " " * (len(clsname) + 2)).join(
                str(r) for r in self.__routes__
            ),
        )
            ...
            for traversal in routecollection.get_routes(path, method, request):
                try:
                    ...
                    for f in self.process_view_handlers:
                        try:
                            r = f(
                                request, view, traversal.args, traversal.kwargs
                            )
                            if r is not None:
                                response = r
                        except Exception:
                            return self.handle_exception(
                                request, allow_reraise=False
                            )
                    ...
                    if (
                        route.fallthrough_statuses
                        and error_response.status_code
                        in route.fallthrough_statuses
                    ):
                        continue
    ...
    def remove_middleware(self, middleware):
        ...
        self.reset_wsgi_app()
        self._middleware = [
            m for m in self._middleware if m[0] is not middleware
        ]
    ...
    def requestcontext(
        ...
        content_iterator = app(environ, fake_start_response)
        next(content_iterator, None)
        yield context
        list(content_iterator)
        getattr(content_iterator, "close", lambda: None)()
