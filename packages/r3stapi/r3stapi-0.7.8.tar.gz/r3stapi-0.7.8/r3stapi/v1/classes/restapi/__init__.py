...
class RestAPI(object):
	...
	def __init__(self,
		...
		self.__default_user_data__ = {
			...
			"membership":"free",
			# the requests variables.
			"requests":0,
			"timestamp":None,
			# the signed up timestamp.
			"signed_up":None,
	...
	def get_key_by_uid(self, uid):
		...
		if api_key != None:
			...
			return r3sponse.error_response(f"Failed to find the uid for the specified user [{uid}], uid_api_keys: {json.dumps(self.uid_api_keys, indent=4)}.")
	...
	def set_email_permission(self, uid, permission=True):
		...
		# edit data.
		data["email_permission"] = permission
		...
		# handlers.
		return r3sponse.success_response(f"Successfully set the email permission of user [{uid}] to [{permission}].")
	...
	def give_free_requests(self, uid, requests=100):
		...
		# edit data.
		data["requests"] = -1*requests
		data["timestamp"] = Formats.Date().date
	...
	def __save_new_user__(self, uid):
		...
		data["signed_up"] = Formats.Date().date
