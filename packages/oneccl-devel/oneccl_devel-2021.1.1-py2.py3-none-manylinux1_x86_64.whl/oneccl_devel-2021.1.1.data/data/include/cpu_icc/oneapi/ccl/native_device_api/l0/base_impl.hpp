...
template <TEMPLATE_DECL_ARG>
const typename cl_base<TEMPLATE_DEF_ARG>::handle_t& cl_base<TEMPLATE_DEF_ARG>::get()
    const noexcept {
    return handle;
}
...
template <TEMPLATE_DECL_ARG>
const typename cl_base<TEMPLATE_DEF_ARG>::handle_t* cl_base<TEMPLATE_DEF_ARG>::get_ptr()
    const noexcept {
...
indexed_storage<value_type> merge_indexed_values(const IndexedContainer& indexes,
    ...
    try {
        ...
        std::transform(values.begin(),
                       ...
                       [functor](value_type& handle) ->
                       typename indexed_storage<value_type>::value_type {
        ...
        std::transform(
            ...
            [&indexed_values](const typename IndexedContainer::value_type& index) ->
            typename indexed_storage<value_type>::value_type {
                auto values_it = indexed_values.find(std::get<index_id>(index));
                if (values_it == indexed_values.end()) {
                    throw std::runtime_error(std::string(__PRETTY_FUNCTION__) +
                                             "Cannot find index: " + ccl::to_string(index) +
                                             ", from collected devices");
                }
                return std::make_pair(std::get<index_id>(index), values_it->second);
            });
    }
        ...
        ss << "\nvalues to indexed: ";
        std::copy(values.begin(), values.end(), std::ostream_iterator<value_type>(ss, ", "));
        ss << "\nerror: " << ex.what();
...
indexed_storage<value_type> collect_indexed_data(const ccl::device_indices_type& indexes,
    ...
    {
        std::transform(collected_values.begin(),
                       collected_values.end(),
                       std::inserter(ret, ret.end()),
                       [&functor](value_type handle) ->
                       typename indexed_storage<value_type>::value_type {
                           auto index = functor(handle);
