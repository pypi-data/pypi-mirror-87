...
class RNNBase(Layer):
    ...
    def __init__(self, mode: str, hidden_size: int,
                 num_layers: int = 1, use_bias: bool = True, batch_first: bool = False,
        ...
        if self._built == False:
            ...
            for layer in range(self.num_layers):
                for direction in range(self.num_directions):
                    layer_input_size = input_shape if layer == 0 else self.hidden_size * self.num_directions
...
class LSTM(RNNBase):
    ...
    def __init__(self, hidden_size,num_layers:int =2,activation=None,use_bias=False,batch_first=False,dropout_rate=0,bidirectional=False,name=None,  **kwargs):
        super(LSTM, self).__init__('LSTM', hidden_size,
                 num_layers, use_bias, batch_first,
    ...
    def forward(self, input, hidden=None):  # noqa: F811
        orig_input = input
        ...
        if hidden is None:
            ...
            # Each batch of the hidden state should match the input sequence that
            # the user believes he/she is passing in.
            hidden = self.permute_hidden(hidden, sorted_indices)

        self.check_forward_args(input, hidden, batch_sizes)
        if batch_sizes is None:
            result = _VF.lstm(input, hidden, self._flat_weights, self.bias, self.num_layers,
                              self.dropout_rate, self.training, self.bidirectional, self.batch_first)
        else:
            result = _VF.lstm(input, batch_sizes, hidden, self._flat_weights, self.bias,
                              self.num_layers, self.dropout_rate, self.training, self.bidirectional)
        output = result[0]
        hidden = result[1:]
        # xxx: isinstance check needs to be in conditional for TorchScript to compile
