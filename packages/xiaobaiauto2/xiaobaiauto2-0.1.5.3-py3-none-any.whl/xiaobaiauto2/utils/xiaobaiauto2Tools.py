        ...
        if _method.upper() != 'GET':
            if 'application/json' in request.get('postData').get('mimeType'):
                ...
                if 'text' in request['postData'].keys():  # browser(chrome)
                    _data = loads(request['postData']['text'])
    ...
    for i, v in enumerate(data):
        if is_xiaobaiauto2 == 1:
            ...
            _code += f'''@pytest.mark.run(order={i + 1})\
                ...
                \r\t\tdata=data,\
                \r\t\theaders=headers,\
                \r\t\tassert_mode='', # 例如：assert_mode=assert_mode.json\
                \r\t\tassert_path='',\
                \r\t\tassert_value='',\
                \r\t\textract_at='', # 例如：extract_at=extract_at.body\
                \r\t\textract_mode='', # 例如：extract_mode=extract_mode.json\
                \r\t\textract_path='',\
                \r\t\textract_name=''\
                \r\t)\
