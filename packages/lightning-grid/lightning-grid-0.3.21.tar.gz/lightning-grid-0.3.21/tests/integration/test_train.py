...
def test_train_fails_datastore(monkeypatch):
    """grid train fails if datastore version and name are not set together"""
    monkeypatch.setattr(grid, 'gql', lambda x: x)
    monkeypatch.setattr(grid.Grid, '_init_client', monkey_patch_client)
    monkeypatch.setattr(grid.Grid, 'get_credentials',
                        monkey_patch_get_credentials)
    ...
    result = RUNNER.invoke(cli.train, [
        '--grid_credential', CREDENTIAL_ID, '--grid_datastore_name', 'name',
        '--ignore_warnings', 'tests/data/script.py'
    ])

    assert result.exit_code == 1
    assert result.exception
    assert 'both datastore_name and datastore_version' in result.output

    result = RUNNER.invoke(cli.train, [
        '--grid_credential', CREDENTIAL_ID, '--grid_datastore_version', 'name',
        '--ignore_warnings', 'tests/data/script.py'
    ])
    ...
    result = RUNNER.invoke(cli.train, [
        '--grid_credential', CREDENTIAL_ID, '--grid_datastore_name', 'name1',
        '--grid_datastore_version', 'name2', '--ignore_warnings',
        'tests/data/script.py'
    ])
...
def test_train(monkeypatch):
    ...
    env.DEBUG = True
    result = RUNNER.invoke(cli.train, [
        '--grid_credential', CREDENTIAL_ID, '--grid_strategy', 'grid_search',
        '--grid_trials', "1", '--ignore_warnings', 'tests/data/script.py'
    ])
