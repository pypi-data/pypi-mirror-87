...
class FeaturizerFactory:
    ...
    def get_featurizer(properties: Dict[str, Any], is_onnx_compatible: bool = False) -> "BaseFeaturizer":
        ...
        if is_onnx_compatible:
            return OnnxFeaturizer(index=properties[FeaturizationRunConstants.INDEX_KEY],
...
class OnnxFeaturizer(BaseFeaturizer):
    """
    Class representing a basic featurizer chosen during feature sweeping with onnx enabled.
    """
    def __init__(self,
                 index: int,
                 transformers: List[str],
                 ...
                 **kwargs: Any):
        """
        Initialize an onnx featurizer.

        :param index: The integer index of the featurizer in the DataTransformer's mapper.features list.
        :param is_cached: Whether this featurizer has already been fitted and cached.
        :param is_distributable: Whether this featurizer is distributable or not.
        :param is_separable: Whether this featurizer can be scheduled and processed separately
        from the rest of featurization.
        :param transformers: The transformers associated with this featurizer.
        """
        super(OnnxFeaturizer, self).__init__(index=index,
                                             transformers=transformers,
                                             is_cached=is_cached,
                                             is_distributable=is_distributable,
                                             is_separable=is_separable,
    ...
    def _fit(self,
        ...
        :param df: The input data we will be fitting on.
        :param y: The input label data to be used.
        :return: The fitted onnx featurizer to be cached.
        """
        data_frame_mapper = data_transformer.mapper  # type: DataFrameMapper
        original_feature_list = data_frame_mapper.features  # type: List[TransformerType]
        featurizer_of_interest = original_feature_list[self.index]  # type: TransformerType
        data_frame_mapper.features = [featurizer_of_interest]
        data_frame_mapper.fit(df, y)
        # ensure mapper is restored to previous state, since other onnx featurizers grouped in this run will use it.
        data_frame_mapper.features = original_feature_list
        return featurizer_of_interest
