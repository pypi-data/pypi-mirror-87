import requests,re,datetime,json, pytube
...
class Downloader:
    ...
    class Twitter:
        ...
        def GetUrl(self, Data):
            ...
            return urls
    class YT():
        def GetData(self,url, f):
            if "?list" in url:
                Vtype = "Playlist"
                return [url, Vtype, f]
            elif not "watch?v" in url:
                Vtype = "Playlist"
                return [url, Vtype, f]
            else:
                Vtype = "Video"
                return [url, Vtype, f]
        def to_mp3(self, filename):
            clip = VideoFileClip(filename, verbose=False)
            clip.audio.write_audiofile(filename[:-4] + ".mp3")
            del clip.reader
            del clip
        def download_video(self, url, video=False):
            yt = pytube.YouTube(url)
            yt.streams.filter().first().download()
            return yt.streams.get_by_itag(18).default_filename
        def download_videos(self, urls, filetype):
            filenames = []
            if filetype=="mp3":
                video_or_nah=False
            else:
                video_or_nah=True
            for url in urls:
                filenames.append(self.download_video(url, video_or_nah))
            if filetype == "mp3":
                for video in filenames:
                    self.to_mp3(video)
      
        def download_playlist(self ,urls,url, filetype):
        ...
        def Analyze(self, Data):
            if Data[1] == "Video":
                return [Data[0], True if Data[2]=="mp4" else False]
            elif Data[1] == "Playlist":
                playlist = pytube.Playlist(Data[0])
