...
class Scheduler:
    ...
    def add_job(self,
                ...
                next_run_time: Optional[datetime] = None) -> Job:
        ...
        if trigger is None:
            trigger = triggers.DateTrigger(now_dt)
    ...
    def terminate_job(self, instance_id: int):
        with self._executor_lock:
            self._executor.terminate_job(instance_id)
    ...
    def get_job_instance_ids(self, job_id: str) -> List[int]:
        with self._jobstore_lock:
            if not self._jobstore.contains_job(job_id):
                raise jobstores.JobDoesNotExistException
