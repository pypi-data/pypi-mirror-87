...
def post_worker_init(log_prefix):
...
class GunicornApplication(Application):
    def __init__(self, app, additional_settings="", log_prefix="", options=None):
...
class ServerApplication(GunicornApplication):
    ...
    @staticmethod
    def get_access_log_prefix(num_workers):
        prefix = "[%(asctime)s"
    ...
    def from_settings(cls, additional_settings: str = "", num_workers: Optional[int] = None):
        from spell.serving.app import app
...
def main(
    ...
    if server_settings.USE_BATCH_PREDICT:
        ...
        run_proxy(use_gunicorn, gunicorn_proxy_settings=gunicorn_proxy_settings)
    else:
