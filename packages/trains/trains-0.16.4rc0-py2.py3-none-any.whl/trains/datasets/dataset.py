...
from ..storage.util import sha256sum, is_windows
...
class Dataset(object):
    ...
    def __init__(self, _private, task=None, dataset_project=None, dataset_name=None):
        ...
        if not task:
            ...
            from trains import __version__
            task.data.script.requirements = {'pip': 'trains == {}\n'.format(__version__)}
    ...
    def sync_folder(self, local_path, dataset_path=None, verbose=False):
        # type: (Union[Path, _Path, str], Union[Path, _Path, str], bool) -> ()
        """
        ...
        # add remaining files
        self._add_files(path=local_path, dataset_path=dataset_path, recursive=True, verbose=verbose)
    ...
    def upload(self, show_progress=True, verbose=False, output_url=None, compression=None):
        ...
        self._task.get_logger().report_text('Uploading compressed dataset ({})'.format(zip_file_size))
    ...
    def finalize(self, raise_on_error=True):
        # type: (bool) -> bool
        """
        Finalize the dataset (if upload was not called, it will be called automatically)
        ...
        self._task.comment = 'Dependencies: {}\n'.format(list(self._dependency_graph.keys()))
        self._task.close()
    ...
    def delete(cls, dataset_id=None, dataset_project=None, dataset_name=None, force=False):
        ...
        # todo: delete the actual artifacts from storage
        client = APIClient()
    ...
    def _add_files(self,
        ...
        if path.is_file():
            file_entry = self._calc_file_hash(
                FileEntry(local_path=path.as_posix(),
    ...
    def _get_dependencies_by_order(self):
        # type: () -> List[str]
        """
        ...
        while roots:
            ...
            roots.extend(list(reversed([p for p in self._dependency_graph[r] if p in self._dependency_graph])))
    ...
    def _report_dataset_genealogy(self):
        ...
        visited = []
        nodes = self._get_dependencies_by_order() + [self._id]
        for idx, node in enumerate(nodes):
            visited.append(node)
            parents = [visited.index(p) for p in self._dependency_graph[node] or []]
        ...
        self._task.get_logger().report_plotly(
            title='Dataset Genealogy', series='Dataset Genealogy', iteration=0, figure=fig)

        table_values = [["Dataset id", "removed", "modified", "unmodified", "used size"]]
        for node in nodes:
            count = 0
            size = 0
            for f in self._dataset_file_entries.values():
            ...
            table_values += [[node, removed, modified, count, humanfriendly.format_size(size)]]

        # report detailed table
        self._task.get_logger().report_table(
            title='Dataset Details', series='Details', iteration=0, table_plot=table_values,
            extra_layout={"title": "File use by parent dataset id"})
