...
class Presentation:
        ...
        with open(file, mode='w') as output:
            ...
            for slideId in slideIds:
                htmlLinks = slides.getMarkdownLinks(
                    links[slideId], version) if links and slideId in links else None
                self._writeMarkdownSection(output,slideId,
    ...
    def _writeMarkdownSection(self, output, slideId, slideContents, links=None):
        ...
        for index, slideContent in enumerate(slideContents):
            ...
            if index == len(slideContents) - 1 and links and len(links) > 1:
                sep = '-'
                link_line = "  \n"+sep.join(links)
                content += link_line
