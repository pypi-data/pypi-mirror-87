...
class Client:
    ...
    def get_bot_reviews(self):
        headers = {
            "Authorization": self.apiKey
        }
        response = requests.get(f"{self.api}/public/bot/{self.id}/reviews", headers=headers)
        if response.status_code != 200: raise exceptions.CheckException(json.loads(response.text))
        else: return objects.botReviews(json.loads(response.text)["response"]).botReviews
