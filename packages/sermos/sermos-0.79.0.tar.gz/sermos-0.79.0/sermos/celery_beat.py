...
from sermos.schedule_config_schema import ScheduleEntrySchema
...
def instantiate_celery_schedule(schedule_entry: dict) -> c_schedule:
    ...
    if schedule_type == 'interval':
        ...
        period = schedule_entry['schedule']['period']
        every = schedule_entry['schedule']['every']
    ...
    if schedule_type == 'crontab':
        return c_crontab(
            minute=schedule_entry['schedule']['minute'],
            hour=schedule_entry['schedule']['hour'],
            day_of_week=schedule_entry['schedule']['day_of_week'],
            day_of_month=schedule_entry['schedule']['day_of_month'],
            month_of_year=schedule_entry['schedule']['month_of_year'])
    ...
    def __init__(self, schedule_entry: dict = None, **kwargs):
        ...
        if schedule_entry:
            ...
            for key in optional_keys:
                if schedule_entry.get(key, None) is not None:
                    options[key] = schedule_entry[key]

            if schedule_entry.get('last_run_at', None) is None:
                last_run_at = current_app.now()
                schedule_entry['last_run_at'] = last_run_at
            if isinstance(schedule_entry['last_run_at'], str):
                last_run_at = convert_to_datetime(
                    schedule_entry['last_run_at'])
...
class SermosScheduler(Scheduler):
    ...
    def set_under_schedule(self):
        ...
        for sched in latest_schedule['schedules']:
    ...
    def get_current_sermos_schedule(self):
        ...
        """
        ses = ScheduleEntrySchema()
        sched = {'schedules': []}
        for entry_name in self.schedule:
            entry = self.schedule[entry_name]
            entry_obj = {}
            for f, field in ses.fields.items():
                if f == 'id':
                    continue

                this_val = getattr(entry, f, None)
                if this_val is not None:
                    if f == 'name':
                        name_id = this_val.split('/')
                        entry_obj['id'] = name_id[-1]
                        this_val = '/'.join(name_id[:-1])

                    # Convert interval schedule
                    elif isinstance(this_val, c_schedule):
                        this_val = {
                            'period': 'seconds',
                            'every': this_val.seconds
                        }
                        entry_obj['scheduleType'] = 'interval'

                    # convert cron schedule
                    elif isinstance(this_val, c_crontab):
                        this_val = {
                            'minute': str(this_val._orig_minute),
                            'hour': str(this_val._orig_hour),
                            'dayOfWeek': str(this_val._orig_day_of_week),
                            'dayOfMonth': str(this_val._orig_day_of_month),
                            'monthOfYear': str(this_val._orig_month_of_year)
                        }
                        entry_obj['scheduleType'] = 'crontab'

                    # Convert dates
                    elif isinstance(this_val, datetime.datetime):
                        this_val = this_val.isoformat()
                    key = field.data_key or field.name
                    entry_obj[key] = this_val
            sched['schedules'].append(entry_obj)
        return sched
