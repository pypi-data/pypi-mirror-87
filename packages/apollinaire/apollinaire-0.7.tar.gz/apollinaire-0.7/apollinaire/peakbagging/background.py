...
def get_low_bounds (full_param, n_harvey, freq, psd, numax) :
  ...
  for ii in range (n_harvey) :
    if ii==0 :
      low_bounds[3*ii+1] = freq[0] #constraint on Harvey law cut frequency
    else :
...
def background_guess (freq, psd, numax, dnu, m_star=None, n_harvey=2, spectro=False) :
  '''
...
def perform_mle_background (freq, psd, n_harvey=2, r=1., m=1., teff=5770., guess=None, frozen_param=None, power_law=False,
                            ...
                            quickfit=False, num=5000, smoothing=50) :
  '''
  ...
  if guess is None :
    guess = background_guess (freq, psd, numax, dnu, m_star=m, n_harvey=n_harvey, spectro=spectro) 
  ...
  if quickfit :
    aux_freq, aux_psd = degrade_psd (aux_freq, aux_psd, num=num, smoothing=smoothing)
  print ('Beginning fit')
...
def explore_distribution_background (freq, psd,  
                                     ...
                                     coeff_discard=50, thin=1, quickfit=False, num=5000, smoothing=50, save_only_after_sampling=False) :
