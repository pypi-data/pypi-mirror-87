...
class Workspace(object):
    ...
    * `Azure Machine Learning
      SKUs <https://docs.microsoft.com/azure/machine-learning/overview-what-is-azure-ml#sku>`_
    ...
    :param sku: The parameter is present for backwards compatibility and is ignored.
        For more information see `Azure Machine Learning SKUs
        <https://docs.microsoft.com/azure/machine-learning/overview-what-is-azure-ml#sku>`_.
    :type sku: str
    """
    ...
    def create(name, auth=None, subscription_id=None, resource_group=None,
               location=None, create_resource_group=True, sku='basic', tags=None,
               friendly_name=None, storage_account=None,
               key_vault=None, app_insights=None, container_registry=None,
               adb_workspace=None,
               cmk_keyvault=None,
        ...
        :param sku: The parameter is present for backwards compatibility and is ignored.
            For more information see `Azure Machine
            Learning SKUs <https://docs.microsoft.com/azure/machine-learning/overview-what-is-azure-ml#sku>`_.
        :type sku: str
        :param tags: Tags to associate with the workspace.
        :type tags: dict
    ...
    def get(name, auth=None, subscription_id=None, resource_group=None):
        ...
        workspace_from_auth = auth._get_workspace(
            subscription_id, resource_group, name)
        if workspace_from_auth is not None:
            return workspace_from_auth

        result_dict = Workspace.list(
            subscription_id, auth=auth, resource_group=resource_group)
        result_dict = {k.lower(): v for k, v in result_dict.items()}
        name = name.lower()
        if name in result_dict:
            workspaces_list = result_dict[name]
            if len(workspaces_list) == 1:
                list_workspace_object = workspaces_list[0]
                return Workspace(list_workspace_object.subscription_id,
                                 list_workspace_object.resource_group,
                                 list_workspace_object.name, auth=auth,
                                 sku=list_workspace_object.sku,
                                 _location=list_workspace_object.location)
            else:
                workspace_dict_list = []
                ...
                if len(workspace_dict_list) > 1:
                    raise WorkspaceException(
                        "More than 1 workspaces found with name={}.\n"
                        "Please choose one among these{}".format(
                            name, workspace_dict_list), found_multiple=True)
                else:
                    raise WorkspaceException(
                        "No workspace found with name = {}".format(name))
        else:
            if not subscription_id:
                raise WorkspaceException(
                    "No workspaces found with name={} in all the subscriptions "
                    "that you have access to.".format(name))
            else:
                if resource_group:
                    raise WorkspaceException(
                        "No workspaces found with name={} in resource_group={} in subscription={}".format(
                            name, resource_group, subscription_id))
                else:
                    raise WorkspaceException(
                        "No workspaces found with name={} in subscription={}".format(
    ...
    def sku(self):
        ...
        :rtype: str
        """
        has_autorest_sku = (self._workspace_autorest_object is not None and
                            self._workspace_autorest_object.sku)
        if has_autorest_sku:
    ...
    def tags(self):
        ...
        :rtype: dict
        """
        has_autorest_tags = (self._workspace_autorest_object is not None and
                             self._workspace_autorest_object.tags)
    ...
    def discovery_url(self):
        ...
        :rtype: str
        """
        has_autorest_discovery_url = (self._workspace_autorest_object is not None and
                                      self._workspace_autorest_object.discovery_url)
    ...
    def get_details(self):
        ...
        .. remarks::
            ...
            * `sku`: The workspace SKU (also referred as edition). For more information see `Azure Machine Learning
              SKUs <https://docs.microsoft.com/azure/machine-learning/overview-what-is-azure-ml#sku>`_.
    ...
    def linked_services(self):
        ...
        """
        return {linked_service.name: linked_service for linked_service in LinkedService.list(self)}
    ...
    def _create_legacy(
        ...
        :rtype: azureml.core.workspace.Workspace
        """
        workspace_object_autorest = _commands.create_workspace(
