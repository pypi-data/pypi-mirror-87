...
"""
    ...
    def _Options(self, p):
        ...
        p.add_option('-p', '--sdk_chip',
                     dest='sdk_chip', action='store_true',
                     help='list all sdk_chip component')
    ...
    def Execute(self, opt, args):
        ...
        show = opt.board or opt.chip or opt.common or opt.solution or opt.sdk or opt.sdk_chip
        show_cnt = 0
        for c in components:
            if count == 0 or c.name in args:
                if show:
                    if (opt.board and c.type == 'board') or (opt.chip and c.type == 'chip') \
                        ...
                        or (opt.sdk and c.type == 'sdk') or (opt.sdk_chip and c.type == 'sdk_chip'):
