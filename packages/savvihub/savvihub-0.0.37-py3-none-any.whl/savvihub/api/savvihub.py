...
class SavviHubClient:
    ...
    def volume_file_list(
        ...
        r = self.get(f'/v1/api/volumes/{volume_id}/files/', params={
            'path': path,
            'recursive': recursive,
            'snapshot': snapshot,
    ...
    def volume_file_copy(self, volume_id, source_path, source_snapshot, dest_path, recursive=False, **kwargs):
        r = self.post(f'/v1/api/volumes/{volume_id}/files/copy/', {
    ...
    def volume_file_delete(self, volume_id, path, recursive=False):
        r = self.delete(f'/v1/api/volumes/{volume_id}/files/delete/', params={'path': path, 'recursive': recursive})
        if r.status_code != 200:
            return None
