...
class TestScheduler(unittest.TestCase):
    ...
    def test_execution(self):
        ...
        self.assertGreater(len(sched.time_points), sched.triggered_time_points, 'total greater than number triggered')
        sched.join()  # wait for scheduler to complete
        self.assertEqual(sched.remaining_time_points, 0, 'no more time points')
        self.assertEqual(sched.triggered_time_points, len(sched.time_points), 'triggered count equals time points')
        self.assertEqual(sched.completed_time_points, len(sched.time_points), 'complete count is accurate')
...
class TestTimePointConditional(unittest.TestCase):
    ...
    def test_conditional(self):
        ...
        def check_conditional_return(tp, expected):
            tp.wait_for_completion()
            self.assertEqual(tp.action_returns['check_and_run'], expected)
        ...
        sched.insert_time_point(0.2, number_of_points=6, relative_to='previous')
        sched.start_sequence()
        sched.join()
        for ind, timepoint in enumerate(sched.time_points):
            ...
            self.assertEqual(
                ...
                expected,
                msg='ensure expected timepoints executed'
            )
