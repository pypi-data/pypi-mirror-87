...
class DataFrameWrapper(object):
    ...
    def update_col_from_series(self, column_name, series, cast=False):
        ...
        print('updating column {!r} in table {!r}'.format(
            column_name, self.name))
...
def run(steps, iter_vars=None, data_out=None, out_interval=1,
    ...
    if data_out:
        add_injectable('year', iter_vars[0])
    ...
    for i, var in enumerate(iter_vars, start=1):
        add_injectable('year', var)

        if var is not None:
            print('Running Year Iteration {} with YEAR XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX  {!r} @ {} '.format(
                i, var,datetime.now()))
            logger.debug(
                'Running year iteration {} with YEAR {!r}'.format(
                    i, var))
