...
# 获取浏览器cookie
# 主页：https://bitbucket.org/richardpenman/browsercookie
# 最近更新时间：2018-06-08
...
# pip install keyring
# pip install lz4
# Windows 访问 FireFox 数据可能出错 更新sqlite
...
class BrowserCookieLoader(object):
    ...
    def load(self):
        ...
        # LWPCookieJar 替换为 CookieJar 支持支持保存到文件，和从文件载入
...
def create_cookie(host, path, secure, expires, name, value):
    ...
    """
    return cookielib.Cookie(0, name, value, None, False, host, host.startswith('.'), host.startswith('.'), path, True,
...
if __name__ == '__main__':
    ...
    # 初始化 session.cookies 为 LWPCookieJar 对象
    session.cookies = cookiejar.LWPCookieJar(filename='./cookies.txt')

    # session.get('https://www.baidu.com/s?wd=LWPCookieJar&ie=UTF-8')
    ...
    cj = firefox()
    # cj = chrome()
    # cj = session.cookies  # 这个应该可以相互使用

    for cookie in cj:
        # print(dir(cookie))
        # if cookie.domain:
        #     print(type(cookie), cookie.domain)
        cookie_dict = {
            'path': cookie.path,
            ...
            # 'path_spec': cookie.path_spec,  # 无
            # 'domain_dot': cookie.domain_dot,  # 无
        }
        print(cookie_dict)
        # # 添加 cookie 到 selenium
        # try:
