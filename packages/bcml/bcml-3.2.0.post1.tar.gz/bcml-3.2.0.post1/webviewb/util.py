    ...
    def generate_func():
        if window._js_api:
            functions = {
                ...
                if callable(getattr(window._js_api, name)) and not name.startswith("_")
            }
        else:
            functions = {}
        ...
        if len(window._functions) > 0:
            expose_functions = {
                name: get_args(f) for name, f in window._functions.items()
            }
        else:
            expose_functions = {}
...
def js_bridge_call(window, func_name, param, value_id):
    def _call():
        try:
            ...
            code = 'window.pywebview._returnValues["{0}"]["{1}"] = {{value: \'{2}\'}}'.format(
                func_name, value_id, result
            )
        except Exception as e:
            error = {
                "message": str(e),
            ...
            code = 'window.pywebview._returnValues["{0}"]["{1}"] = {{isError: true, value: \'{2}\'}}'.format(
                func_name, value_id, result
            )
    ...
    if func is not None:
        try:
            ...
            logger.exception(
                "Error occurred while evaluating function {0}".format(func_name)
            )
    else:
