...
from savvihub.api.errors import parse_error_code
...
class SavviHubClient:
    ...
    def volume_file_read(self, volume_id, path, snapshot, **kwargs):
        r = self.get(f'/api/v1/volumes/{volume_id}/files/read', params={'path': path, 'snapshot': snapshot}, **kwargs)
        if r.status_code == 500:
            r.raise_for_status()
        elif r.status_code == 404:
            return None
    ...
    def volume_file_uploaded(self, volume_id, path, **kwargs):
        return self.post(f'/api/v1/volumes/{volume_id}/files/uploaded', {
            'path': path,
        }, **kwargs)
    ...
    def experiment_token_read(self, **kwargs):
        r = self.get(f'/api/v1/experiments', **kwargs)
        if r.status_code != 200:
            return None
    ...
    def experiment_read(self, workspace, project, experiment_number, **kwargs) -> SavviHubExperiment:
        return SavviHubExperiment(
            self.get(f'/api/v1/workspaces/{workspace}/projects/{project}/experiments/{experiment_number}',
                     **kwargs).json())
    ...
    def experiment_log(self, workspace, project, experiment_number, **kwargs):
        return [SavviHubExperimentLog(x)
                for x in self.get(
                f'/api/v1/workspaces/{workspace}/projects/{project}/experiments/{experiment_number}/log',
