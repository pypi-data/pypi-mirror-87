...
class DltkAiClient:
    ...
    def folder_upload(self, image_folder, folder_name):
        """
        :param str image_path_1: The path of the image file
        :param str image_path_1: The path of the image file
        :return
            obj : A json obj containing matched status and value info
        """
        files = []
    ...
    def visual_search(self, image_path_1, image_folder):
        """
        :param str image_path_1: The path of the image file
        :param str image_path_1: The path of the image file
        :return
        ...
        body = {'image_1': (image_path_1, open(image_path_1, 'rb'), 'multipart/form-data')}
        data = {'image_folder':image_folder}
        url = self.base_url + '/visual_search_ai/'
        headers = {'ApiKey': self.api_key}
        response = requests.post(url=url, files=body, headers=headers, data=data).json()
        return response 
    ...
    def text_to_speech(self, text):
        ...
        body = {'body': text}
        body = json.dumps(body)
        url = self.base_url + '/text-to-speech/'
        headers = {'ApiKey': self.api_key, 'Content-Type': 'application/json'}
        response = requests.post(url=url, data=body, headers=headers)
        return response.content

    def text_to_speech_compare(self, text, sources, gender):
        """
        :param str text: the text on which tts is to be applied.
        :param str gender: text to speech supports 'MALE' , 'FEMALE' or 'NEUTRAL'
        :return:
            obj: A audio content obj which can be written into a audio file.
        """
        body = {'text': text,"sources":sources,"gender":gender}
        body = json.dumps(body)
        url = self.base_url + '/text-to-speech/compare'
        headers = {'ApiKey': self.api_key, 'Content-Type': 'application/json'}
        response = requests.post(url=url, data=body, headers=headers)
        return response.content
    ...
    def speech_to_text(self, audio_path, language):
        """
