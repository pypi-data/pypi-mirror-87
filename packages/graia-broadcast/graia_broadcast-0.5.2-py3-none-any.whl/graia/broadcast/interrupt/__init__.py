...
class InterruptControl:
    ...
    async def wait(self, waiter: Waiter, priority: Optional[Union[int, Priority]] = None, **kwargs):
        ...
        for event_type in waiter.listening_events:
            listener_callable = self.leader_listener_generator(local_event, waiter, event_type, value_container, active_stat)
    ...
    def leader_listener_generator(self,
        event_lock: asyncio.Event,
        waiter: Waiter, event_type: Type[BaseEvent],
        value_container: Value, active_stat: ActiveStats
    ):
