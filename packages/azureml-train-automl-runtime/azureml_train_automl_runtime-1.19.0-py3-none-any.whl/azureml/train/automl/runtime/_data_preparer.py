...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import MalformedJsonString, InsufficientMemory
...
class DataPreparer(ABC):
    ...
    def prepare(self,
                ...
                dataset_types: Optional[List[str]] = None) -> Dict[str, Any]:
        """
...
class DataPreparerUtils:
    ...
    def _helper_get_test_data_from_dict(dataflow_dict: Dict[str, Any],
        ...
        if 'test_data' in dataflow_dict and automl_settings_obj.label_column_name is not None:
            ...
            data_dict['X_test_raw_column_names'] = X_test.columns.values
            data_dict['X_test_dtypes'] = X_test.dtypes
            if X_test.shape[1] == 1:
                # if the DF is a single column ensure the resulting output
                # is a 1 dim array by converting to series first.
                data_dict['X_test'] = X_test[X_test.columns[0]].values
            else:
                data_dict['X_test'] = X_test.values
