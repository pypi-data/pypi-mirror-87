...
@click.command()
@click.option("--local", "-R", default=False, help="use local metadata mode")
...
@click.argument("resource_ids", nargs=-1)
def get(local, page_size, page_token, resource, resource_ids, format):
    list_func = None
    try:
...
@click.option(
    "--token",
    type=str,
    help="token, you could get token at http://bmlx.mlp.bigo.inner:8888/user-center",
)
...
def login(user, token):
    ...
    if not token:
        click.echo(
            "Hint: you could go http://bmlx.mlp.bigo.inner:8888/user-center get your token"
        )
        token = click.prompt("Entreing token", type=str, hide_input=True)

    import_func_from_module("bmlx.cli.handlers.login", "login")(user, token)
