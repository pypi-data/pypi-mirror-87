...
class ImageRecognition(object):
    ...
    def optimize(self, 
        ...
        for i in range(iteration):
            ...
            fp = np.sum(np.absolute(val_out - y_val) * y_val)
            fn = np.sum(np.absolute((val_out - 1) - (y_val -1)) * (y_val - 1))
            precision = float(tp /(tp + fp + 0.001))
            recal = float(tp /(tp + fn + 0.001))
            ...
            print ("            precision: {}, recall: {}, f1: {}".format(precision, recal, f1))
            print (val_out, y_val)
