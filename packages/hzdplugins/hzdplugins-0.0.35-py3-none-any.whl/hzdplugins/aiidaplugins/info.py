...
def getChargeAndMagneticMoments(uuid):
...
def checkDistance(cell, atom1, atom2, bond_length):
    ...
    if distance(atom1.position, atom2.position) <= bond_length:
        return True, distance(atom1.position, atom2.position), ''

    if distance(atom1.position, atom2.position + x_cell) <= bond_length:
        return True, distance(atom1.position, atom2.position + x_cell), '+x'

    if distance(atom1.position, atom2.position - x_cell) <= bond_length:
        return True, distance(atom1.position, atom2.position - x_cell), '-x'

    if distance(atom1.position, atom2.position + y_cell) <= bond_length:
        return True, distance(atom1.position, atom2.position + y_cell), '+y'

    if distance(atom1.position, atom2.position - y_cell) <= bond_length:
        return True, distance(atom1.position, atom2.position - y_cell), '-y'

    if distance(atom1.position, atom2.position + z_cell) <= bond_length:
        return True, distance(atom1.position, atom2.position + z_cell), '+z'
