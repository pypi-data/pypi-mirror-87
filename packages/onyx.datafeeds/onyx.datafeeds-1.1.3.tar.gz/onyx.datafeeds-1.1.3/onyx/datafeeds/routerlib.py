...
class DataRouter(tornado.web.Application):
    ...
    # -------------------------------------------------------------------------
    async def with_timeout(self, future, timeout):
        return await tornado.gen.with_timeout(
            datetime.timedelta(seconds=timeout), future)

    # -------------------------------------------------------------------------
    async def fetch(self, req, addr, port):
        client = tornado.tcpclient.TCPClient()
        stream = await self.with_timeout(client.connect(addr, port), 1)

        try:
            req = encode_message(req)
            await self.with_timeout(stream.write(req), TCP_TIMEOUT)
            response = await self.with_timeout(
                stream.read_until(b"\n"), TCP_TIMEOUT)
        finally:
            try:
                stream.close()
        ...
        if response is None:
            ...
            while True:
                ...
                try:
                    ...
                    self.drop_dataserver((addr, port, hostname), msg=str(err))
                    continue
                else:
                    break
