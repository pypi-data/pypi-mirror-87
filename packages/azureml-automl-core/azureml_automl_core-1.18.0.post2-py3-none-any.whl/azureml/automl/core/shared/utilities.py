...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import (
    AutoMLInternal, InsufficientMemory, HttpConnectionFailure, RunInterrupted, Service)
        ...
        if dataset is not None and hasattr(dataset, 'timeseries_param_dict') \
           and dataset.timeseries_param_dict is not None:
            ...
            if hasattr(automl_settings, constants.TimeSeries.SHORT_SERIES_HANDLING_CONFIG):
                dict_time_series[constants.TimeSeries.SHORT_SERIES_HANDLING_CONFIG] = \
                    getattr(automl_settings, constants.TimeSeries.SHORT_SERIES_HANDLING_CONFIG)
...
def interpret_exception(exception: BaseException, is_aml_compute: bool = True) -> AzureMLException:
    ...
    if isinstance(exception, MemoryError):
        ...
        result = ResourceException._with_error(AzureMLError.create(RunInterrupted),
                                               inner_exception=exception).with_traceback(safe_traceback)
    elif isinstance(exception, ClientRequestError) and not is_aml_compute:
