...
def _check_efield_type(efield):
    ...
    if type(efield) == fields.efield:
        raise TypeError("deprecated `fields` module is used. The former fields-class " +
                        "and field-generators are not compatible with `core_py`. " +
                        "Please use `pyGDM2.fields_py` instead. " + 
                        "(switch by using `import pyGDM2.fields_py as fields`)")
