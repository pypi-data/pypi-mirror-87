...
class FundDataManager(DataManager):
    ...
    _BLOCK_DF_LIST = ('fund_manager_score', 'fund_indicator')
    ...
    def init(self, index_list=None, score_pre_calc=True, print_time=False, use_weekly_monthly_indicators=False, block_df_list=_BLOCK_DF_LIST):
        if self._s3_retriever.is_activated:
            ...
            std_dm = DataManager(datetime.datetime(2005, 1, 1), datetime.datetime.now())
        ...
        with BasicDatabaseConnector().managed_session() as quant_session:
            ...
            in_market_fund_list = self.dts.fund_status_latest[~self.dts.fund_status_latest['trade_status'].isnull()].fund_id.tolist()
    ...
    def fund_analysis_by_stock_factors(self, fund_id: str, factor_names: Tuple[str], universe: str = 'default') -> Optional[Tuple[pd.Series, pd.DataFrame]]:
        '''对基金计算因子暴露, 同时返回因子收益率序列'''
        try:
            try:
            ...
            return
        fund_nav = fund_nav.reindex(fund_nav.index.intersection(self.dts.trading_days.datetime))
        return StockFactorApi.decomp_ret(fund_nav.pct_change(), factor_names, universe)

    def index_analysis_by_stock_factors(self, index_id: str, factor_names: Tuple[str]) -> Optional[pd.DataFrame]:
        '''对指数计算因子暴露'''
        try:
