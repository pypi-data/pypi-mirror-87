...
class Zillow(Api):
    ...
    def get_listings(
        ...
        while len(listings) < min_results:
            new_listings = self.__get_listings(region.slug, searchQueryState, page_number, ignored_ids, needs_video, debug)
    ...
    def __get_listings(
        ...
        try:
            ...
            return [sl for sl in [SearchResultListing(e)for e in j] if sl.has_video == needs_video and sl.id not in ignored_ids]
        except Exception as e:
            if debug:
                print('err', e)
    ...
    def __download_listing_video(
        ...
        res = self.download_async(urls_paths, user_agent=user_agent, proxy=proxy, max_request_try_count=max_request_try_count, debug=debug)
        ...
        if not ffmpeg.concat_videos_copy(paths, p_ts, debug=debug):
            [os.remove(p) for p in paths if os.path.exists(p)]
