...
#ifdef MKL_STDCALL
#define SMALL_SIZE_DGEMM(transa, sa, transb, sb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc) mkl_dc_dgemm(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
#define SMALL_SIZE_SGEMM(transa, sa, transb, sb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc) mkl_dc_sgemm(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
#define SMALL_SIZE_ZGEMM(transa, sa, transb, sb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc) mkl_dc_zgemm(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
#define SMALL_SIZE_CGEMM(transa, sa, transb, sb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc) mkl_dc_cgemm(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
#else
...
#ifdef MKL_DIRECT_CALL_JIT
#define FNAME_UNROLLEDC_CALL(name,transa,transb,m,n,k,alpha,a,lda,b,ldb,beta,c,ldc) \
    name(transa,1,transb,1,m,n,k,alpha,a,lda,b,ldb,beta,c,ldc);
#else
#define FNAME_UNROLLEDC_CALL(name,transa,transb,m,n,k,alpha,a,lda,b,ldb,beta,c,ldc) \
    name(transa,transb,m,n,k,alpha,a,lda,b,ldb,beta,c,ldc);
#endif
...
#define MKL_DC_CBLAS_DGEMM_CONVERT(layout, transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc, fname_unrolledc, fname_direct)  do { \
    ...
    if ((layout) == CblasRowMajor) {                \
        if (MKL_DC_DGEMM_CBLAS_CHECKSIZE(m,n,k)) { \
            FNAME_UNROLLEDC_CALL(fname_unrolledc,ftrans[index_transb], ftrans[index_transa], \
        ...
        if (MKL_DC_DGEMM_CBLAS_CHECKSIZE(m,n,k)) { \
            FNAME_UNROLLEDC_CALL(fname_unrolledc,ftrans[index_transa], ftrans[index_transb], \
...
#define MKL_DC_DGEMM_CONVERT(transa, sa, transb, sb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)  do { \
    if (MKL_DC_DGEMM_CHECKSIZE(m,n,k)) { \
        SMALL_SIZE_DGEMM((transa), sa, (transb), sb, (m), (n), (k), (alpha), (a), (lda), (b), (ldb), (beta), (c), (ldc)); \
    } else {  \
        MKL_DIRECT_CALL_INIT_FLAG; \
...
#define MKL_DC_SGEMM_CONVERT(transa, sa, transb, sb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)  do { \
    if (MKL_DC_SGEMM_CHECKSIZE(m,n,k)) { \
        SMALL_SIZE_SGEMM((transa), sa, (transb), sb, (m), (n), (k), (alpha), (a), (lda), (b), (ldb), (beta), (c), (ldc)); \
    } else {  \
        MKL_DIRECT_CALL_INIT_FLAG; \
...
#define MKL_DC_CGEMM_CONVERT(transa, sa, transb, sb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)  do { \
    if (MKL_DC_CGEMM_CHECKSIZE(m,n,k)) { \
        SMALL_SIZE_CGEMM((transa), sa, (transb), sb, (m), (n), (k), (alpha), (a), (lda), (b), (ldb), (beta), (c), (ldc));\
    } else {  \
        MKL_DIRECT_CALL_INIT_FLAG; \
...
#define MKL_DC_ZGEMM_CONVERT(transa, sa, transb, sb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)  do { \
    if (MKL_DC_ZGEMM_CHECKSIZE(m,n,k)) { \
        SMALL_SIZE_ZGEMM((transa), sa, (transb), sb, (m), (n), (k), (alpha), (a), (lda), (b), (ldb), (beta), (c), (ldc));\
    } else {  \
        MKL_DIRECT_CALL_INIT_FLAG; \
