...
def field_for_schema(
    ...
    if origin:
        ...
        field_type = _field_by_type(origin, base_schema)
        if origin in (list, List):
            if field_type is None:
                field_type = marshmallow.fields.List
            child_type = field_for_schema(arguments[0], base_schema=base_schema)
            return field_type(child_type, **metadata)
        if origin in (tuple, Tuple):
            if field_type is None:
                field_type = marshmallow.fields.Tuple
            children = tuple(
                field_for_schema(arg, base_schema=base_schema) for arg in arguments
            )
            return field_type(children, **metadata)
        elif origin in (dict, Dict):
            if field_type is None:
                field_type = marshmallow.fields.Dict
            return field_type(
...
def _base_schema(
    ...
    def _make_clazz(data):
        # print(f'{type(data)} instanceof {clazz}')
    ...
    class BaseSchema(base_schema or marshmallow.Schema):  # type: ignore
        def load(self, data: Mapping, *, many: bool = None, **kwargs):
            ...
            if many:
                return [_make_clazz(loaded) for loaded in all_loaded]
            else:
                return _make_clazz(all_loaded)
