...
import math
import time
from sqlalchemy import func, false, distinct, Integer, cast, and_, Float
from sqlalchemy.util import KeyedTuple

from rasa.core.events import Restarted, UserUttered, BotUttered
from ragex.community import config, utils
from ragex.community.database.analytics import ConversationSession, AnalyticsCache
...
SESSION_TIMEOUT = 48 * 3600  # 48 hours session timeout in seconds
...
class AnalyticsService(DbService):
    ...
    def run_analytics_caching() -> None:
        import ragex.community.database.utils as database_utils
        ...
        with database_utils.session_scope() as session:
            ...
            for k, v in CACHED_ANALYTICS_CONFIG.items():
                window = utils.duration_to_seconds(v["window"])
                start = now - utils.duration_to_seconds(v["range"])
    ...
    def calculate_analytics(
        ...
        result = (
            self.query(
                ...
                (
                    func.count()
                    / cast(  # cast to float since it's otherwise a integer division
    ...
    def _persist_analytics(
        ...
        result = json.dumps(result, cls=utils.DecimalEncoder)
    ...
    def update_session_data(self, event: Dict[Text, Any]) -> None:
        ...
        if (
            not latest_session
            or event_name == Restarted.type_name
            or event_timestamp - latest_session.latest_event_time > SESSION_TIMEOUT
        ):
        ...
        # update session
        self._update_session(latest_session, event_name, event_timestamp)
    ...
    def _update_session(
        ...
        if event_name == BotUttered.type_name:
            conversation_session.bot_messages += 1
        elif event_name == UserUttered.type_name:
