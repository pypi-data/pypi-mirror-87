...
class MLModelClient(MLSClient):
    ...
    def get_automl_model(self, id: int = None, name: str = None, version: str = None) -> AutoMLModel:
        ...
        return AutoMLModel(**self._request(method="GET", url=f"{MLS_MODELS_API_URL}/{automl_models[0].id}").results)
    ...
    def is_batch_prediction_ready(self, manual_model: ManualModel) -> bool:
        """
        일반 모델에 등록된 배치추론 테이블이 존재하고 업로드가 끝났는지 체크합니다.
        ...
        ml_model_client.manual_model_table_exists(manual_model=my_model)
        ```
        """
        assert type(manual_model) == ManualModel
        ...
        dynamodb_tables = self._request(method="GET", url=MLS_DYNAMODB_TABLES_API_URL).results.get("tables")
        if table not in dynamodb_tables:
            return False

        if self._request(method="GET", url=f"{MLS_DYNAMODB_TABLES_API_URL}/{table}/is_loading").results["is_loading"]:
            return False
