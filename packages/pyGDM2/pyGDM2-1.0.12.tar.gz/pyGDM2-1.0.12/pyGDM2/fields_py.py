...
#==============================================================================
def nullfield(pos, env_dict, wavelength, returnField='E'):
    """Zero-Field
...
def planewave(pos, env_dict, wavelength,
    ...
    if 'eps_env' in env_dict.keys():
        ...
        if cn2 != cn3:
            warnings.warn("planewave only supports a single interface so far (between `n1`/`n2`). " +
                          "The simulation might not be correct.")
...
def _three_layer_pw(wavelength, theta_inc, polar, z_d, spacing, n1, n2, n3, x, y, z):
...
def evanescent_planewave(pos, env_dict, wavelength, 
    ...
    polar : str, default: 'p'
        incident linear polarization. Either 's' or 'p'. 
        At 0 / 180 degrees incident angle (normal incindence), 's' is 
        polarized along x, 'p' along y.
...
def gaussian(pos, env_dict, wavelength, theta=None, polarization_state=None, 
...
def dipole_electric(pos, env_dict, wavelength, 
    ...
    R_farfield_approx : float, default: 9000
        emitter-observer distance (in nm) from which on an asymptotic 
        farfield approximation will be used
    ...
    for i,R2 in enumerate(pos):
        if returnField.lower() == 'e':
            ...
            if np.linalg.norm(R2-R1) <= R_farfield_approx:
