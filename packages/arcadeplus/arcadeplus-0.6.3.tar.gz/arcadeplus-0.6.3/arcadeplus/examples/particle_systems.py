...
python -m arcadeplus.examples.particle_systems
"""
import arcadeplus
...
def emitter_0():
    ...
    e = arcadeplus.Emitter(
        ...
        particle_factory=lambda emitter: arcadeplus.LifetimeParticle(
            ...
            change_xy=arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_FAST),
...
def emitter_3():
    ...
    e = arcadeplus.Emitter(
        ...
        particle_factory=lambda emitter: arcadeplus.LifetimeParticle(
            filename_or_texture=TEXTURE,
            change_xy=arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_SLOW),
            lifetime=DEFAULT_PARTICLE_LIFETIME,
...
def emitter_6():
    ...
    e = arcadeplus.Emitter(
        ...
        particle_factory=lambda emitter: arcadeplus.LifetimeParticle(
            ...
            change_xy=arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_SLOW),
            lifetime=DEFAULT_PARTICLE_LIFETIME,
            center_xy=arcadeplus.rand_on_line((0.0, 0.0), (SCREEN_WIDTH, SCREEN_HEIGHT)),
            scale=DEFAULT_SCALE,
            alpha=DEFAULT_ALPHA
        )
    )
...
def emitter_7():
    ...
    e = arcadeplus.Emitter(
        ...
        particle_factory=lambda emitter: arcadeplus.LifetimeParticle(
            filename_or_texture=TEXTURE,
            change_xy=arcadeplus.rand_on_circle((0.0, 0.0), PARTICLE_SPEED_FAST),
            lifetime=DEFAULT_PARTICLE_LIFETIME,
            scale=DEFAULT_SCALE,
            alpha=DEFAULT_ALPHA
        )
...
def emitter_13():
    ...
    e = arcadeplus.Emitter(
        center_xy=CENTER_POS,
        emit_controller=arcadeplus.EmitterIntervalWithTime(DEFAULT_EMIT_INTERVAL, DEFAULT_EMIT_DURATION),
        particle_factory=lambda emitter: arcadeplus.LifetimeParticle(
            filename_or_texture=TEXTURE,
            change_xy=arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_FAST),
            lifetime=DEFAULT_PARTICLE_LIFETIME,
...
def emitter_22():
    ...
    e = arcadeplus.Emitter(
        center_xy=CENTER_POS,
        emit_controller=arcadeplus.EmitterIntervalWithTime(0.2, DEFAULT_EMIT_DURATION),
        particle_factory=lambda emitter: arcadeplus.LifetimeParticle(
            filename_or_texture=TEXTURE,
            change_xy=(1.0, 1.0),
            lifetime=DEFAULT_PARTICLE_LIFETIME,
...
def emitter_23():
    ...
    e = arcadeplus.Emitter(
        center_xy=CENTER_POS,
        emit_controller=arcadeplus.EmitterIntervalWithTime(DEFAULT_EMIT_INTERVAL * 8, DEFAULT_EMIT_DURATION),
        particle_factory=lambda emitter: arcadeplus.LifetimeParticle(
            filename_or_texture=TEXTURE,
            change_xy=arcadeplus.rand_vec_magnitude(45, 1.0, 4.0),
            lifetime=DEFAULT_PARTICLE_LIFETIME,
...
def emitter_28():
    ...
    e = arcadeplus.Emitter(
        center_xy=CENTER_POS,
        emit_controller=arcadeplus.EmitterIntervalWithTime(DEFAULT_EMIT_INTERVAL * 5, DEFAULT_EMIT_DURATION),
        particle_factory=lambda emitter: arcadeplus.LifetimeParticle(
...
def emitter_34():
    ...
    textures = [arcadeplus.make_soft_circle_texture(48, p) for p in (arcadeplus.color.GREEN, arcadeplus.color.BLUE_GREEN)]
    e = arcadeplus.Emitter(
        center_xy=CENTER_POS,
        emit_controller=arcadeplus.EmitBurst(BURST_PARTICLE_COUNT),
        particle_factory=lambda emitter: arcadeplus.FadeParticle(
...
def emitter_35():
    """Use most features"""
    soft_circle = arcadeplus.make_soft_circle_texture(80, (255, 64, 64))
    textures = (TEXTURE, TEXTURE2, TEXTURE3, TEXTURE4, TEXTURE5, TEXTURE6, TEXTURE7, soft_circle)
    e = arcadeplus.Emitter(
        center_xy=CENTER_POS,
        emit_controller=arcadeplus.EmitterIntervalWithTime(0.01, 1.0),
        particle_factory=lambda emitter: arcadeplus.FadeParticle(
            filename_or_texture=random.choice(textures),
            change_xy=arcadeplus.rand_in_circle((0.0, 0.0), PARTICLE_SPEED_FAST * 2),
...
def emitter_36():
    ...
    class MovingEmitter(arcadeplus.Emitter):
    ...
    e = MovingEmitter(
        center_xy=CENTER_POS,
        emit_controller=arcadeplus.EmitInterval(0.005),
        particle_factory=lambda emitter: arcadeplus.FadeParticle(
            filename_or_texture=TEXTURE,
            change_xy=arcadeplus.rand_in_circle((0.0, 0.0), 0.1),
...
def emitter_38():
    ...
    e = arcadeplus.make_burst_emitter(
        center_xy=CENTER_POS,
...
def emitter_39():
    ...
    e = arcadeplus.make_interval_emitter(
        center_xy=CENTER_POS,
        filenames_and_textures=(TEXTURE, TEXTURE3, TEXTURE4),
        emit_interval=0.01,
        emit_duration=2.0,
...
class MyGame(arcadeplus.Window):
    def __init__(self):
        ...
        self.obj = arcadeplus.Sprite(":resources:images/pinball/bumper.png", 0.2, center_x=0, center_y=15)
        self.obj.change_x = 3
    ...
    def on_draw(self):
        ...
        self.obj.draw()
        if self.label:
            arcadeplus.draw_text("#{} {}".format(self.emitter_factory_id, self.label),
                             SCREEN_WIDTH / 2, SCREEN_HEIGHT - 20,
                             arcadeplus.color.PALE_GOLD, 20, width=SCREEN_WIDTH, align="center",
                             anchor_x="center", anchor_y="center")
        if self.emitter:
            self.emitter.draw()
            arcadeplus.draw_text("Particles: " + str(self.emitter.get_count()), 10, 30, arcadeplus.color.PALE_GOLD, 12)
