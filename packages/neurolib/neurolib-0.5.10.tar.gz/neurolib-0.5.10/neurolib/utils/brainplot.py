...
def plot_involvement_mean_amplitude(model, involvement, skip=300, lingress=True, invert=False):
    figsize = (3, 2)
    figsize = np.multiply(figsize, 0.75)
    plt.figure(figsize=figsize)
    rates = np.mean(model.output, axis=0)[::skip]
    plot_involvement = involvement[::skip] * 100 if not invert else ((1 - involvement[::skip]) * 100)
    plt.scatter(
        plot_involvement,
        rates,
    ...
    if lingress:
        slope, intercept, r_value, p_value, std_err = plot_linregress(
            plot_involvement, rates, kwargs={"c": "k", "alpha": 0.75}
        )
        plt.text(50 if not invert else 25, np.max(rates) * 0.75, f"$R^2={r_value**2:0.2f}$")
...
def plot_involvement_nodewise_amplitude(model, involvement, skip=300, lingress=True, invert=False):
    figsize = (3, 2)
    figsize = np.multiply(figsize, 0.75)
    plt.figure(figsize=figsize)
    rates = np.mean(model.output, axis=0)[::skip]
    ...
    for r in model.output:
        ...
        if lingress:
            slope, intercept, r_value, p_value, std_err = plot_linregress(
                involvement * 100 if not invert else (1 - involvement) * 100,
                r,
                kwargs={"c": "k", "lw": 1, "alpha": 0.25},
            )
    plt.xlim(0, 100)
    plt.xticks([0, 50, 100])
    plt.xlabel("Involvement [%]")
