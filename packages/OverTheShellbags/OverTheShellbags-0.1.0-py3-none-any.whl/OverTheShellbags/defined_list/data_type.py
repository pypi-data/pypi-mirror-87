...
class TypeList:
  ...
  SHELL_ITEM_TYPES = [
    ...
    {"type": "favorite_shell_item",               "sig": b"\x78"},
    {"type": "added_shell_item",                  "sig": b"\x74"},
...
def interprete_tuple(_type_data_sig_tuple):
  (start, end) = _type_data_sig_tuple

  sig_list = []
  for sig_data in range(start, end + 1):
    hexString_sig = cv.add_hex_paddings(hex(sig_data))
    bytes_sig = bytes.fromhex(hexString_sig)
    sig_list.append(bytes_sig)

  return sig_list

def convert_type_for_python():
  type_list = get_list_only()
  ...
  for _type_data in type_list:
    for type_data in _type_data:
      if type(type_data["sig"]) is bytes:
        ...
        sig_list = interprete_tuple(type_data_sig_tuple)
        for added_sig in added_sig_list:
          sig_list.append(added_sig)
        type_data["sig"] = sig_list

      else:
        type_data["sig"] = interprete_tuple(type_data["sig"])
convert_type_for_python()