...
class AutoMLStep(PipelineStep):
    ...
    def _get_automl_settings(self, context):
        ...
        # set console writer in automl client
        automl_client._console_writer = ConsoleWriter(sys.stdout)
        ...
        if run_config_object.target != constants.ComputeTargets.LOCAL:
            automl_client.automl_settings.compute_target = run_config_object.target
        ...
        if self._passthru_automl_config:
            ...
            automl_client._validate_input(parent_run_dto)
