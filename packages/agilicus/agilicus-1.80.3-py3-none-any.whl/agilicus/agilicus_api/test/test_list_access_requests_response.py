# coding: utf-8

"""
    Agilicus API

    Agilicus API endpoints  # noqa: E501

    The version of the OpenAPI document: 2020.11.25
    Contact: dev@agilicus.com
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import agilicus_api
from agilicus_api.models.list_access_requests_response import ListAccessRequestsResponse  # noqa: E501
from agilicus_api.rest import ApiException

class TestListAccessRequestsResponse(unittest.TestCase):
    """ListAccessRequestsResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test ListAccessRequestsResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # model = agilicus_api.models.list_access_requests_response.ListAccessRequestsResponse()  # noqa: E501
        if include_optional :
            return ListAccessRequestsResponse(
                access_requests = [
                    agilicus_api.models.access_requests.AccessRequests(
                        metadata = agilicus_api.models.metadata_with_only_id.MetadataWithOnlyId(
                            id = '123', ), 
                        status = agilicus_api.models.access_requests_status.AccessRequestsStatus(
                            user = {"id":"as3sdfl","first_name":"Example","last_name":"User","email":"user@example.com","type":"user"}, 
                            user_requests = [
                                agilicus_api.models.user_request_info.UserRequestInfo(
                                    spec = agilicus_api.models.user_request_info_spec.UserRequestInfoSpec(
                                        user_id = 'tuU7smH86zAXMl76sua6xQ', 
                                        org_id = 'IAsl3dl40aSsfLKiU76', 
                                        requested_resource = 'phone', 
                                        requested_sub_resource = 'self', 
                                        requested_resource_type = 'application_access', 
                                        request_information = 'I need this to do my job', 
                                        state = 'pending', ), )
                                ], ), )
                    ], 
                limit = 56, 
                previous_page_email = 'foo@example.com', 
                next_page_email = 'foo@example.com'
            )
        else :
            return ListAccessRequestsResponse(
                access_requests = [
                    agilicus_api.models.access_requests.AccessRequests(
                        metadata = agilicus_api.models.metadata_with_only_id.MetadataWithOnlyId(
                            id = '123', ), 
                        status = agilicus_api.models.access_requests_status.AccessRequestsStatus(
                            user = {"id":"as3sdfl","first_name":"Example","last_name":"User","email":"user@example.com","type":"user"}, 
                            user_requests = [
                                agilicus_api.models.user_request_info.UserRequestInfo(
                                    spec = agilicus_api.models.user_request_info_spec.UserRequestInfoSpec(
                                        user_id = 'tuU7smH86zAXMl76sua6xQ', 
                                        org_id = 'IAsl3dl40aSsfLKiU76', 
                                        requested_resource = 'phone', 
                                        requested_sub_resource = 'self', 
                                        requested_resource_type = 'application_access', 
                                        request_information = 'I need this to do my job', 
                                        state = 'pending', ), )
                                ], ), )
                    ],
                limit = 56,
        )

    def testListAccessRequestsResponse(self):
        """Test ListAccessRequestsResponse"""
        inst_req_only = self.make_instance(include_optional=False)
        inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == '__main__':
    unittest.main()
