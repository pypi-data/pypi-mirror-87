...
def power_law_guess (freq, psd) :

  '''
  Build power law guess.
  '''

  log_psd = np.log (psd[freq<10.])
  log_freq = np.log (freq[freq<10.])
...
def background_guess (freq, psd, numax, dnu, m_star=None, n_harvey=2, spectro=False, power_law=False) :
  ...
  if power_law :
    try :
      a, b = power_law_guess (freq, psd)
      guess[n_harvey*3] = a
      guess[n_harvey*3+1] = b
    except :
      print ('log-log regression for power law guess failed, this guess will build a null power law vector')
      guess[n_harvey*3] = 0
...
def perform_mle_background (freq, psd, n_harvey=2, r=1., m=1., teff=5770., guess=None, frozen_param=None, power_law=False,
                            ...
                            quickfit=False, num=5000, apodisation=False) :
  '''
