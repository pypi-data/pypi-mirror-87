...
class DialogueStateTracker:
    ...
    def recreate_from_dialogue(self, dialogue: Dialogue) -> None:
        ...
        self.total_steps = dialogue.total_steps
        self.total_errors = dialogue.total_errors
        self._reset()
    ...
    def as_dialogue(self) -> Dialogue:
        ...
        return Dialogue(self.sender_id, list(self.events), self.total_steps, self.total_errors)

    def add_conversation_steps(self, event: Event) -> None:
        should_add_step = False
        if isinstance(event, BotUttered):
            should_add_step = True
        elif isinstance(event, UserUttered):
            if event.text:
                should_add_step = not (event.text.startswith('/greet') or event.text.startswith('/disconnect')
                                       or event.text.startswith('/restart'))
        elif isinstance(event, SlotSet):
            should_add_step = event.key == 'system_event'
    ...
    def update(self, event: Event, domain: Optional[Domain] = None) -> None:
        ...
        if isinstance(event, UserUttered) and self.latest_message:
            if self.conversation_ended() and event.text == CONV_END_TRIGGER:
                return
    ...
    def conversation_ended(self):
        """Check if the latest message is the end call trigger"""
        return self.latest_message and self.latest_message.text == CONV_END_TRIGGER
