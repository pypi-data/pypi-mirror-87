...
from pygments.lexers import TOMLLexer, JsonLexer
...
class Rely:
    ...
    def template(self):
        ...
        if "from" in self.kwargs:
            ...
            from_object = Rely(self.client, from_type, "retrieve", id=from_id).act()
...
def fill_in_template(template, obj):
    ...
    if obj is None:
        ...
        return [
            fill_in_template(template_child, obj_child)
            for template_child, obj_child in zip(template, obj)
        ]
    else:
