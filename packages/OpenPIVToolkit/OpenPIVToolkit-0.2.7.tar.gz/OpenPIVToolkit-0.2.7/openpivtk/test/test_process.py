try:
    from openpivtk.process import extended_search_area_piv as piv
except: 
    from openpivtk.pyprocess import extended_search_area_piv as piv
import numpy as np
...
def test_piv_vs_extended_search():
    """ test of the simplest PIV run """
    import openpivtk.process
    import openpivtk.pyprocess
    frame_a = np.zeros((32,32))
    frame_a = random_noise(frame_a)
    frame_a = img_as_ubyte(frame_a)
    frame_b = np.roll(np.roll(frame_a,3,axis=1),2,axis=0)
    u,v = openpivtk.process.extended_search_area_piv(frame_a.astype(np.int32),frame_b.astype(np.int32),window_size=32)
    u1,v1 = openpivtk.pyprocess.extended_search_area_piv(frame_a.astype(np.int32),
                                           frame_b.astype(np.int32),
