...
ORDER BY tablespace_name'''
            ...
            location = (location,)
            measure = sheet[1] / 10
            if float(sheet[3]) > 1 and float(sheet[4]) < 1000 and sheet[0] not in (
                    'SYSTEM', 'SYSAUX', 'UNDOTBS1', 'USERS'):
                sql3 = f'''select TABLESPACE_NAME,ROUND(MAX_SIZE / (1024 * 1024), 2) Extensible,STATUS from dba_tablespaces 
                where TABLESPACE_NAME='{sheet[0]}' '''
                self.cursor.execute(sql3)
                count2 = self.cursor.fetchall()
                # print(count2)
                if count2[0][2] == 'ONLINE':
                    extensible = count2[0][1]-sheet[1]
                    print(sheet[0] + '表空间的占用率为：' + sheet[3] + '%,剩余空闲空间为：' + str(sheet[4]) + 'MB，最大还可扩容'+str(extensible)+'MB.')
    ...
    def createdatafile(self):
        ...
        for i in range(len(filename)):
            ...
            response = str(f"{filename[i]}表空间扩容{int(size[i]) + 1}mb成功")
            res.append(response)
        # print(res)
        return res
