...
def main():
    ...
    main_inner(pkg_root_dir, args.agent_path, args.env_path,
               args.port, args.debug, args.n_cores, args.cache)


def main_inner(pkg_root_dir, agents_path=None, env_path=None, port=8050,
               debug=False, n_cores=2, cache=False):

    os.environ["GRID2VIZ_ROOT"] = pkg_root_dir
    config_path = os.path.join(pkg_root_dir, "config.ini")

    if agents_path is not None:
        agents_dir = os.path.abspath(agents_path)
    else:            
...
if __name__ == "__main__":
    main_inner(pkg_root_dir=os.path.dirname(os.path.abspath(__file__)),
               agents_path="D:/Projects/RTE-Grid2Viz/grid2viz/NeurIPS_1.2.2_week1_debug",
               n_cores=2)
