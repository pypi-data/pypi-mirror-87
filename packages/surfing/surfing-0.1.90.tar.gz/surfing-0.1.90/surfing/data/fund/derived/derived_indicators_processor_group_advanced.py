    ...
    def calc_score(self):
        ...
        for fund_type in self.FUND_CLASSIFIER:
            ...
            if fund_type == 'stock':
                ...
                df.loc[:,'risk_ability'] = score_rescale(0.3 * score_rescale(-df['mdd~history_m']) + 0.4 * score_rescale(-df['annual_vol~history_m']) + 0.3 * score_rescale(-df['downside_std~history_m']))
                ...
                df.loc[:,'alpha_ability'] = score_rescale( 0.4 * score_rescale(df['annual_ret~history_m']) + 0.6 * score_rescale(df['total_ret']))
                #df.loc[:,'track_ability'] = score_rescale(-normalize(df['track_err~history_m']))
                df.loc[:,'risk_ability'] = score_rescale(0.3 * score_rescale(-df['mdd~history_m']) + 0.4 * score_rescale(-df['annual_vol~history_m']) + 0.3 * score_rescale(-df['downside_std~history_m']))
                df.loc[:,'stable_ability'] = score_rescale(0.4 * score_rescale(df['trade_days']) + 0.4 * score_rescale(df['continue_regress_v~history_m']) + 0.2 * score_rescale(df['scale']))
                df.loc[:,'total_score'] = score_rescale(df.loc[:,'alpha_ability'] + df.loc[:,'risk_ability'] + df.loc[:,'stable_ability'] )
