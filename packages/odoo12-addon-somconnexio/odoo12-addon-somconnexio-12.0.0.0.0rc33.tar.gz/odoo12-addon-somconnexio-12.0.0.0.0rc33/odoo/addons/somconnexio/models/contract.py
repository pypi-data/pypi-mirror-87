...
class Contract(models.Model):
    ...
    current_tariff_contract_line = fields.Many2one(
        'contract.line',
        compute='_get_current_tariff_contract_line',
    ...
    def _get_current_tariff_contract_line(self):
        adsl = self.env.ref('somconnexio.service_technology_adsl')
        mobile = self.env.ref('somconnexio.service_technology_mobile')

        if self.service_technology_id == mobile:
            service_categ = self.env.ref('somconnexio.mobile_service')
        elif self.service_technology_id == adsl:
        ...
        for line in self.contract_line_ids:
            if (line.product_id.categ_id.id == service_categ.id
                    and self._is_contract_line_active(line)):
                self.current_tariff_contract_line = line
                break
    ...
    @job
    def add_one_shot_contract_line(self, id, product_id):
        contract = self.browse(id)
        SubscriptionService(contract).create_one_shot(product_id)
