...
class Geometry:
    ...
    def get_summ_str(self):
        ...
        add('---------------geometry-----------------------')
        add('        Rthrt =', '%.3f'%self.Rthrt, 'in (Dthrt=%.3f in)'%(self.Rthrt*2))
        add('         Rinj =', '%.3f'%self.Rinj, 'in (Dinj=%.3f in)'%(self.Rinj*2))
        add('        Rexit =', '%.3f'%self.Rexit, 'in (Dexit=%.3f in)'%(self.Rexit*2))
        add('           At =', '%g'%self.At, 'in**2')
        add('          eps =', '%g'%self.eps, '')
        
        add('    pcentBell =', '%g'%self.pcentBell, '% ' + self.get_attr_comment('pcentBell'))
            
        add('           CR =', '%g'%self.CR, 'injector area / throat area')
        add('         Dinj =', '%g'%self.Dinj, 'in', maybe_show_ft( self.Dinj ))
        add('         Ainj =', '%g'%self.Ainj, 'in**2')
        add(' cham_conv_deg=', '%g'%self.cham_conv_deg, 'deg convergent section half angle' )
        
        noz = self.getNozObj()
        add('entrance_angle=', '%g'%noz.theta, 'deg nozzle initial expansion angle' )
        add('    exit_angle=', '%g'%noz.exitAng, 'deg nozzle exit angle' )
                    
        add('        Lcham =', '%g'%self.Lcham, 'in',self.Lcham_desc, maybe_show_ft( self.Lcham ))
        add('    Lcham_cyl =', '%g'%self.Lcham_cyl, 'in', maybe_show_ft( self.Lcham_cyl ))
        add('   Lcham_conv =', '%g'%self.Lcham_conv, 'in', maybe_show_ft( self.Lcham_conv ))
        
        add('       Ltotal =', '%g'%self.Ltotal, 'in', maybe_show_ft( self.Ltotal ))
        
        add('         Lnoz =', '%g'%self.Lnoz, 'in ' + \
            self.get_attr_comment('Lnoz'), maybe_show_ft( self.Lnoz ))
        
        
        add('     RchmConv =', '%g'%self.RchmConv, 'convergent section turn radius / throat radius')
        add('    RupThroat =', '%g'%self.RupThroat, 'upstream radius / throat radius')
        add('   RdwnThroat =', '%g'%self.RdwnThroat, 'downstream radius / throat radius')
        add('    LchmOvrDt =', '%g'%self.LchmOvrDt, 'chamber length / throat diameter')
        add('      LchmMin =', '%g'%self.LchmMin, 'in, minimum allowed chamber length')
        add('        Vcham =', '%g'%self.Vcham, 'in**3, approximate chamber volume')
