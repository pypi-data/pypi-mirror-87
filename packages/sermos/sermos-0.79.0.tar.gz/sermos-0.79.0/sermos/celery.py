...
def pipeline_retry(event: dict):
    ...
    if deployment_id is None or pipeline_id is None or execution_id is None:
    ...
    gen = PipelineGenerator(deployment_id=deployment_id,
    ...
    def generate(self):
        """ Loads methods based on sermos config file and decorates them as
            ...
            def demo_task(*args, **kwargs):t
                return True
        """
        # Set in k8s deployment as an environment variable when Sermos Admin
...
def configure_celery(celery: Celery):
    ...
    try:
        ...
        logger.warning(f"Unable to dynamically generate celery tasks: {e}")
