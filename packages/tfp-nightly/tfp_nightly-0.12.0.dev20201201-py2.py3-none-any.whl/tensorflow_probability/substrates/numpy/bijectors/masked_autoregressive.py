...
#   `tensorflow_probability/substrates/numpy/bijectors/masked_autoregressive.py`
...
from tensorflow_probability.substrates.numpy.bijectors import scale as scale_lib
from tensorflow_probability.substrates.numpy.bijectors import shift as shift_lib
...
def _make_dense_autoregressive_masks(
  ...
  def random_made(x):
    ...
    for (mask, output_size) in zip(masks, output_sizes):
      ...
      x = tf.matmul(x, tf.random.stateless_normal([input_size, output_size]) * mask)
      x = tf.nn.relu(x)
      input_size = output_size
    x = tf.matmul(
        x,
        tf.random.stateless_normal([input_size, params * event_size]) * masks[-1])
