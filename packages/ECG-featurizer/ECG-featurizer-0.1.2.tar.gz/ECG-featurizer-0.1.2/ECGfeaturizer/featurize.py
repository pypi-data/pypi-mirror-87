...
class get_features:
    ...
    def featurizer_dat(self, features , labels, directory, demographical_data):
        """
    ...
    def featurizer_mat(self, num_features,mat_dir):
        """
        Automatically derives features from ECG-files (only .dat files for now)
        Args:
            num_features (int): an integer representing the amount of ECGs in the dataset
            mat_dir (str): path to the ECG-data

        Returns:
            features_out (DataFrame): A DataFrame with features for all ECG-records
        """

        def _load_challenge_data(filename):
            x = loadmat(filename)
        ...
        voltage_res = 1
        feature_array = np.zeros((num_features,112))
        counter_all = 0
        counter_auto_annotated = 0
        labels_auto_annotated = []
        labels_not_annotated = []
        ...
        for ecgfilename in sorted(os.listdir(mat_dir)):
            ...
            if ecgfilename.endswith(".mat"):
                ...
                ecgdata, header_data = _load_challenge_data(mat_dir + ecgfilename)
                print(ecgdata.shape)
                #fs = header_data[0].split(" ")[2]
                fs = 500
                ...
                try:
                    ...
                    print("Not able to get R-peak features")
                    print("Future version will deal with this")
                    labels_not_annotated.append(header_data[15][5:-1])
                    ecg_file_not_annotated.append(ecgfilename)
                    continue
