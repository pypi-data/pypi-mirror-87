...
from stac_api.auth import utils
...
def test_decode_jwt(app, user):
    ...
    assert set(decoded_jwt.keys()) == set(["id", "exp", "iat", "iss", "max-exp"])
    ...
    a_jwt = jwt.encode({}, app.config["SECRET_KEY"], algorithm=utils.JWT_ALGORITHM,)
