...
class ObjectIdentifier(list):
    def __init__(self, data, metadata, filterRadiusLowpass=.7, axis='x'):
        ...
        if not filterRadiusLowpass == filtRadLowpass:
            ...
            weightsLowpass = calc_gauss_weights(filtRadLowpass)
            weightsHighpass = calc_gauss_weights(2*filtRadLowpass)
            
        preparePSF(metadata, axis)
    ...
    def FindObjects(self, thresholdFactor, numThresholdSteps="default", blurRadius=1, mask=None, splitter=None, debounceRadius=4, maskEdgeWidth=10, discardClumpRadius = 0):
        ...
        if (self.numThresholdSteps == 0): #don't do threshold scan - just use lower threshold (faster)
            ...
            self.tIm = (im > self.lowerThreshold)
            (labeledPoints, nLabeled) = ndimage.label(self.tIm)
            ...
            for i in range(nLabeled):
                ...
                m_i = imO.argmax()
                #print imO.shape, m_i, X[objSlices[i]].shape
                #x = (X[objSlices[i]]*imO).sum()/imO.sum()
                #y = (Y[objSlices[i]]*imO).sum()/imO.sum()
                
                x = X[objSlices[i]].flat[m_i]
                y = Y[objSlices[i]].flat[m_i]
                
                if self.PRIaxis == 'x':
                    Itl = im[x - lobeSeparation, y]
                    
                    Itr = im[x + lobeSeparation, y]
                    
                    if Itl > Itr:
                        x = x - 0.5*lobeSeparation
                    else:
                        x = x + 0.5*lobeSeparation
                else:
                    Itl = im[x, y - lobeSeparation]
                    
                    Itr = im[x, y + lobeSeparation]
                    
                    if Itl > Itr:
                        y = y - 0.5*lobeSeparation
                    else:
                        y = y + 0.5*lobeSeparation
