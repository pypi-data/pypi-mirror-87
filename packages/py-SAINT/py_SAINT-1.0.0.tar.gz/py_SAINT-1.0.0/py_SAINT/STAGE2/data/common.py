...
def get_patch_3d(*args, patch_size=96, scale=1):
    # print(args[0].shape)
    ih, iw, ic = args[0].shape[1], args[0].shape[2], args[0].shape[3]
    # tp = int(scale* patch_size)
    ip = patch_size
    ix = random.randrange(0, iw - ip + 1)
    iy = random.randrange(0, ih - ip + 1)
    # iz = random.randrange(0, ic - ip + 1)
    # print(ic,iz, iz+ip, args[0][:,iy:iy + ip, ix:ix + ip, iz:iz + ip].shape, args[1][iy:iy + ip, ix:ix + ip, iz:iz + ip].shape)
    # tc = 64
    ret = [
        args[0][:,iy:iy + ip, ix:ix + ip],
