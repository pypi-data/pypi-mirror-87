...
class AttrDict(jzSHc):
 def __init__(self,*args,**kwargs):
  jzSHC(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and jzSHi(socket,'IPV6_V6ONLY'):
...
class Half(jzSHP):
 def __init__(self,opt,sock,addr,jzSHV):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.jzSHV=jzSHV
  self.name='peer' if self.jzSHV else 'client'
  self.queue=[]
  self.dest=jzSHk
  self.err=jzSHk
  self.ready=jzSHy
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':jzSHy,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':jzSHy,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':jzSHy,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':jzSHy,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
