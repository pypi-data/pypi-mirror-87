...
class BarrierStream:
    ...
    def get_summ_str(self):
        ...
        add('    pcentFFC =', '%g'%self.pcentFFC, '%')
        add('   MRbarrier =', '%g'%self.MRbarrier, '')
        add('      MRwall =', '%g'%self.MRwall, '')
        add('    Twallgas =', '%.1f'%self.Twallgas, 'degR (TcBarrier=%.1f degR)'%self.TcODE_b)
        add('   effnessFC =', '%g'%self.effnessFC)
        add('WentrOvWcool =', '%g'%self.WentrOvWcool)
        add('    IspDel_b =', '%.2f'%self.IspDel_b, 'sec delivered vacuum barrier Isp')
        add('    IspODF_b =', '%.2f'%self.IspODF_b, 'sec')
        add('    IspODK_b =', '%.2f'%self.IspODK_b, 'sec (fracKin=%g)'%self.fracKin_b)
        add('    IspODE_b =', '%.2f'%self.IspODE_b, 'sec')
        add('  cstarERE_b =', '%.1f'%self.cstarERE_b, 'ft/sec')
        add('  cstarODE_b =', '%.1f'%self.cstarODE_b, 'ft/sec')
...
class CoreStream:
    ...
    def get_summ_str(self):
        ...
        c = self.geomObj.get_summ_str()
        print(c, file=output)
        
        
        add('---------------%s/%s core stream-----------------------'%(self.oxName, self.fuelName))
        add('    FvacTotal =', '%.2f'%self.FvacTotal, 'lbf')
        add('     FvacCore =', '%.2f'%self.FvacCore, 'lbf')
        if self.Pamb > 0.0:
            if self.Pamb > 14.5:
                add('     FseaLevel =', '%.2f'%self.Fambient, 'lbf, (Pamb=%g)'%self.Pamb)
            else:
                add('      Fambient =', '%.2f'%self.Fambient, 'lbf, (Pamb=%g)'%self.Pamb)
        
        if self.add_barrier:
            add('  FvacBarrier =', '%.2f'%self.FvacBarrier, 'lbf')
        add('   MRthruster =', '%g'%self.MRthruster, '')
        add('       MRcore =', '%g'%self.MRcore, '')
        if self.add_barrier:
            add('    MRbarrier =', '%g'%self.barrierObj.MRbarrier, '')
        add('           Pc =', '%g'%self.Pc, 'psia')
        
        # -------------------- Isp --------------------------
        add('=====> IspDel =', '%.2f'%self.IspDel, 'sec, ___DELIVERED VACUUM ISP___')
        
        if True:#self.Pamb > 0.0:
            add('       IspAmb =', '%.2f'%self.IspAmb, 'sec, (Pamb=%.4f) %s'%(self.Pamb, self.noz_mode))
            #add('        Pexit =', '%.4f'%self.Pexit, 'psia, Nozzle Exit Pressure')
        
        if self.effObj('Pulse') < 1.0:
            add('  IspDelPulse =', '%.2f'%self.IspDelPulse, 'sec, ___DELIVERED PULSING ISP___')
        
        add('   IspDelCore =', '%.2f'%self.IspDel_core, 'sec')
        add('       IspODF =', '%.2f'%self.IspODF, 'sec')
        add('       IspODK =', '%.2f'%self.IspODK, 'sec (fracKin=%g)'%self.fracKin)
        add('       IspODE =', '%.2f'%self.IspODE, 'sec')
        add('===> cstarERE =', '%.1f'%self.cstarERE, 'ft/sec, measured cstar = cstarERE / CdThroat')
        add('     cstarODE =', '%.1f'%self.cstarODE, 'ft/sec')
        add()
        
        add('   CfVacIdeal =', '%.5f'%self.CfVacIdeal, 'Ideal Vacuum Thrust Coefficient')
        add('     CfVacDel =', '%.5f'%self.CfVacDel, 'Delivered Vacuum Thrust Coefficient')
        add('     CfAmbDel =', '%.5f'%self.CfAmbDel, 'Delivered Ambient  Thrust Coefficient')
        add()
        
        # -------------------- efficiencies -----------------------
        c = self.effObj.get_summ_str()
        print(c, file=output)
        
        # --------------------- Flow Rates -------------------------
        add()
        #add('       oxName =', '%s'%self.oxName, '')
        #add('     fuelName =', '%s'%self.fuelName, '')
        add('     CdThroat =', '%g'%self.CdThroat, '(%s)'%self.CdThroat_method, 'throat flow coefficient')
        
        # -------------------------- Fuel Film Cooloing ------------------
        if self.add_barrier:
            add('      wdotTot =', '%g'%self.wdotTot, 'lbm/sec (O/F = %g)'%( self.wdotOx/self.wdotFl, ))
            add('  wdotTotCore =', '%g'%self.wdotTotcore, 'lbm/sec (O/F = %g)'%( self.wdotOxCore/self.wdotFlCore, ))
            add('  wdotTotBarr =', '%g'%self.wdotTot_b, 'lbm/sec (O/F = %g)'%( self.wdotOx_b/self.wdotFl_b, ))
            
            add('  wdotOxTotal =', '%g'%self.wdotOx, 'lbm/sec')
            add('   wdotOxCore =', '%g'%self.wdotOxCore, 'lbm/sec')
            add('   wdotOxBarr =', '%g'%self.wdotOx_b, 'lbm/sec')
            
            add('  wdotFlTotal =', '%g'%self.wdotFl, 'lbm/sec')
            add('   wdotFlCore =', '%g'%self.wdotFlCore, 'lbm/sec')
            add('   wdotFlBarr =', '%g'%self.wdotFl_b, 'lbm/sec')
        else:
            add('      wdotTot =', '%g'%self.wdotTot, 'lbm/sec (O/F = %g)'%( self.wdotOx/self.wdotFl, ))
            add('       wdotOx =', '%g'%self.wdotOx, 'lbm/sec')
            add('       wdotFl =', '%g'%self.wdotFl, 'lbm/sec')

        
        # ------------------- gas properties ------------------
        add('        TcODE =', '%.1f'%self.TcODE, 'degR')
        add('        MWchm =', '%g'%self.MWchm, 'g/gmole')
        add('     gammaChm =', '%g'%self.gammaChm, '')
        
        if self.add_barrier:
            c = self.barrierObj.get_summ_str()
            print(c, file=output)
