...
"""Reads images from an lfw-style folder (visit http://vis-www.cs.umass.edu/lfw),
   ...
   Some .npy files and corresponding names files generated by this script can
   be found at

       https://www.dropbox.com/sh/3tyt1o1n9y689bj/AADtlpovVJmi3nZcILm6rnSWa?dl=0
...
Options:
  ...
  -s <asym>, --asymm=<asym>   select images that differ from their mirror ones
                              ...
                              if s > 0, or -s times mean difference otherwise,
                              and save them to photos.npy [default: 1.0]
...
def _rgb2gray(rgb):
    R = rgb[:, :, 0].astype(numpy.float32)
    G = rgb[:, :, 1].astype(numpy.float32)
    B = rgb[:, :, 2].astype(numpy.float32)
    return R*0.2989 + G*0.587 + B*0.114


def _imread(filename):
    if OLD_IMREAD:
        return imread(filename, mode='L').astype(numpy.float32)
    else:
        return _rgb2gray(imread(filename))
...
if asymm != 1.0:
    ...
    pylab.title('images asymmetry distribution')
    pylab.show()
