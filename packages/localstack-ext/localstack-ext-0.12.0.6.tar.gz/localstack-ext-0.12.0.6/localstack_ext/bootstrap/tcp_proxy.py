...
class AttrDict(YDmbi):
 def __init__(self,*args,**kwargs):
  YDmbo(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and YDmbl(socket,'IPV6_V6ONLY'):
...
class Half(YDmbX):
 def __init__(self,opt,sock,addr,YDmbA):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.YDmbA=YDmbA
  self.name='peer' if self.YDmbA else 'client'
  self.queue=[]
  self.dest=YDmby
  self.err=YDmby
  self.ready=YDmbg
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':YDmbg,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':YDmbg,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':YDmbg,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':YDmbg,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
