...
class Script(PlainScript):
    ...
    def main(self):
        ...
        opts = self.options
        if opts.projects:
            opts.projects = list(map(str.strip, opts.projects.split(',')))
        if opts.platforms:
            opts.platforms = list(map(str.strip, opts.platforms.split(',')))
        ...
        try:
            ...
            installed = {}
            for status, result in Parallel(n_jobs=-1)(
            ...
            if ('confSummary.py' in installed
                ...
                data = {}
                exec(compile(open(os.path.join(dest, 'confSummary.py'), "rb").read(), os.path.join(dest, 'confSummary.py'), 'exec'), data)
