...
class simulation(object):
    ...
    def scatter(self, **kwargs):
        """evaluate the simulation via :func:`.scatter`"""
        scatter(self, **kwargs)
    ...
    def __eq__(self, other):
        ...
        if (len(self.struct.geometry) == len(other.struct.geometry) and
            ...
            if (np.sum(np.abs(self.struct.geometry - other.struct.geometry))==0 and 
                    ...
                    self.struct.n1_material.__name__ == other.struct.n1_material.__name__ and 
                    self.struct.n2_material.__name__ == other.struct.n2_material.__name__ and 
                    self.struct.n3_material.__name__ == other.struct.n3_material.__name__ and 
...
def scatter(sim, method='lu', matrix_setup='fortran', 
    ...
    method : string, default: "lu"
        inversion method. One of ["lu", "numpyinv", "scipyinv", "pinv2", "dyson", "cupy", "cuda"]
    ...
    callback : func, default: None
        ...
        simulation and timing info. Available dict_keys: ['i_wl', 'wavelength', 'sim', 't_inverse', 't_repropa'].
        ...
        returns the simulation results until the moment where it was interrupted.
    ...
    - The `scipy` solvers (like 'lu') run parallelized if BLAS
...
# =============================================================================
    ...
    if method.lower() not in ["numpyinv", "scipyinv", "lu", 
                                       "pinv2", "dyson", "cupy"]:
        raise ValueError('Error: Unknown solving method. Must be one of' +
                         ' ["lu", "numpyinv", "scipyinv", "pinv2",'+
                         ' "dyson", "cuda", "cupy"].')
    
    ## -- get field generator and wavelengths for spectrum
...
def decay_rate(sim, method='lu', matrix_setup='fortran', verbose=False):
    ...
    **Decay Rate of Magnetic Dipoles near Non-magnetic Nanostructures.** 
    ...
    """
    from pyGDM2.pyGDMfor import propa_sp_decay as forDecay
    ...
    if field_generator == fields.dipole_electric:
        ...
        if sim.struct.n1_material.__name__ != sim.struct.n2_material.__name__ or \
              sim.struct.n2_material.__name__ != sim.struct.n3_material.__name__:
...
def get_side_by_side(sim, wavelength, invertible=True, times_alpha=True, 
    ...
    """
    from pyGDM2.pyGDMfor import setupmatrix as forSetupmatrix
...
# =============================================================================
    ...
    if type(CAM0) != np.ndarray:
        ...
        if method.lower() == "cuda":
            method = "cupy"
        elif method.lower() in ["lu", "numpyinv", "scipyinv", "pinv2", "cupy"]:
...
# =============================================================================
    ...
    if method.lower() in ["numpyinv", "scipyinv", "lu", "pinv2"]:
    ...
    ## --- Dyson sequence (openMP - fortran routine)
        ...
        from pyGDM2.pyGDMfor import dysonseq as forDysonSeq
    ...
    ## --- CUDA based inversion on GPU via `cupy`
        ...
        raise ValueError('Invalid inversion method. Must be one of ["lu", "numpyinv", "scipyinv", "pinv2", "dyson", "cupy", "cuda"].')
...
# =============================================================================
# NUMBA  - pure python Green's dyad implementation
# =============================================================================
...
def get_SBS_numba(sim, wavelength, invertible=True, times_alpha=True, 
    ...
    if np.imag(eps2)!=0:
        warnings.warn("2D simulations do not support lossy environments so far. Using only real part of environment permittivity.")
        eps2 = np.real(eps2)
    

    ## --- run numba routine
    M = np.zeros((len(geo)*3,len(geo)*3), dtype=np.complex64, order='F')
    # if sim.struct.two_dimensional:
    #     from pyGDM2.propagators import t_sbs_2D
    #     t_sbs_2D(geo, wavelength, cnorm[0], cnorm[1], eps1, eps2, eps3, spacing, alpha, M)
    # else:
...
def get_SBS_cuda(sim, wavelength, invertible=True, times_alpha=True, 
    ...
    ## --- cuda kernel
    @cuda.jit()
    def t_sbs_kernel_cuda(geo, wavelength, cnorm, eps1, eps2, eps3, spacing, alpha, M):
        """
        Code for kernel.
        """
        ...
        if i < M.shape[0]/3 and j < M.shape[1]/3:
            ...
            if i==j:
                ...
                xx, yy, zz, xy, xz, yx, yz, zx, zy = G0_cuda(R1, R2, wavelength, eps2)
            
            ## --- 1-2-3 surface dyad
            xxs,yys,zzs,xys,xzs,yxs,yzs,zxs,zys = Gs123_cuda(R1, R2, wavelength, 
