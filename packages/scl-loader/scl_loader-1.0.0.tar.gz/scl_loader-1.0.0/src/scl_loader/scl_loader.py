...
class SCDNode:
    ...
    def add_subnode_by_elem(self, elem: etree.Element):
        ...
        elif re.fullmatch(REG_LN, elem.tag):
            new_node = LN(self._datatypes, elem, self._fullattrs, **attributes)
        elif re.fullmatch(REG_LN0, elem.tag):
    ...
    def get_DA_leaf_nodes(self) -> {} :
        """
            ...
            `array`
                ...
                (IntAdr is the path build with the names of the ancestors starting at the SCDNode on which get_DA_leaves is triggered)
        """
    ...
    def _collect_DA_leaf_nodes(self, node, leaves: {}, mms=False) -> {}:
        """
            /!\\ PRIVATE : do not use /!\\ 
    ...
    def _manage_SDI(self, inst_node: etree.Element, current_node=None):
        """
            /!\\ PRIVATE : do not use /!\\ 
