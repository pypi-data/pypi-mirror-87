...
from azureml.interpret.common.exceptions import DirectoryExistsException, \
    MissingRawTransformationsException, InitDatasetMissingException, \
...
def _save(scoring_explainer, directory='.', exist_ok=False):
    ...
    if not os.path.exists(directory):
        ...
        raise DirectoryExistsException('A directory already exists at this path. Please choose another path '
...
class ScoringExplainer(ChainedIdentity):
    ...
    def _get_raw_importances(self, engineered_importances):
        ...
        if self._data_mapper is not None:
            ...
            raise MissingRawTransformationsException('get_raw should not be passed in if neither '
                                                     'transformations nor feature maps are passed in. '
                                                     'Please see documentation for the ScoringExplainer '
...
class KernelScoringExplainer(ScoringExplainer):
    ...
    def __init__(self, original_explainer, initialization_examples=None, **kwargs):
        ...
        if initialization_examples is not None:
            ...
            raise InitDatasetMissingException('Please pass initialization_examples to create a new explainer.')
...
class DeepScoringExplainer(ScoringExplainer):
    ...
    :type serializer:
        An object with save method that returns a dictionary state and load method that returns the model.
    """
    ...
    def __getstate__(self):
        ...
        if not (hasattr(self._model, '__setstate__') and hasattr(self._model, '__getstate__')):
            if self._serializer is not None:
                ...
                raise UnsupportedModelException('Model passed to explainer must be serializable by pickle or '
                                                'custom serializer must be specified')
