...
def install_timescaledb():
 ...
 if os.path.exists('/usr/lib/postgresql/timescaledb.so'):
  return
 check_or_install('gcc','cmake gcc musl-dev libc-dev','Timestream')
 run('cd /tmp; git clone https://github.com/timescale/timescaledb.git')
 run('cd /tmp/timescaledb; ./bootstrap -DREGRESS_CHECKS=OFF; cd build; make; make install')
...
def check_or_install(command,package,api):
 try:
  run('which %s'%command,print_error=ezEsd)
 except ezEsR:
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=ezEsd):
  try:
