...
def raster_calculator_af_flex(input_, op, output_path, **kwargs): #KWARGS: datatype=None, ndv=None, gtiff_creation_options=None, compress=False, add_overviews=False
    ...
    if not datatype:
        print('input_', input_)
        datatypes = [hb.get_datatype_from_uri(i) for i in input_ if type(i) not in [float, int]]
    ...
    if not ndv:
        ndvs = [hb.get_ndv_from_path(i) for i in input_ if type(i) not in [float, int]]
        if len(set(ndvs)) > 1:
