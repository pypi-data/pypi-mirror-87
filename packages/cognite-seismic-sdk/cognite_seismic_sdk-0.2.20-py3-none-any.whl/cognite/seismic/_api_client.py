...
if not os.getenv("READ_THE_DOCS"):
    ...
    from cognite.seismic.data_classes.errors import _from_grpc_error, SeismicServiceError
...
def with_wrapped_rpc_errors(f):
    def wraps(*args, **kwargs):
        try:
            ...
            # strip messy internal grpc stack that confuses users
            tb = sys.exc_info()[2]
            while True:
                if tb.tb_next == None:
                    break
                    ...
                    tb = tb.tb_next
            raise _from_grpc_error(e).with_traceback(tb) from None
...
class CogniteSeismicClient:
    ...
    def __init__(self, api_key=None, base_url=None, port=None, custom_root_cert=None, *, insecure=False):
        # configure env
