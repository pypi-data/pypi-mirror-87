...
def expand(data_, definition_, items, data_mods=None):
  data = {}
  if data_mods is None: data_mods = {}
  if 'definition:'+definition_ not in items:
    logging.error('The definition {d} does not exist'.format(d=definition_))
    return None
    
  definition = items['definition:'+definition_]
  for el in definition['element']:
    ...
    elif 'definition:'+el['type'] in items:
      obj = data_mods.get(el['name'], data_.get(el['name'],{}))
      if type(obj) == str:
        if 'object:'+obj in items:
          defin = items['object:'+obj]['definition']
          if defin == el['type']:
            obj = items['object:'+obj]['data']
          else:
...
def find_all(items, add_missing_fields=None):
  if add_missing_fields is None: add_missing_fields = {'fields':{},'added':False}
  success = utils.search_all(r'(.+)\.object\.json', parse, items)
  if success:
    for (type_name, item) in items.items():
      if type_name.startswith('object:'):
        ...
        if 'definition:'+item['definition'] in items:
          item['depends_on'].append('definition:'+item['definition'])
        else:
          success = False
