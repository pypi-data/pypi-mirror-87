    ...
    def fit(self, data_dir, label=None, *, force_refit=False):
        ...
        if type(data_dir) is not str:
            ...
            for i in classes:
                ...
                df_i = data_dir.loc[label.index[label[list(label)[0]] == i]]
                df_i.to_csv(
    ...
    def _try_apply_quantizer(
        ...
        *,
        ...
        if type(data) is not str:
            ...
            return quantized
        # if (type(data) is not str):
        #  mask = unquantized.notna()
        #  quantized = quantized.where(mask, axis = 0, other=np.nan)
        #  toReturn = quantized.dropna(how='all', axis='columns')
        #  print(toReturn)
        #  return toReturn
