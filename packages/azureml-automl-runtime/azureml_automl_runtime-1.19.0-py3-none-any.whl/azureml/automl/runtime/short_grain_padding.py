...
from azureml.automl.runtime.data_transformation import infer_objects_safe
...
def pad_short_grains_or_raise(
    ...
    *Note:* If automl_settings.short_series_handling_configuration has value
    ...
    if freq is None:
        ...
        # we are working with the data frame where each grain has one value.
...
def pad_short_grains(X: pd.DataFrame,
    ...
    numeric_col = get_numeric_columns(
...
def get_numeric_columns(X: pd.DataFrame,
    ...
    :return: The set of numeric and non-numeric columns.
    """
    # First infer the column types and get numeric and other column types.
    # Subtract the special column types as grain and type.
    ...
    for col in X.columns:
        ...
        if _check_if_column_data_type_is_numerical(col_stats.column_type):
            numeric_columns.add(col)
    numeric_columns -= special_columns
    ...
    if isinstance(featurization_config, FeaturizationConfig):
        if featurization_config.column_purposes is not None:
            for col, purpose in featurization_config.column_purposes.items():
                if col in X.columns and purpose == FeatureType.Numeric:
                    numeric_columns.add(col)
                else:
                    numeric_columns.discard(col)
    ...
    for col in X_one.columns:
        if not type(X_one[col].dtype).__module__.startswith('pandas'):
            if np.issubdtype(X_one[col].dtype, np.int):
                ...
                # np.nan can not be converted to a date time, we need to use NaT for that.
                df_pad[col] = pd.NaT
