...
def check_openmp():
    ...
    print ("Compiling fortran module with OpenMP.\n"+
           ...
           "  - use '--no-fortran' to install without the fortran parts.")
    return True



if "--no-fortran" not in sys.argv:
    if check_openmp():
        openmp = "-fopenmp" 
        openmp_linker = "-lgomp"
    else:
...
if ("--no-fortran" in sys.argv):
    ...
    print ("Omitting compilation of fortran module.\n"+
           "Only the python API (`core_py` and `linear_py`) will be working!")
    optional_fortran_extension = []
else:
    optional_fortran_extension = \
            [Extension(
                    name = 'pyGDM2.pyGDMfor', 
