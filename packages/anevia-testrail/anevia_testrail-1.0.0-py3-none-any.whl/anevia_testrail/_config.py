...
class Config:
    ...
    def is_test_run_available(self):
        """
        In the case that an existing test run ID is supplied, validate that it exists and is not closed
        :return: True if the testrun exists and is OPEN
        """
        if self.test_run:
