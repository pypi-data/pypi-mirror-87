...
class BaseDataContext:
    ...
    def __init__(self,
        ...
        :type X: numpy.ndarray or pandas.DataFrame or Dataflow
        :param y: Input training labels.
        :type y: numpy.ndarray or pandas.DataFrame or Dataflow
        :param X_valid: validation data.
        :type X_valid: numpy.ndarray or pandas.DataFrame or Dataflow
...
class RawDataContext(BaseDataContext):
    ...
    def __init__(self,
        ...
        :type cv_splits_indices: numpy.ndarray or pandas.DataFrame
        """
        if isinstance(automl_settings_obj.featurization, Dict):
            featurization_config = FeaturizationConfig()
            featurization_config._from_dict(automl_settings_obj.featurization)
            self.featurization = featurization_config
        else:
            self.featurization = automl_settings_obj.featurization
        self.lag_length = automl_settings_obj.lag_length
        self.task_type = automl_settings_obj.task_type
        ...
        if automl_settings_obj.enable_streaming:
            ...
            if automl_settings_obj.label_column_name is not None:
                training_columns_to_drop.append(automl_settings_obj.label_column_name)
                validation_columns_to_drop.append(automl_settings_obj.label_column_name)
            if automl_settings_obj.weight_column_name is not None:
                training_columns_to_drop.append(automl_settings_obj.weight_column_name)
                validation_columns_to_drop.append(automl_settings_obj.weight_column_name)
            if automl_settings_obj.cv_split_column_names is not None:
                column_names = list(filter(None, automl_settings_obj.cv_split_column_names))  # type: List[str]
                training_columns_to_drop.append(column_names)

            X = training_data.drop_columns(training_columns_to_drop)
            y = training_data.keep_columns(automl_settings_obj.label_column_name) \
                if automl_settings_obj.label_column_name is not None else None
            sample_weight = training_data.keep_columns(automl_settings_obj.weight_column_name) \
            ...
            y_valid = validation_data.keep_columns(automl_settings_obj.label_column_name) \
                if validation_data is not None and automl_settings_obj.label_column_name is not None else None
            sample_weight_valid = validation_data.keep_columns(automl_settings_obj.weight_column_name) \
        ...
        super().__init__(X=X, y=y,
                         ...
                         weight_column_name=automl_settings_obj.weight_column_name,
                         cv_split_column_names=automl_settings_obj.cv_split_column_names)
