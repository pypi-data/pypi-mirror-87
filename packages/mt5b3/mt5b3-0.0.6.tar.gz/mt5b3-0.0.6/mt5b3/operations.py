...
def getCurrTime(ops):
    assets=ops['assets']
    bars=b3.getBars(assets[0],1,timeFrame=b3.INTRADAY)
    return bars['time'][0]
...
def startOps(ops): 
    ...
    mem=ops['mem']
    for asset in assets:
        dbar=b3.getBars(asset,mem,timeFrame=b3.INTRADAY)
...
def getCurrBars(ops,dbars):
    ...
    for asset in assets:
        dbar=dbars[asset]
        #pega nova barra    
        aux=b3.getBars(asset,1,timeFrame=b3.INTRADAY) # pega uma barra!
