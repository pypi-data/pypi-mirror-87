...
def stellar_framework (freq, psd, r, m, teff, nwalkers=64, dnu=None, back=None, numax=None, Hmax=None, Wenv=None, n_harvey=2, low_cut=10., 
                       guess_back=None, spectro=False, filename_back='background', 
                       ...
                       filemcmc_pattern='mcmc_sampler_pattern.h5', nsteps_mcmc_pattern=1000, parallelise=False, fit_l1=True, fit_l3=False, use_numax_back=False,  
                       progress=False, a2z_file='modes_param.a2z', a2z_in=None, nsteps_mcmc_peakbagging=1000, mcmcDir='.', instr='geometric', filename_peakbagging='summary_peakbagging.pdf',
                       nopeakbagging=False, coeff_discard_back=50, coeff_discard_pattern=50, coeff_discard_pkb=50, thin_back=10, thin_pattern=10, quickfit=False,
  ...
  :param use_numax_back: if set to ``True``, the ``numax`` values computed with the background fit will be used as input guess for the pattern fit.
    Otherwise, ``numax`` will be computed with the scaling laws. 
  ...
  if back is None :
    fitted_back, param_back = perform_mle_background (freq, psd, n_harvey=n_harvey, r=r, m=m, 
                              ...
                              frozen_harvey_exponent=False, low_cut=low_cut, fit_log=True, quickfit=quickfit,
    ...
    if back_mcmc :
      fitted_back, param_back, sigma_back  = explore_distribution_background (freq, psd,
                                       ...
                                       filemcmc=filemcmc_back, coeff_discard=coeff_discard_back, thin=thin_back, save_only_after_sampling=save_only_after_sampling)
  ...
  if a2z_in is None :
    df_a2z, pattern = perform_mle_pattern (dnu, numax, Hmax, Wenv, teff, freq, psd, back=fitted_back, wdw=None, 
                           n_order=n_order, split=0, angle=0, fit_l1=fit_l1, fit_l3=fit_l3, mass=m, show=False, filename=filename_pattern+'.pdf',
                           )

    if pattern_mcmc :
      df_a2z, pattern = explore_distribution_pattern (dnu, numax, Hmax, Wenv, teff, freq, psd, back=fitted_back, wdw=None,
                           n_order=n_order, split=0, angle=0, fit_l1=fit_l1, fit_l3=fit_l3, mass=m, guess=pattern, show=False,
                           show_corner=True, nsteps=nsteps_mcmc_pattern, filename=filename_pattern+'.pdf', parallelise=parallelise, progress=progress,
                           nwalkers=nwalkers, filemcmc=filemcmc_pattern, coeff_discard=coeff_discard_pattern, thin=thin_pattern, save_only_after_sampling=save_only_after_sampling)
  ...
  if not nopeakbagging :
    ...
    df_a2z = peakbagging (a2z_to_read, freq, psd, back=fitted_back, wdw=None, spectro=spectro,
                   fit_splittings=False, nsteps_mcmc=nsteps_mcmc_peakbagging, show_corner=True,
                   store_chains=True, mcmcDir=mcmcDir, order_to_fit=None, parallelise=parallelise, progress=progress,
                   strategy='order', nwalkers=nwalkers, normalise=False, instr=instr, filename_summary=filename_peakbagging, show=False,
