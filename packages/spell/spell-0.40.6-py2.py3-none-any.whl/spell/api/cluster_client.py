...
class ClusterClient(base_client.BaseClient):
    ...
    def rotate_cluster_storage_key(self, cluster_name, storage_key):
        payload = {
            "cluster_name": cluster_name,
            "storage_key": storage_key,
        }
        endpoint = url_path_join(
            CLUSTER_RESOURCE_URL, self.owner, cluster_name, "rotate_storage_key"
        )
        r = self.request("put", endpoint, payload=payload)
        self.check_and_raise(r)
    ...
    def register_serving_cluster(
        self, cluster_name, kube_config, default_sg_config, version, is_public
    ):
        ...
        version -- version for serving cluster
        """
        payload = {
