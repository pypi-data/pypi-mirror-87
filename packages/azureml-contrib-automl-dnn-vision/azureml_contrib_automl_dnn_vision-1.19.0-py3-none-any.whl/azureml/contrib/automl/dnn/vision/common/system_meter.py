        ...
        try:
            ...
            for i in range(device_count):
                ...
                if self.gpu_mem_usage_accumulator is not None:
                    ...
                    if mem_info_key not in self.gpu_mem_usage_accumulator[i]:
                        self.gpu_mem_usage_accumulator[i][mem_info_key] = AverageMeter()
                    self.gpu_mem_usage_accumulator[i][mem_info_key].update(mem_info.used / MEGABYTE)
                    usage_str += ", avg_mem_info_used={0:.2f} ".format(
                        self.gpu_mem_usage_accumulator[i][mem_info_key].avg)
                if self.gpu_usage_accumulator is not None:
                    ...
                    if gpu_usage_key not in self.gpu_usage_accumulator[i]:
                        self.gpu_usage_accumulator[i][gpu_usage_key] = AverageMeter()
                    self.gpu_usage_accumulator[i][gpu_usage_key].update(utilization_rates.gpu)
                    usage_str += ", avg_gpu_utilization={}% ".format(
                        self.gpu_usage_accumulator[i][gpu_usage_key].avg)
            return usage_str + "\n"
