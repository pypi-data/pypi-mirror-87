...
class GP(RandomProcess):
    ...
    @property
    def processes(self):
        """If this GP is a multi-output GP, this will give the underlying processes."""
        if not (isinstance(self.mean, MOM) and isinstance(self.kernel, MOK)):
            raise RuntimeError(
                f"Can only matrix-multiply multi-output GPs: "
                f"{self} is not a multi-output GP."
            )
        if self.mean.ps != self.kernel.ps:
            raise RuntimeError(
                "Processes corresponding to the mean and kernel are inconsistent. "
...
def matmul(h, x):
    x = x.processes  # Extract the processes that form the outputs.
    m, n = h.shape
