...
class Package(CleverDict):
    ...
    def create_essential_files(self):
        ...
        create_file(self.setup_filepath.parent / "LICENSE", self.license, overwrite=True)

        # The other files are just bare-bones initially, created as placeholders which can't then be overwritten by easyPyPI:
        create_file(
            self.setup_filepath.parent / self.name / "__init__.py",
            [f"from .{self.name} import *"],
        )
        create_file(
            self.setup_filepath.parent / self.name / (self.name + ".py"),
            [f"# {self.name} by {self.author}\n", f"# {datetime.datetime.now()}\n"],
        )
        create_file(
            self.setup_filepath.parent / self.name / ("test_" + self.name + ".py"),
            [
        ...
        create_file(
            self.setup_filepath.parent / "README.md",
            [
                f"# `{self.name}`\n",
        ...
        # TODO: Import defaults from README_template, test_template,
        # init_template to enable easier editing/personalisation, rather than
        # hard coding their template values as strings in .create_essential_file
