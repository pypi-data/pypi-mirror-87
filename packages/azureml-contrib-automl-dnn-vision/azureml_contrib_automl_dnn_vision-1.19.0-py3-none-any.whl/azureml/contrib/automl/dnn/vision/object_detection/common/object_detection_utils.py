...
from .constants import ArtifactLiterals, PredictionLiterals, TrainingLiterals, ValidationMetricType
...
def compute_metrics(eval_bounding_boxes, val_metric_type,
    ...
    if val_metric_type in ValidationMetricType.ALL_COCO:
        ...
        if eval_bounding_boxes and "segmentation" in eval_bounding_boxes[0]:
            coco_score = cocotools.score_from_index(val_coco_index, eval_bounding_boxes, "segm")
        else:
            coco_score = cocotools.score_from_index(val_coco_index, eval_bounding_boxes)
...
def score_validation_data(run, model_settings, settings, score_with_model):
    ...
    model = _fetch_model_from_artifacts(run_id=run.id, **model_settings)
