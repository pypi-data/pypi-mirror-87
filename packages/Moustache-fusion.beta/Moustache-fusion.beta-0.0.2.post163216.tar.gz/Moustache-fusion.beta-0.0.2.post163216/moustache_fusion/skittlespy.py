...
def set_annexes_length(annexes):
    for annexe in annexes:
        ...
        if output.returncode != 0:
            cmd = '%s %s' % (PDFINFO_PATH, annexe['path'])
            msgstr = 'Error trying to get page count for annexe "%s" using command %s (error code %d, output was: %s)'
            raise RuntimeError(msgstr % (annexe['name'], cmd, output.returncode, output.stderr.decode('utf-8')))
        output_array = output.stdout.decode('utf-8').split()
        pageinfo = output_array.index("Pages:")
        v = output_array[pageinfo + 1]
        if not v.isdigit():
            msgstr = 'Could not read page count for annexe "%s" using command %s %s (output was: %s)'
            raise RuntimeError(msgstr % (annexe['name'], PDFINFO_PATH, annexe['path'], v))
        annexe['length'] = int(v)
    ...
    for annex in annexes:
        ...
        if res.returncode != 0:
            if not stderr_content:
                msgstr = 'Error code %d returned by command for annexe "%s": %s'
                stderr_content = msgstr % (res.returncode, annex['name'], cmd)
