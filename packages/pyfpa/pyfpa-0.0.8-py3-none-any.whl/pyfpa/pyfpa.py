...
class fpa:
    ...
    def make_records(self, data_obj="data"):
        ...
        namer = list(self.function_result.index.names)
        count = 1
        for item in namer:
            if item == None:
                idx_item = namer.index(item)
                namer[idx_item] = 'NewCol' + str(count)
                count += 1
        self.rename_dimensions(namer, 'function_result')
    ...
    def plot_object(self,
        ...
        if data_obj == 'data':
            ...
            df = self.slice.reset_index()
        elif data_obj == 'pivot':
            df = self.pivot.reset_index()
        elif data_obj == 'function_result':
            df = self.function_result.reset_index()
        else:
            df = self.consolidation.reset_index()
