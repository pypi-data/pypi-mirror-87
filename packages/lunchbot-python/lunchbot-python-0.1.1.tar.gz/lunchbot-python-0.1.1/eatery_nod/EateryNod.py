...
class Day(object):
    ...
    def __init__(self, menu_items, day_name):
        ...
        self._day_names_swedish = {"Monday": "Måndag", "Tuesday": "Tisdag", "Wednesday": "Onsdag", "Thursday": "Torsdag", "Friday": "Fredag", "Other": "Andra rätter"} #Day name mappings from English to Swedish.
        #Get the day name in Swedish. If not found, return None
        if self.day_name in self._day_names_swedish.keys():
            self.day_name_sv = self._day_names_swedish[self.day_name]
        else: #If the day name in Swedish can not be found
            self.day_name_sv = None
        now = datetime.datetime.now() #Store the current time, just temporarily.
