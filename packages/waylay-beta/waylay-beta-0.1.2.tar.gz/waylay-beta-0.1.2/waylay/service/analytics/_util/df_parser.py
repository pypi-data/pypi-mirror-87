...
def _parse_df_header_row_array(data, data_axis=DATA_AXIS_COLUMN, **r_arg):
    ...
    if 'columns' in data:
        ...
        if row_names[0] == 'timestamp':
            idx = pd.DatetimeIndex(pd.to_datetime(dataframe.iloc[:, 0], unit='ms', utc=True))
            idx.name = row_names[0]
...
def _keys_and_key_series(index, key, iso_timestamp, **render_options):
    ...
    if isinstance(index, pd.DatetimeIndex):
        ...
        keys.append(key)
        key_series.append(
            list((index - pd.Timestamp('1970-01-01T00:00Z')) // pd.Timedelta('1ms'))
        )
        if iso_timestamp:
            keys.append(key+'_iso')
            key_series.append(
                list(index.strftime('%Y-%m-%dT%H:%M:%S,%f%z'))
            )
...
def render(v, **render_options):
    return _render_value(v, **render_options)


def render_timestamp(v, iso_timestamp=False, **render_options):
    if iso_timestamp:
        return v.isoformat()
