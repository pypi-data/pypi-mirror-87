...
def discrete_curvature_tensor_quad_edge_mesh_filter(*args, **kwargs):
    """Procedural interface for DiscreteCurvatureTensorQuadEdgeMeshFilter"""
    import itk
...
def discrete_curvature_tensor_quad_edge_mesh_filter_init_docstring():
    ...
    import itkTemplate
    import itkHelpers
    if isinstance(itk.DiscreteCurvatureTensorQuadEdgeMeshFilter, itkTemplate.itkTemplate):
        filter_object = itk.DiscreteCurvatureTensorQuadEdgeMeshFilter.values()[0]
    else:
        filter_object = itk.DiscreteCurvatureTensorQuadEdgeMeshFilter

    discrete_curvature_tensor_quad_edge_mesh_filter.__doc__ = filter_object.__doc__
    discrete_curvature_tensor_quad_edge_mesh_filter.__doc__ += "\n Args are Input(s) to the filter.\n"
    discrete_curvature_tensor_quad_edge_mesh_filter.__doc__ += "\n Available Keyword Arguments:\n"
    if isinstance(itk.DiscreteCurvatureTensorQuadEdgeMeshFilter, itkTemplate.itkTemplate):
        discrete_curvature_tensor_quad_edge_mesh_filter.__doc__ += itkHelpers.filter_args(filter_object)[0]
        discrete_curvature_tensor_quad_edge_mesh_filter.__doc__ += "\n"
        discrete_curvature_tensor_quad_edge_mesh_filter.__doc__ += itkHelpers.filter_args(filter_object)[1]
    else:
        discrete_curvature_tensor_quad_edge_mesh_filter.__doc__ += "".join([
