...
#
#     for block_data, kernel_block in pygeoprocessing3.iterblocks(
#             kernel_filepath):
...
def build_file_registry(base_file_path_list, file_suffix):
    ...
    for base_file_dict, path in base_file_path_list:
        for file_key, file_payload in list(base_file_dict.items()):
...
def build_lookup_from_csv(
    ...
    with open(table_path, 'rbU') as table_file:
        ...
        header_row = [str(x) for x in header_row]
        ...
        for row in reader:
            ...
            if warn_if_missing and '' in row:
                LOGGER.warn(
                    ...
                    table_path)
            lookup_dict[row[key_index]] = dict(list(zip(header_row, row)))
        return lookup_dict
