...
class DbFactory(with_metaclass(Singleton), IdleTips):
    ...
    def __init__(
            ...
            use_connection_pool=None,   # 非必需 True/False(默认) 是否使用线程池
            mincached=None,
            maxcached=None,
    ...
    def __init_log(self):
        ...
        if not self._log:
            ...
            self._log = self._kwargs["log"] = DLog(debug=self._debug).get_log
    ...
    def __print_config(self):
        self._log.info('singleton_sign:{}'.format(self.singleton_sign))
    ...
    def close(self):
        try:
            self._log.info('close {}'.format(self.singleton_sign))
    ...
    def origin_client(self):
        """测试功能，直接动态反射出db， """
        self._log.warning('use origin_client 直接反射出 db 暂时没有 异常处理加成')
    ...
    def switch_db(self, db_name):
        ...
        self._log.info("switch db to: {}".format(db_name))
