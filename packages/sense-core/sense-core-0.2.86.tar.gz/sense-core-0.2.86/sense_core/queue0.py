...
class BaseRabbit(object):
    ...
    def __init__(self, label='rabbit', socket_timeout=300, heartbeat=30, config_info=None, **kwargs):
        if config_info:
        ...
        self.no_ack = False
        self.delay = 0
        if kwargs.get('delay', 0) == 1:
            # dlx_queue:死信路由关联的队列  dlx_exchange: 死信exchange dlx_routing: 死信队列路由
            self.delay = 1
            self.queue_ttl = kwargs['queue_ttl']
            self.dlx_exchange = kwargs.get('dlx_exchange', '')
            self.dlx_queue = kwargs['dlx_queue']
            self.dlx_routing = kwargs.get('dlx_routing', '')
    ...
    def _declare_retry_queue(self):
        ...
        :return:
        """
        self._channel.exchange_declare(exchange=self.dlx_exchange,
                                       exchange_type='fanout',
                                       durable=True)
        self._channel.queue_declare(queue=self.dlx_queue,
                                    durable=True)
        self._channel.queue_bind(self.dlx_queue, self.dlx_exchange, self.dlx_routing)
...
class RabbitProducer0(BaseRabbit):
    ...
    def __init__(self, label='rabbit', socket_timeout=300, heartbeat=30, config_info=None, **kwargs):
        super(RabbitProducer0, self).__init__(label, socket_timeout, heartbeat, config_info, **kwargs)
    ...
    def _get_properties(self, **kwargs):
        # msg_ttl 消息过期时间
        if kwargs.get('msg_ttl', None):
            pro = pika.BasicProperties(
                delivery_mode=2,  # 使得消息持久化
                expiration=str(kwargs['msg_ttl'])
            )
        else:
            pro = pika.BasicProperties(
                delivery_mode=2,  # 使得消息持久化
