...
class GenWeights(nn.Module):
    def __init__(self,inpC, kernel_size=3, outC=32):
...
class GenWeightsInput(nn.Module):
    def __init__(self,inpC, kernel_size=3, outC=32):
        super(GenWeightsInput,self).__init__()
        self.kernel_size=kernel_size
        self.outC = outC
        self.inpC = inpC
        self.meta_block=nn.Sequential(
            nn.Conv2d(self.inpC, 32, 3, padding=(3 - 1) // 2, stride=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, 64, 3, padding=(3 - 1) // 2, stride=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 64, 3, padding=(3 - 1) // 2, stride=1)
...
class MSR_RDN(nn.Module):
    def __init__(self, args):
        ...
        self.GW = GenWeights(inpC=1)
        self.GW_input = GenWeightsInput(inpC=2)
        # self.Final = nn.Conv2d(32, 1, kSize, padding=(kSize - 1) // 2, stride=1)

    def forward(self, inp_x, dist, thickness):
        # print(dist)
        # f__1 = self.SFENet1(inp_x)
        input_weights = self.GW_input(thickness).view(inp_x.size(0), 64,1,3,3)
        input_unfold = nn.functional.unfold(inp_x, 3, padding=1)
        f__1 = input_unfold.transpose(1, 2).matmul(
            input_weights.view(input_weights.size(0), input_weights.size(1), -1).transpose(1, 2)).transpose(1, 2)
        f__1 = f__1.view(-1, 64, inp_x.size(2), inp_x.size(3))
        # f__1 = self.SFENet1(inp_x)
        x = self.SFENet2(f__1)
        RDBs_out = []
        for i in range(self.D):
