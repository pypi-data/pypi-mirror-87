...
class RestServerConnection(object):
    ...
    def add_tags(self, added_tags):
        ...
        r = self._low_level_http_client.post(url=endpoint_url, payload=payload)
...
UPLOAD_TYPE_URL_MAP = {
    ...
    "confusion-matrix": asset_upload_url(),
    "curve": asset_upload_url(),
    "dataframe": asset_upload_url(),
    "dataframe-profile": asset_upload_url(),
    "embeddings": asset_upload_url(),
    "git-patch": get_git_patch_upload_endpoint(),
    "histogram2d": asset_upload_url(),
    "histogram3d": asset_upload_url(),
    "model-element": asset_upload_url(),
    "notebook": asset_upload_url(),
    "text-sample": asset_upload_url(),
...
class RestApiClient(BaseApiClient):
    ...
    def set_experiment_system_details(
        ...
        payload = {
            ...
            "ip": ip,
            "machine": machine,
            "networkInterfaceIps": network_interfaces_ips,
            "os": _os,
    ...
    def get_registry_model_zipfile(self, workspace, registry_name, version, stage):
        ...
        if version is None and stage is None:
            raise ValueError("Please specify version or stage to download model")
...
class CometApiClient(BaseApiClient):
    ...
    def create_user(self, email, username, signup_source, send_email=True):
        ...
        response = self.post_from_endpoint("users", payload)
