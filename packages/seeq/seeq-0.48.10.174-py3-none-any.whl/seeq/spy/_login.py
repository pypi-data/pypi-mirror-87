...
def _login(username, password, access_key, url, directory, ignore_ssl_errors, proxy, credentials_file, auth_token,
    ...
    global client, user, supported_units

    try:
        if force or not client:
            # Clear out any global state and officially log out before attempting to log in
            logout(quiet=True)
        ...
        server_major, server_minor, server_patch = validate_seeq_server_version(status)
...
def logout(quiet=False, status=None):
    ...
    status = Status.validate(status, quiet)
    global client, supported_units, user, https_verify_ssl, https_key_file, https_cert_file
...
def validate_seeq_server_version(status: Status):
    ...
    if seeq_server_major == 0:
        if seeq_module_major != seeq_server_major or \
                seeq_module_minor != seeq_server_minor:
            status.warn(f'The major/minor version of the seeq module ({seeq_module_major}.{seeq_module_minor}) '
        ...
        if seeq_module_major != seeq_server_major:
            status.warn(f'The major version of the seeq module ({seeq_module_major}) '
                        f'does not match the major version of the Seeq Server you are connected to '
                        f'({seeq_server_major}). You may encounter unexpected errors and '
                        f'experience other forms of incompatibility.\n\n'
                        f'It is recommended that you issue the following PIP command to install a compatible '
                        f'version of the seeq module:\n'
                        f'"pip install -U seeq~={seeq_server_major}.{seeq_server_minor}"')
