...
class MaskLM(tf.keras.Model):
    ...
    def call(self, inputs, mode):
        ...
        if mode != tf.estimator.ModeKeys.PREDICT:
            ...
            mlm_labels = features['masked_lm_ids']
            tf.print('position', tf.shape(positions))
            tf.print('position idx', positions)
            tf.print('input_tensor', tf.shape(input_tensor))
            tf.print('mlm_labels', tf.shape(mlm_labels))
            tf.print('mlm labels idx', mlm_labels)
            mlm_labels.set_shape([None, None])
            # compute loss
            mlm_loss = empty_tensor_handling_loss(
                mlm_labels,
