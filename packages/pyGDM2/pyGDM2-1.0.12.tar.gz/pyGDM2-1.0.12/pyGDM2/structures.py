...
class struct_py(object):
    ...
    normalization : str, default: 'cube'
        either 'cube' (default), or 'hex'. Used to determine normalization
        factor for mesh-cells.
    ...
    """
    def __init__(self, step, geometry, material, n1=None,n2=None, normalization='cube', 
        ...
        elif type(normalization) in (int, float):
            self.normalization = normalization 
        else:
            raise Exception("Unknown datatype for `normalization`. " +
                            "New API requires to provide a str (mesh-type)")
        if n1 is not None and n2 is not None:
            ...
            warnings.warn("Deprecation: environment should be defined thorugh the `dyads` interface. " +
                          "Using the `struct` class will raise an error in future versions.")
