...
def parse_remote_volume_path(remote_path: str) -> (SavviHubVolume, str, str):
    ...
    return get_volume(volume_id), snapshot, file_path


def get_volume(volume_id: int) -> SavviHubVolume:
    context = Context(login_required=True)
    client = SavviHubClient(token=context.token)

    r = client.volume_read(volume_id)
    if not r:
        typer.echo('[Error] Volume not found!')
        sys.exit(1)
    return r
...
def ls(
    ...
    volume, snapshot, path = parse_remote_volume_path(source_path)
    volume_file_list(volume.id, snapshot, path, recursive, directory)
    return
...
def rm(
    ...
    volume, snapshot, path = parse_remote_volume_path(source_path)
    volume_file_remove(volume.id, snapshot, path, recursive)
...
def cp(
    ...
    if is_source_remote:
        source_volume, source_snapshot, source_path = parse_remote_volume_path(source_path)
        source_volume_id = source_volume.id
    if is_dest_remote:
        dest_volume, dest_snapshot, dest_path = parse_remote_volume_path(dest_path)
    ...
    while True:
        if source_volume_id is not None and dest_volume_id is not None:
            ...
            client.volume_file_copy(
                source_volume_id, source_file.path, source_snapshot,
