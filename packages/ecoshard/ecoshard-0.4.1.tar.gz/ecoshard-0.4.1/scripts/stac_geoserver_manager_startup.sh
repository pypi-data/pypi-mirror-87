...
git pull
nohup python3 expand_drive_service.py --app_port 8082 --disk_name geoserver-data-disk --device_name /dev/sdb --zone us-west1-b --max_size 256 --project salo-api > expand_drive_service_log.txt &
...
docker run --rm -d -it -v /mnt/geoserver_data/:/usr/local/geoserver/data_dir --name stac_manager --network=host us.gcr.io/salo-api/stac-geoserver-container:latest api.salo.ai 8888 maps.salo.ai 8080 4g
