...
def load_simulation(fname, pickle_encoding=None):
    ...
    if not hasattr(sim.struct, 'n1_material'):
        warnings.warn("Loading old API simulation. Reconstructing internal attributes.")
        from pyGDM2.structures import _n_env123_to_materials
        n1, n2, n3 = sim.struct.n1, sim.struct.n2, sim.struct.n3
        n1_material, n2_material, n3_material = _n_env123_to_materials(n1, n2, n3)
        sim.struct.n1_material = n1_material
        sim.struct.n2_material = n2_material
        sim.struct.n3_material = n3_material
...
def print_sim_info(sim, prnt=True, verbose=False):
    ...
    ## -------------------- environment
    if type(sim) == core.simulation:   # !!! COMPATIBILITY
        out_str += '\n' + ''
        out_str += '\n' + ' ------ environment -------'
        out_str += '\n' + '   n3 = {}  <-- top'.format(sim.struct.n3_material.__name__)
        out_str += '\n' + '   n2 = {}  <-- center layer (height "spacing" = {}nm)'.format(
                        sim.struct.n2_material.__name__, sim.struct.spacing)
        out_str += '\n' + '   n1 = {}  <-- substrate'.format(sim.struct.n1_material.__name__)
    elif type(sim) == core_py.simulation:
        out_str += '\n\n'
        out_str += sim.dyads.__repr__()
...
def adapt_map_to_structure_mesh(mapping, structure, projection=None,
    ...
    if nthreads is not  None:
        warnings.warn('deprecation warning: "nthreads" will be removed from' + 
                      '`adapt_map_to_structure_mesh` in a future release.')
    try:
        import scipy
...
def split_simulation(sim, geo_split, verbose=0):
    ...
    geo_split : list of 3-tuples
        list of coordinates to be split off the simulation geometry
    ...
    for i, pos in enumerate(geo):
        ...
        if np.linalg.norm(geo_split - pos, axis=1).min() < step/4 :
            idx_split.append(i)
        else:
            idx_remain.append(i)
    ...
    if verbose:
        ...
        print("number of search meshpoints:       {}".format(len(geo_split)))
...
def combine_simulations(sim_list):
    ...
    for sim in sim_list:
        ...
        if (sim.struct.n1_material.__name__ != combined_sim.struct.n1_material.__name__) or \
                (sim.struct.n2_material.__name__ != combined_sim.struct.n2_material.__name__) or \
                (sim.struct.n3_material.__name__ != combined_sim.struct.n3_material.__name__):
    ...
    combined_sim.struct = structures.struct_py(
                    ...
                    n1=combined_sim.struct.n1_material, 
                    n2=combined_sim.struct.n2_material, 
                    n3=combined_sim.struct.n3_material, 
