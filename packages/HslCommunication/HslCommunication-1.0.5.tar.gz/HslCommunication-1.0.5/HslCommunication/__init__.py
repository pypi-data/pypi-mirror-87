...
class NetworkBase:
	...
	def ReceiveMqttMessage( self, socket: socket, timeOut : int, reportProgress ):
		...
		if readCode.IsSuccess == False:
			return readCode

		readContentLength = self.ReceiveMqttRemainingLength( socket )
		if readContentLength.IsSuccess == False:
...
class SiemensFetchWriteNet(NetworkDeviceBase):
	...
	def __init__( self, ipAddress = '127.0.0.1', port = 1000 ):
		...
		self.port = port
		self.WordLength = 2
		self.iNetMessage = FetchWriteMessage()
		self.ByteTransform = ReverseBytesTransform()
	@staticmethod
...
class MqttHelper:
	@staticmethod
	def CalculateLengthToMqttLength(length):
		...
		if length < 128 * 128 * 128:
			...
			buffer[2] = length // 128 // 128 // 128
			return OperateResult.CreateSuccessResult(buffer)
	@staticmethod
	def BuildMqttCommand( control, flags, variableHeader, payLoad):
		'''将一个数据打包成一个mqtt协议的内容
	...
	def BuildSegCommandByString(message:str):
		...
		return buffer
	@staticmethod
	def ExtraMsgFromBytes( buffer, index):
		'''从MQTT的缓存信息里，提取文本信息'''
		indexTmp = index
	...
	@staticmethod
	def ExtraIntFromBytes( buffer, index ):
		'''从MQTT的缓存信息里，提取长度信息'''
		length = buffer[index] * 256 + buffer[index + 1]
		index += 2
	...
	def ExtraMqttReceiveData( mqttCode : int, data : bytearray ) -> OperateResult:
		...
		if len(data) < 2 + topicLength:
			return OperateResult("Code[" + mqttCode + "] Subscribe Error: " + SoftBasic.ByteToHexString(data, ' '))
...
class MqttSyncClient(NetworkDoubleBase):
	...
	def InitializationOnConnect(self, socket:socket):
		...
		print('MqttHelper.BuildConnectMqttCommand:' + SoftBasic.ByteToHexString(command.Content))
	...
	def ReadMqttFromCoreSocketServer( self, socket:socket, send:bytearray, sendProgress, handleProgress, receiveProgress ):
		...
		while True:
			...
			if server_receive.IsSuccess == False:
				return server_receive
			
			server_back = MqttHelper.ExtraMqttReceiveData(server_receive.Content1, server_receive.Content2)
			if server_back.IsSuccess == False:
				return server_back
			
			if len(server_back.Content2) != 16:
				return OperateResult(StringResources.Language.ReceiveDataLengthTooShort)
