...
class BlueprintFactory:
    ...
    def create_blueprint(self, name, authorizer, lifecycle, cors=False):
        ...
        @routes.route('/register', methods=['POST'], **kwargs)
        ...
        @routes.route('/confirm_registration', methods=['POST'], **kwargs)
        @handle_client_errors
        def confirm():
            body = routes.current_request.json_body
            username = get_param(body, 'username', required=True)
        ...
        @routes.route('/login', methods=['POST'], **kwargs)
        @handle_client_errors
        def login():
            body = routes.current_request.json_body
            username = get_param(body, 'username', required=True)
        ...
        @routes.route('/auth_challenge', methods=['POST'], **kwargs)
        @handle_client_errors
        def auth_challenge():
            body = routes.current_request.json_body
        ...
        @routes.route('/refresh', methods=['POST'], **kwargs)
        @handle_client_errors
        def refresh():
            body = routes.current_request.json_body
