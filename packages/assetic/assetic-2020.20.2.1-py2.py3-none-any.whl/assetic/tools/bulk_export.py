...
class SyncToLocalProcesses(object):
    ...
    def dbsync_initiate(self, dbserver, dbname, profiles, incremental=True,
        ...
        for profile in profiles:
            errorcode = self.create_export_task_db(profile, dbtools,
                                                   incremental)
            if errorcode != 0:
    ...
    def dbsync_finalise(self, dbserver, dbname, username=None, password=None,
        ...
        if tasks == None:
            ...
            sql = """update assetic_sync_manager set documentid = ?,
                    status = ?, date_last_modified=getdate()
    ...
    def initiate_export_task_to_file(self, profile, hidecontroldoc=False,
        ...
        """
        # first see how may rows are returned, if less than 10,000
        ...
        if 0 < totalrecords <= 10000:
            # use standard search api to get data - will need the keyfield
            # internal name to sort by
            ...
            ##record as processed
            self.create_immediate_export_as_doclink(profile.profilename, \
                                                    profile.profileguid,
                                                    hidecontroldoc, doc_group,
                                                    doc_category, \
                                                    doc_subcategory)
            msg = "Immediate creation of csv file '{0}'".format(fullfilename)
            self.logger.info(msg)
        else:
            ##else create background worker export task
            taskid = self.create_export_task(profile.profileguid)

            if taskid is not None:
                chk = self.create_export_task_as_doclink(profile.profilename,
                                                         profile.profileguid,
                                                         taskid, doc_group,
                                                         doc_category,
                                                         doc_subcategory)
                if chk == 0:
                    msg = "Background worker export task created for {0}".\
                ...
                if csvstring:
                    ...
                    if chk == 0:
                        ...
                        chk = self.update_completed_export_task_as_doclink(
                            doc, exportdocid, hidecontroldoc)
                        if chk == 0:
    ...
    def create_export_task_db(self, profile, dbtools, incremental):
        ...
        sql = """insert into assetic_sync_manager
            ...
            ,status,date_initiated,date_last_modified)
        ...
        if 0 < totalrecords <= 10000:
            ...
            ##update as processed
            record = "not required", profile.profileguid, profile.profilename, \
                     ...
                     profile.truncation_indicator,"Complete"
            ...
            if taskid != None:
                record = taskid, profile.profileguid, profile.profilename, \
                         ...
                         profile.spacedelimiter, profile.set_empty_as_null,\
                         profile.allow_delete,replace_special,\
                         profile.truncation_indicator,"In Progress"
                returncode = dbtools.execute_single(sql, record)
            else:
                returncode = 1
    ...
    def run_search_for_pages(self, searchguid, order_by_field
        ...
        if order_by_field:
            ...
            self.logger.warning("order_by_field not set for paginated search. "
    ...
    def update_completed_export_task_as_doclink(self, document, exportdocid,
                                                hidecontroldoc=False):
        """
        Use Assetic documents to record the completed export of a profile
        Allow document status of deleted, can still find history via API but
        not clog up searches with these documents.
