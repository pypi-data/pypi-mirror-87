...
@click.option(
    "--num-processes",
    type=int,
    help="The number of processes to run the model server on. By default this is (2 * numberOfCores) + 1",
    hidden=True,
)
@click.option(
    "--enable-batching",
...
def make_modelserver_create_request(
    ...
    **kwargs,
    ...
    if validate:
        validate_predictor(entrypoint, classname=classname)
    ...
    if enable_batching or max_batch_size or request_timeout:
        batching_config = BatchingConfig(
            ...
            request_timeout=request_timeout or BatchingConfig.DEFAULT_REQUEST_TIMEOUT,
            is_enabled=True,
        )
    else:
    ...
    **kwargs,
    ...
    if batching_flag is None:
        if max_batch_size or request_timeout:
            batching_config = BatchingConfig(
                max_batch_size=max_batch_size, request_timeout=request_timeout, is_enabled=True
            )
...
@click.option(
    ...
    help=(
        ...
        'To use the default, enter "default"'
    ),
    hidden=True,
)
@click.option(
...
def validate_predictor(entrypoint, classname=None):
    try:
    ...
    try:
        API.from_entrypoint(Path(entrypoint), classname=classname)
