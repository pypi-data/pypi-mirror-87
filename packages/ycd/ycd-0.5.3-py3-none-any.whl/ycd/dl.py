...
class ChatReplayDownloader:
    ...
    def get_youtube_messages(self, video_id, start_time=0, end_time=None, message_type='messages', chat_type='live', callback=None,spath="./",duration=100):
        pbar = ProgressBar(duration, status='Extracting...')
            ...
            while True:
                ...
                if('actions' in info):
                    for action in info['actions']:
                        ...
                        # print(time_in_seconds,flush=True)
    ...
    def get_chat_replay(self, url, start_time=0, end_time=None, message_type='messages', chat_type='live', callback=None,spath='./',duration=100):
        match = re.search(self.__YT_REGEX, url)
        if(match):
            return self.get_youtube_messages(match.group(1), start_time, end_time, message_type, chat_type, callback, spath, duration)
...
def dlmain(spath, duration, video_id):
    ...
    try:
        ...
        if(args.output is not None):
            if(args.output.endswith('.json')):
                ...
                open(spath, 'w').close()  # empty the file
        ...
        if(args.output is not None):
            ...
            print('\nFinished writing', num_of_messages,
                  'messages to', spath, flush=True)
