...
import itertools
import pathlib
import tensorflow.python.framework as tpf
...
KF_PIPELINE_HOST = "164.90.77.14"  # "bmlx.mlp.bigo.inner" #
...
def _retry_handler(f):
    return proc_utils.retry(
        retry_count=3,
        delay=5,
        allowed_exceptions=(tpf.errors_impl.UnavailableError),
    )(f)
...
class KubeflowStore(object):
    ...
    def create_pipeline_execution(
        ...
        except tpf.errors_impl.AlreadyExistsError:
            previous_context = self._ml_store.get_context_by_type_and_name(
                type_name=_PIPELINE_EXECUTION_CONTEXT_TYPE_NAME,
                context_name=execution.name,
            )
    ...
    def _make_mlpb_context_type(self, type_name, spec) -> int:
        ...
        except tpf.errors_impl.NotFoundError:
...
def _parse_status(status_str: Text):
    if status_str == "Running":
        return execution_pb2.Execution.State.RUNNING
    elif status_str == "Failed":
        return execution_pb2.Execution.State.FAILED
    elif status_str == "Completed":
        return execution_pb2.Execution.State.COMPLETED
    else:
        logging.warning("unrecogonize status: %s" % status_str)
        return execution_pb2.Execution.State.UNKNOWN
