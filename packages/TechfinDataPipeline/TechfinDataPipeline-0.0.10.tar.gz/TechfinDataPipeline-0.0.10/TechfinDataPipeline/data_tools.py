...
from invoice_struct import *
from nfe_xml_parser import *
...
def getData(stag, list_of_tables, connector, rename_rule=None, valid_assert=True, max_workers=1):
    ...
    for table in list_of_tables:
        ...
        if rename_rule is not None:
            data = data.rename(rename_rule, axis=1)
...
def getListOfTables(session,connector_name):
    ...
    tables =  [['arinvoice',['_orgid', 'businesspartner_id', 'deleted', 'docissue', 'invoice_id', 'transactiontype'],True],
               ['arinvoiceinstallment',['_orgid', 'invoice_id', 'invoiceinstallment_id', 'duedatecash', 'deleted', 'sequencecode'],True],
               ...
               ['apinvoiceinstallment', ['_orgid', 'invoice_id', 'invoiceinstallment_id', 'duedatecash', 'deleted', 'sequencecode'],True],
               ...
               ['arinvoicebra', None, False],
               ['apinvoicebra', None, False],
               ['arinvoiceorigin', None, False],
               ['organization', None, True],
               ['mddocreference', None, True],
               ['mdbusinesspartnerdocreference', None, True],
...
def ar_mergeData(downloaded_tables):
    ...
    # Dados do Cliente
    #-----------------------
    ar_merged_data = ar_merged_data.merge(downloaded_tables['mdbusinesspartnerdocreference'][['businesspartner_id', 'docreference_id']],
                                     left_on=['businesspartner_id'],
                                     right_on=['businesspartner_id'],
                                     how='left')
    
    ar_merged_data = ar_merged_data.merge(downloaded_tables['mddocreference'][['docreference_id', 'docreference', 'doctype']],
    ...
    ar_merged_data = ar_merged_data.merge(downloaded_tables['organization'][['federalid', 'uuid']],
                                    left_on='_orgid',
                                    right_on='uuid',
                                    how='left')
...
def ar_mergeDataBordero(ar_merged_data, downloaded_tables):
    ...
    if 'arpaymentsbank' in keys and 'arbankpayment' in keys and 'mdbankaccount' in keys:
        ...
        # Dados de borderos
        #-----------------------
        ar_merged_data = ar_merged_data.merge(downloaded_tables['arpaymentsbank'],
        ...
        ar_merged_data = ar_merged_data.merge(downloaded_tables['arbankpayment'],
        ...
        ar_merged_data = ar_merged_data.merge(downloaded_tables['mdbankaccount'],
                                                   left_on=['bank_id'],
                                                   right_on=['bank_id'],
                                                   how='left')
...
def ap_mergeData(downloaded_tables):
    ...
    ap_merged_data = ap_merged_data.merge(downloaded_tables['mdbusinesspartnerdocreference'][['businesspartner_id', 'docreference_id']],
                                     left_on=['businesspartner_id'],
                                     right_on=['businesspartner_id'],
                                     how='left')
    
    ap_merged_data = ap_merged_data.merge(downloaded_tables['mddocreference'][['docreference_id', 'docreference', 'doctype']],
                                     left_on=['docreference_id'],
                                     right_on=['docreference_id'],
                                     how='left')
    ...
    ap_merged_data = ap_merged_data.merge(downloaded_tables['organization'][['federalid', 'uuid']],
                                    left_on='_orgid',
                                    right_on='uuid',
                                    how='left')
...
def default_techfin_data_parser(connector, max_workers=1):
    ...
    if possible:
        ...
        ap_invoices_df = nfe_parser(downloaded_tables, 'ap', ap_invoices_df)
