        ...
        if message is None:
            if not self.silent_fail:
                logger.error(
                    ...
                    "".format(tracker.sender_id, self.template_name)
                )
            return []
    ...
    async def run(self, output_channel, nlg, tracker, domain) -> List[Event]:
        ...
        if not self.action_endpoint:
            logger.error(
                ...
                "--endpoints flag. "
                "{}/core/actions"
                "".format(tracker.sender_id, self.name(), DOCS_BASE_URL)
            )
        ...
        try:
            ...
            headers = {"Conversation-Id": tracker.sender_id}
            response = await self.action_endpoint.request(
                json=json_body, method="post", headers=headers, timeout=DEFAULT_REQUEST_TIMEOUT
            )
            status = 200
            ...
            if e.status == 400:
                ...
                logger.error(f"[{tracker.sender_id}] - {exception.message}")
                raise exception
            else:
            ...
            logger.error(
                ...
                "Error: {}".format(tracker.sender_id, self.name(), self.action_endpoint.url, e)
            )
            raise Exception("Failed to execute custom action.")
            ...
            logger.error(
                ...
                "Error: {}".format(tracker.sender_id, self.name(), status, e)
            )
            raise Exception("Failed to execute custom action.")
        finally:
            duration = time.perf_counter() - start
            logger.info(f"[{tracker.sender_id}] - Remote action took: {round(duration, 2)}s, url: {self.action_endpoint.url}")
            OUTGOING_REQUEST_LATENCY_SEC.labels(self.action_endpoint.url, status, "POST").observe(duration)
