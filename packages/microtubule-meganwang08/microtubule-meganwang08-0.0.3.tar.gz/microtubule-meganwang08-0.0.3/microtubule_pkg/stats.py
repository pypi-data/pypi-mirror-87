...
def ks_stat(data1, data2):
    """Compute the K-S test statistic between two ECDFs
    generated from `data1` and `data2`."""
    # Compute ECDF from data1
    x, y1 = np.sort(data1), np.arange(1, len(data1) + 1) / len(data1)

    # Compute corresponding values of the target CDF at x
    y2 = ecdf(x, data2)
    ...
    D_bottom = y2 - y1 + 1/len(data1)

    return max(D_top.max(), D_bottom.max())

@numba.njit
def draw_perm_reps_ks(x, y, size=1):
    """Generate array of permuation replicates of
    Behrens-Fisher test statistic."""
    out = np.empty(size)
    for i in range(size):
        out[i] = ks_stat(*draw_perm_sample(x, y))
