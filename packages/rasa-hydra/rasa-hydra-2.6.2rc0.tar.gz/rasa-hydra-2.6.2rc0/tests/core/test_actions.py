...
async def test_remote_action_logs_events(
    ...
    assert events[1] == BotUttered("hey there None!", metadata={"template_name": "utter_greet"})
...
async def test_action_utter_template(
    ...
    assert events == [BotUttered("this is a default channel", metadata={"template_name": "utter_channel"})]
...
async def test_action_utter_template_channel_specific(
    ...
    assert events == [
        BotUttered("you're talking to me on slack!", metadata={"channel": "slack", "template_name": "utter_channel"})
    ]
...
async def test_action_restart(
    ...
    assert events == [BotUttered("congrats, you've restarted me!", metadata={"template_name": "utter_restart"}), Restarted()]
...
async def test_action_default_fallback(
    ...
    assert events == [
        BotUttered("sorry, I didn't get that, can you rephrase it?", metadata={"template_name": "utter_default"}),
        UserUtteranceReverted(),
    ]
...
async def test_action_default_ask_rephrase(
    ...
    assert events == [BotUttered("can you rephrase that?", metadata={"template_name": "utter_ask_rephrase"})]
