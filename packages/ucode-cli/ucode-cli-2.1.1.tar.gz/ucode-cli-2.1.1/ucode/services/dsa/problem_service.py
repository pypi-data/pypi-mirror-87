...
class ProblemService:
    ...
    def detect_problem_code_in_folder(problem_folder):
        ...
        problem_files = {
            ...
            "stock_solution_cpp": stock_solution_file_cpp,
            "testcases": testcase_file,
            "testcases_sample": testcase_sample_file,
            "test_generator": test_generator_file
        }
    ...
    def check_problem(problem_folder):
        ...
        if not os.path.isfile(problem_files["stock_solution"]) \
                and not os.path.isfile(problem_files["stock_solution_cpp"]):
            CLog.error(f"Main Python solution file `{problem_files['stock_solution']}` "
                       f"or `{problem_files['stock_solution_cpp']}.cpp` is missing!")
        if not os.path.isfile(problem_files['test_generator']):
            CLog.error(f"Testcase input generator file `{problem_files['test_generator']}` is missing!")
    ...
    def create_problem(folder, problem_name, problem_code=None, lang="vi", translations=[], overwrite=False,
                       tags=[], difficulty=0.0, programming_language='py'):
        problem = Problem()
        ...
        with open(os.path.join(template_path, f'editorial.md'),
                  encoding="utf-8") as f:
            problem.editorial = Template(f.read()).render(problem=problem)

        ProblemService._parse_statement_file(os.path.join(template_path, "statement.md"), problem)
        problem.name = problem_name
        problem.code = problem_code
        ...
        with open(os.path.join(template_path, f'generator.{programming_language}.j2'),
                  encoding="utf-8") as f:
            content = Template(f.read()).render(problem_code=problem_code)
            problem.testcase_generator = {"lang": programming_language, "code": content}
    ...
    @staticmethod
    def load(problem_folder, load_testcase=False, load_testcases_only=False, force_testcase_format=None):
        problem = Problem()
        ...
        if not load_testcases_only:
            ...
            solution_file = os.path.join(problem_folder, f"{problem_files['stock_solution']}")
            if os.path.isfile(solution_file):
                idx = solution_file.rfind(".")
                with open(solution_file, encoding="utf-8") as f:
                    problem.solution = f.read()
                    problem.solution_lang = solution_file[idx + 1:]

            additional_solution_files = glob.glob(os.path.join(problem_folder, f"*_solution.*"))
            ...
            file_path = problem_files['test_generator']
    ...
    def save(problem : Problem, base_folder=".", problem_folder=None, problem_code=None,
        ...
        if not save_testcases_only:
            ...
            if problem.testcase_generator:
                f = open(os.path.join(problem_folder, f"input_generator.{problem.testcase_generator['lang']}"),
                         'w', encoding="utf-8", newline='')
                f.write(problem.testcase_generator["code"])
                f.close()

            if problem.solution:
                f = open(os.path.join(problem_folder, f"solution.{problem.solution_lang}"), 'w',
            ...
            if problem.solutions:
                for solution in problem.solutions:
                    f = open(os.path.join(problem_folder, f"solutions.{solution['lang']}"), 'w',
                             encoding="utf-8", newline='')
        ...
        if save_testcases:
            ...
            if not problem_code:
                problem_code = os.path.basename(problem_folder)
            ...
            # if problem.testcase_format == "themis" and overwrite:
            #     # clear all themis testcases
...
if __name__ == "__main__":
    ...
    #                       translations=['vi'], load_testcase=True)
    problem = ProblemService.load('../../../problems/another_one_bites_the_dust_', load_testcase=True)
    # problem = ProblemService.load('../../../problems/another_one_bites_the_dust', load_testcase=True)
    ProblemService.save(problem, "../../../problems", problem_folder="big_mod_py_", overwrite=True)
    print(problem.name)
