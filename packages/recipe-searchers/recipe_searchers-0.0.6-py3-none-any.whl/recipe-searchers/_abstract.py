...
class AbstractSearcher(metaclass=ExceptionHandlingMetaclass):
    ...
    def search(self, keyword) -> SearchResult:
        ...
        all_recipes : SearchResult = SearchResult([], self.website, keyword)
        found = True
        index = 1
        while(found):
            url = self.build_url(keyword, index)
            recipes = self.fetch_results(url)
            if recipes.length > 0:
    ...
    def fetch_results(self, url) -> SearchResult:
    ...
    def parse_results(self, soup) -> SearchResult:
