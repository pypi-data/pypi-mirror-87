...
from ..utility import TypeClass, TypeEnum, TypeRegister
...
class Label(TypeClass[LabelType]):
...
@TypeRegister(LabelType.CLASSIFICATION)  # pylint: disable=too-few-public-methods
...
class LabeledBox2D(Box2D, Label):
...
class LabeledPolygon2D(Polygon2D, Label):
...
class LabeledPolyline2D(Polyline2D, Label):
