...
class HTTPAdapterWithTCPKeepalive(HTTPAdapterWithSocketOptions):
    def __init__(self, *args, **kwargs):
        ...
        if getattr(socket, 'TCP_KEEPINTVL', None) is not None:
            self.socket_options += [(socket.IPPROTO_TCP, socket.TCP_KEEPINTVL, 15)]
        elif sys.platform == 'darwin':
            TCP_KEEPALIVE = getattr(socket, 'TCP_KEEPALIVE', 0x10)
            self.socket_options += [(socket.IPPROTO_TCP, TCP_KEEPALIVE, 15)]

        if getattr(socket, 'TCP_KEEPCNT', None) is not None:
            self.socket_options += [(socket.IPPROTO_TCP, socket.TCP_KEEPCNT, 5)]

        if getattr(socket, 'TCP_KEEPIDLE', None) is not None:
            self.socket_options += [(socket.IPPROTO_TCP, socket.TCP_KEEPIDLE, 180)]
...
class Connection:
    ...
    def do_post_request(self, url, allowable_response_status_codes=(200,), retries=None, retry_delay=None, **kwargs):
        assert url is not None
