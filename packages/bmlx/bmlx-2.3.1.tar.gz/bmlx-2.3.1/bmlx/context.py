...
class BmlxContext(object):
    ...
    def init(
        ...
        self.debug = debug
        self.engine = engine
        self.pipeline_storage_base = self.project.configs["settings"][
            "pipeline"
        ]["pipeline_storage_base"].as_filename()
        ...
        for k, v in custom_parameters:
            if self.custom_parameters[k].exists():
                if isinstance(self.custom_parameters[k].get(), int):
                    self.custom_parameters[k] = int(v)
                elif isinstance(self.custom_parameters[k].get(), float):
                    self.custom_parameters[k] = float(v)
                else:
                    self.custom_parameters[k] = v
