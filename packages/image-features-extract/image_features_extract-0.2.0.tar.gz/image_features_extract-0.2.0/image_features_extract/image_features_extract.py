...
def analyse_morphologique(img_bin, img):
    ...
    Returns:
        df (dataframe): index|'x'|'long_x'| 'y'|'long_y'| 'size'|'depth'
    '''
    ...
    def height(i):
        ...
        '''
        compute the maximum hegth of feature with label i
        '''
...
def top_hat_flattening(im,struct_elmt_size):
    ...
    ''' 
    mise à plat de l image en appliquant une transformation top hat à chacune
    des lignes de l image im
    
    Arguments:
        im (2D array): image to process
        struct_elmt_size (int): length of the structure element (must be larger then
            the larger length of the feature of interest)
    Returns:
