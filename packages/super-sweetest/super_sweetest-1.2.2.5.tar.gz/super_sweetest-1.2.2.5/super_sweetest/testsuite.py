...
from super_sweetest.globals import g, timestamp
from super_sweetest.windows import w
from super_sweetest.testcase import TestCase
...
class TestSuite:
    ...
    def run(self):
        ...
        try:
            ...
            elif g.platform.lower() in ('android',):
                g.driver.quit()
                logger.info('--- Quit the Driver: %s' % g.platform)
            elif g.platform.lower() in ('air_android'):
                air_android_ = AirAndroid()
                air_android_.teardown_air_android(g)
                logger.info('--- Stop the Server and App: %s  for %s' % (g.appPackage, g.platform))
            elif g.platform.lower() in ('air_ios'):
                air_ios_ = AirIos()
                air_ios_.teardown_air_ios(g)
                logger.info('--- Stop the Server and App: %s  for %s' % (g.appPackage, g.platform))
            if g.mqtt_client_:
                # 初始化类传{}即可
                mqtt_init = Mqtt_Init({})
                mqtt_init.teardown_mqttclient(g.mqtt_client_)
                logger.info('--- Stop the MQTT Server')
        except:
