...
class WPanel(QtWidgets.QTreeWidget):
    ...
    def open_menu(self, position):
        ...
        if w_item is not None:
            ...
            save_as = QtWidgets.QAction('Save to file ...', menu)
            save_as.triggered.connect(lambda x: self.save_object(obj))
            menu.addAction(save_as)

            if isinstance(obj, (GlobalN, DataN)):
                add_items = QtWidgets.QAction('Add items by cif ...', menu)
                add_items.triggered.connect(lambda: self.add_items(w_item))
                menu.addAction(add_items)
            ...
            if ((type(obj) is GlobalN) | (type(obj) is DataN)):
                ...
                for cls_item in l_list:
                    ...
                    add_item = QtWidgets.QAction(f'{prefix :}',
                                                 menu_item)
