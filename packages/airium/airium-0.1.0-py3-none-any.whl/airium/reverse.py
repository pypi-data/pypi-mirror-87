...
def _warn_dependencies(fcn):
    if not bs4_imported:  # pragma: no cover
        def no_bs4_warning(*_, **__):
            sys.stderr.write("In order to parse HTML, please install beautifulsoup4 package.")
...
@_warn_dependencies
def from_html_to_airium(html_str: str, include_module_head: bool = True) -> str:
...
def translate_el(node: Union[Tag, PreformattedString, NavigableString, str], doc_obj: str) -> Optional[str]:
    ...
    if not attribute_getter.isidentifier():
