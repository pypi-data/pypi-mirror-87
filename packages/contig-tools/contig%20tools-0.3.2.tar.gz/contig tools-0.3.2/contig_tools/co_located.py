...
def find_co_located(query_file, output_file, percent_id_threshold, genome_file=None, genome_file_list=None):
    ...
    for genome_filepath in genome_files:
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            parsed_blast_results.append(parsed_blast_result)
...
def write_result_table(genome_results, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        fieldnames = [f'{key}_{index+1}' for index, result in enumerate(genome_results[0]) for key in result.keys()]
        fieldnames.append('co-located')
        ...
        for genome_result in genome_results:
            ...
            if len(set(hit_subjects)) == 1:
                values.append('Y') # co-located
            else:
                values.append('N') # not co-located
