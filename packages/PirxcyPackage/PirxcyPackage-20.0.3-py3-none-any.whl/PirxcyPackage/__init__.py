...
print(crayons.red(f'▒▓▒░ ░  ░░▓  ░ ▒▓ ░▒▓░▒▒ ░ ░▓ ░░ ░▒ ▒  ░ ██▒▒▒ '))
print(crayons.red(f'░▒ ░      ▒ ░  ░▒ ░ ▒░░░   ░▒ ░  ░  ▒  ▓██ ░▒░ '))
print(crayons.red(f'╚═╝  ╚═══╝╚══════╝╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝   ╚═╝   ' 'V20.0.3'))
print(crayons.green(f'Coded By Pirxcy \nJoin The Discord - https://discord.gg/25fKfvrqwC\n'))
print(crayons.blue(f'Changelog: \n1. Added EID_ CID_ etc for messages\n2. Finally Fixed AntiBot\nAdded Ping Command To Check Latency'))
...
with open('config.json') as f:
    try:
        ...
        print(Fore.RED + ' [ERROR] ' + Fore.RESET + "There was an error in one of the bot's files! (config.json). If you have problems trying to fix it, join the discord support server for help - https://discord.gg/25fKfvrqwC")
...
async def event_party_member_join(member: fortnitepy.PartyMember) -> None:
    delay.sleep(0)
    await client.party.send(f'This Bot was Made For {nickname} and Made By Pirxcy\nTo Get Yours Join The Discord\nhttps://discord.gg/25fKfvrqwC\nEnjoy!')
...
@client.event
async def event_party_message(message):
    if "Lupus" in message.content:
        await message.author.kick()
        await ctx.send(f'Kicked {message.author} LMAO'
        ...
        await message.author.kick()
        await ctx.send(f'Kicked {message.author} LMAO')
    elif "Poki" in message.content:
        await message.author.kick()
        await ctx.send(f'Kicked {message.author} LMAO')
    elif "Hailey" in message.content:
        await message.author.kick()
        await ctx.send(f'Kicked {message.author} LMAO')
