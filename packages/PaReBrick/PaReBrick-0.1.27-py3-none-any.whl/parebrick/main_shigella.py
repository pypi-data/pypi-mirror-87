...
from parebrick.characters.unbalanced import \
...
def get_characters_stats_unbalanced(blocks, characters, tree_holder):
    ans = []
    labels = make_labels_dict(labels_file)
    # coli_shigella_count_all = Counter([label.split()[0] for label in labels.values()])

    for block, genome_colors in zip(blocks, characters):
        tree_holder.count_innovations_fitch(genome_colors)

        labeled_colors = {labels[k]: v for k, v in genome_colors.items()}

        ecoli_shigella_count_indel = Counter(label.split()[0] for label, color in labeled_colors.items() if color == 0)
        ...
        ans.append([block, score_rear, count_rear, count_all_rear,
                    ecoli_shigella_count_indel['Escherichia'], ecoli_shigella_count_indel['Shigella']])
...
def write_stats_csv_unbalanced(stats, cls, stats_file):
    rows = [['block', 'cluster', 'parallel_rear_score', 'parallel_rear_unique_innovation_count',
             'parallel_rear_all_innovations_count', 'e_coli_indels_count', 'shigella_indels_count']] + \
