...
class Regex:
    def __init__(self):
        ...
        self.month = re.compile(
            r"(?P<month>\b(?:january|february|march|april|june|july|august|september|october|november|december|jan\.?|feb\.?|mar\.?|apr\.?|may\.|jun\.?|jul\.?|aug\.?|sep\.?|sept\.?|oct\.?|nov\.?|dec\.?)\b)",
            re.IGNORECASE)
        ...
        self.rating=re.compile(r'\b(A|AA|AAA|B|BB|BBB)[+-]?\b')
        self.percentage=re.compile(r'\b[0-9]{1,3}[\.,][0-9]{2}\%')
        self.filename=re.compile(r'([A-Za-z0-9\(\)\. _,-])+(\.)(docx|doc|xlsx|xls|pdf|png|jpg|pptx|ppt)')

    def get_regex(self, regex_name):
