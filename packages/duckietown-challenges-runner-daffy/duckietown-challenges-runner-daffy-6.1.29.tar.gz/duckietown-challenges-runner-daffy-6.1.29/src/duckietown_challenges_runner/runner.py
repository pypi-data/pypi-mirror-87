...
from .login import do_docker_login
...
"""
    ...
    evaluator_name = (parsed.name or "noname") + "_" + precise_str
...
#
# class TermHandler:
#     def __init__(self, *, token, job_id, machine_id, process_id, evaluator_version, impersonate, timeout):
#         self.token = token
#         self.job_id = job_id
...
#         s = traceback.format_stack(_stack_frame)
#         msg_ = f"Evaluator received SIGTERM \n\n{s} "
#
#         status = ChallengesConstants.STATUS_JOB_ABORTED
#         cr0 = ChallengeResults(status, msg_, scores={})
#         logger.error(cr0=cr0)
#         dtserver_report_job(
#             token=self.token,
#             job_id=self.job_id,
#             stats=cr0.stats,
#             result=cr0.get_status(),
#             ipfs_hashes=cr0.ipfs_hashes,
...
def go_(
    ...
    sc = read_shell_config()
    if sc.docker_password and sc.docker_username:
        do_docker_login(sc.docker_username, sc.docker_password)
    #
    # handler = TermHandler(
    #     token=token,
    #     job_id=job_id,
...
def run_one(
    ...
    try:
        ...
        while True:
            ...
            logger.info(
                msg, running_details=running_details, exited=exited2, visualization_info=visualization_info
            )
