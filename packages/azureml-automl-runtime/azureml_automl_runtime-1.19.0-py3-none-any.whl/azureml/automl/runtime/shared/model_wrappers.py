...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import (
    AutoMLInternal, GenericTransformError, InsufficientMemory, MissingColumnsInData,
    ForecastHorizonExceeded, PowerTransformerInverseTransform, QuantileRange,
    TimeseriesDfContainsNaN, TimeseriesNoDataContext, TimeseriesNothingToPredict, TimeseriesNonContiguousTargetColumn,
    TimeseriesMissingValuesInY, TransformerYMinGreater,
    TimeseriesWrongShapeDataSizeMismatch, TimeseriesWrongShapeDataEarlyDest,
    TimeseriesGrainAbsentNoGrainInTrain, TimeseriesGrainAbsentNoLastDate, TimeseriesGrainAbsentNoDataContext,
    InvalidArgumentType, TimeseriesDfInvalidArgOnlyOneArgRequired,
...
class ForecastingPipelineWrapper(RegressionPipeline):
    ...
    def _infer_missing_data(
        ...
        for grain, df_one in X.groupby(self.grain_column_names):
            first_unknown_y_date = self._get_last_y_one_grain(df_one, grain,
                                                              ignore_data_errors,
    ...
    def _lag_or_rw_enabled(self) -> bool:
        tmp = \
            self._ts_transformer._transforms.get(constants.TimeSeriesInternal.LAG_LEAD_OPERATOR) is not None or \
            self._ts_transformer._transforms.get(constants.TimeSeriesInternal.ROLLING_WINDOW_OPERATOR) is not None
        return tmp
...
class PipelineWithYTransformations(sklearn.pipeline.Pipeline):
    ...
    def predict_proba(self, X):
        ...
        try:
            ...
            # We need to explicitly assign the column names and NOT through the constructor as the functionality of
            # providing inline column names might be considered as filtering by pandas in case `predict_proba` also
            # returns a `pandas.DataFrame`.
            probabilities_df.columns = self.y_transformer.classes_
...
class QuantileTransformerWrapper(BaseEstimator, TransformerMixin):
    ...
    def transform(self, y):
        ...
        :rtype: numpy.ndarray or scipy.sparse.spmatrix
        """
        try:
