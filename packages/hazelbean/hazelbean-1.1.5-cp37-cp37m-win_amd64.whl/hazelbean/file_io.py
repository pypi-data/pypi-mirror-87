...
def propose_fuzzy_merge(left_df, right_df, on=None, left_on=None, right_on=None, how=None,
                        fuzzy_merge_csv_path=None, fuzzy_merge_report_path=None):
    ...
    potential_merge, report = hb.fuzzy_merge(left_df, right_df, left_on, right_on, how=how, cutoff=0.5)
    ...
    # hb.write_to_file(report_string, fuzzy_merge_report_path)
    from io import StringIO
    df_string = StringIO(report_string)
    print('df_string', df_string, report_string)
    ...
    report = report[['possible_labels_from_left'] + [i for i in report.columns if i not in ['possible_labels_from_left']]]
    ...
    if fuzzy_merge_report_path is not None:
        report.to_csv(fuzzy_merge_report_path)
        # hb.write_to_file(report_string, fuzzy_merge_report_path)
    else:
        pass
...
def merge_dataframes_with_remap(left_df, right_df, remap_df_or_path, remap_left_col='remap_input', remap_right_col='remap_output', on=None, left_on=None, right_on=None, how='outer'):
    ...
    print(list(remap_df_or_path.columns))
