...
class AutoMLBaseSettings:
    ...
    def __init__(self,
        ...
        self.whitelist_models = self._filter_model_names_to_customer_facing_only(allowed_models)
    ...
    def _validate_model_filter_lists(self):
        ...
        if self.blacklist_algos is not None:
            ...
            # If everything but arima and/or prophet are blocked (equivalent to allowing arima/prophet) but
                ...
                if len(enabled_algos) == 0:
                    raise ConfigException._with_error(
                        AzureMLError.create(
                            ...
                            feature_name='Forecasting DNNs',
                            arguments=", ".join(["target_lags", "target_rolling_window_size"]),
                            reference_code=ReferenceCodes._SETTINGS_AUTOARIMA_PROPHET_NOT_SUPPORTED_BLOCKED
                        )
                    )
        ...
        if self.whitelist_models is not None:
            ...
            # If only arima and/or prophet are excluded (equivalent to exclude all but arima/prophet) but
            # lags/rolling windows enabled, we should raise scenario not supported.
            if self.is_timeseries and (self.lags is not None or self.window_size is not None):
                for algo in [constants.SupportedModels.Forecasting.Prophet,
                             constants.SupportedModels.Forecasting.AutoArima]:
                ...
                if len(actual_wlm) == 0:
                    raise ConfigException._with_error(
                        AzureMLError.create(
                            ...
                            feature_name='Forecasting DNNs',
                            arguments=", ".join(["target_lags", "target_rolling_window_size"]),
                            reference_code=ReferenceCodes._SETTINGS_AUTOARIMA_PROPHET_NOT_SUPPORTED_ALLOWED
                        )
                    )
