...
def cacheable(expire=cache_life_time, noconflict=False):
    ...
    def m(*args, **kwargs):
        ...
        if cached is not None:
            if cls and hasattr(cls, "__session__"):
                if isinstance(cached, (tuple, list, set)):
                    for c in cached:
                        if isinstance(c, CacheableBase) and c not in cls.__session__:
                            cls.__session__.merge(c, load=False)
                if isinstance(cached, CacheableBase) and cached not in cls.__session__:
