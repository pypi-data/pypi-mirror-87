...
def _get_payload(git_repo, payload_json, path_prefixes, robot_id, robot_run_id):
    ...
    for key in comments:
        if not key.startswith(git_repo.working_dir): ## TODO: if compiledb doesnt have absolute paths, this most likely will fail, fix later
...
def review(rest, git_repo, args, gerrit_config):
    ...
    payload = _get_payload(git_repo, args.payload, args.path_prefixes, args.robot_id, args.robot_run_id)
