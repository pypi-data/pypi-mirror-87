...
from langutil import C_OK, C_FAIL, C_ERROR, Arrays_binarySearch, C_UNKNOWN
...
from pypos3d.pftk.StructuredAttribut import GenericTransform, ValueOpDelta, Deltas, ChannelMorphStatus, ChannelMorphStatusList, PoserMaterial, Keys, calcMapping_KDTree, findNewDelta, filterLength
...
class Figure():
  ...
  def __init__(self, pf, idx=0, frf=None):
    ...
    self._childs = { } # Map (child's tree) sonName:PMOLink(son, parent, PoserMeshedObject)
    self._inkyChains = [ ] # List of inkyChain
    self._defaultPick = None
    self._displayOn = True
    self._linkParms = [ ]
    self._welds  = { } # Map (welding tree) sonName:PMOLink(son, parent, PoserMeshedObject)
...
#   def getName(self):
  ...
  def changeReference(self, oldPartName, newPartName, changeRefToBODY=True):
    ...
    self.__changeReference(self._childs, oldPartName, newPartName)
    self.__changeReference(self._welds, oldPartName, newPartName)
      ...
      for srcGC in lstCurGeom:
        ...
        for channelName in setTargetMorph:
          ...
          if len(setNewDeltas) > 0:
            ...
            srcActor.updateOrCreate(channelName, self.getRefName(), srcWG, setFinalDeltas, self._bodyIdx)
    ...
    if isinstance(pa, PoserMeshedObject):
      ...
      for act in lstdesc:
        try:
          ...
          self._childs = { k:v for k,v in self._childs.items() if (v.son!=actorName) and (v.parent!=actorName) }
          self._welds = { k:v for k,v in self._welds.items() if (v.son!=actorName) and (v.parent!=actorName) }
  ...
  def getWelded(self, firstName):
    ...
    '''
    setres = set( lnk.son for lnk in self._welds.values() if lnk.parent==firstName )
    parentName = self.getWeldParent(firstName)
    if parentName:
      setres.add(parentName)
    return list(setres)
...
#           driverChan.setTrackingScale(1.0)
  ...
  def __lnkAdd(self, dico, msg, sonName:str, parentName:str):
    prev = dico.get(sonName)
    if prev:
      # Child-Parent found --> Update
      logging.warning('Part[%s] already %s to [%s]', sonName, msg, parentName)

    son = self.findActor(sonName)

    parent = self.findActor(parentName)

    dico[sonName] = PMOLink(sonName, parentName, son, parent)
  ...
  def createActorLinks(self, ldesc, weldActors=True):
    ...
    if weldActors:
      for pfils in ldesc:
        gwp = pfils.getWeldParent()
        if gwp:
          self.__lnkAdd(self._welds, '', pfils.getName(), gwp)

  def __changeReference(self, dico, oldPartName, newPartName):
    replace = False
    sonName, parentName = '', ''
    ...
    for k,v in dico.items():
      ...
      if replace:
        replace=False
        dico[sonName] = PMOLink(sonName, parentName, v.sonPMO, v.parentPMO)
        
  def __checkActorDico(self, dico, msg):
    nberrors = 0
    rebuild = False
    for k,(son, parent, sonPMO, parentPMO) in dico.items():
      if not sonPMO:
        sonPMO = self.findActor(son)
        if sonPMO:
          # Create a new link
          rebuild = True
        else:
      ...
      if rebuild:
        dico[k] = PMOLink(son, parent, sonPMO, parentPMO)
  ...
  def checkActorLinks(self):
    ...
    nberrors =   self.__checkActorDico(self._childs, 'hierarchical')
        
    # Check the Weld table
    nberrors += self.__checkActorDico(self._welds, 'weld')
      ...
      if code==PoserFileParser.TT_WORD:
        try:
          ...
          if vc==PoserToken.E_name:
            ...
            code2,nw = st.getFullLine()
            self.__lnkAdd(self._childs, 'parented', val, nw)
            ...
            code2,nw = st.getFullLine()
            self.__lnkAdd(self._welds, 'welded', val, nw)
