...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':RsmnX(perm='m',auth=Rsmnx,arg=Rsmnx,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=Rsmnp)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=Rsmnx)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=Rsmnx):
...
def main():
 do_start_ftp(RsmnO(sys.argv[1]),asynchronous=Rsmnp)
