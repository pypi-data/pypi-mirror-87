...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':kSiRp(perm='m',auth=kSiRI,arg=kSiRI,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=kSiRw)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=kSiRI)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=kSiRI):
...
def main():
 do_start_ftp(kSiRY(sys.argv[1]),asynchronous=kSiRw)
