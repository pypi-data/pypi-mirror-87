...
class Project(CnvrgFiles):
    ...
    def put_files(self, paths=None, pattern="*", message="SDK Commit", depth=False, payload=None, prefix=None):
        if not paths:
        ...
        for p in paths:
            ...
            if is_dir:
                full_paths += FileUploader.get_files_and_dirs_recursive(root_dir=p, regex=pattern)
            elif is_file:
        ...
        commit_sha1 = self.start_commit_new(message=message, payload=payload)
    ...
    def start_commit_new(self, message="SDK Commit", payload=None):
        if payload is None:
            payload = {}
