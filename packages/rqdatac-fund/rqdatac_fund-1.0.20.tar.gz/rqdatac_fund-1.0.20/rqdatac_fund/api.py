...
def secondary_fund_support(func):
    ...
    def wrapper(order_book_ids, *args, **kwargs):
        ...
        oid_listed_date_map = {oid: _get_instrument(oid).listed_date for oid in order_book_ids}
        ...
        related_oid_listed_df = pd.DataFrame(
            {'_oids': order_book_ids,
             ret_df_index_names[0]: [get_exactly_id(oid) for oid in order_book_ids],
             '_listed_date': [oid_listed_date_map[oid] for oid in order_book_ids]})
        related_oid_listed_df['_listed_date'] = pd.to_datetime(related_oid_listed_df['_listed_date'], format='%Y-%m-%d')
        ret_df.reset_index(inplace=True)
        ret_df = ret_df.merge(related_oid_listed_df, how='inner', on=ret_df_index_names[0])
        ret_df = ret_df[ret_df[ret_df_index_names[1]] >= ret_df['_listed_date']]
        # 对于主次基金上架日期不一致修正后，会出现空值情况，对此统一返回 None
        if ret_df.empty:
            return
        ret_df.drop([ret_df_index_names[0], '_listed_date'], axis=1, inplace=True)
...
@export_as_api(namespace="fund")
def get_indicators(order_book_ids, start_date=None, end_date=None, fields=None, rule="deprecated",
    ...
    :param rule:  str, 可选：["ricequant", "deprecated"] (Default value = "deprecated")
    :param market:  (Default value = "cn")
    :returns: DataFrame or Series

    """
    order_book_ids = ensure_list_of_string(order_book_ids)
    check_items_in_container(rule, ["ricequant", "deprecated"], "rule")
    check_items_in_container(indicator_type, ["rank", "value"], "indicator_type")

    if rule == "deprecated":
        warnings.warn("rule=’deprecated’ will be removed in 2 weeks，please use rule=’ricequant’ instead.")

    if indicator_type == "rank" and rule == "deprecated":
        return None
...
@export_as_api(namespace="fund")
def get_snapshot(order_book_ids, fields=None, market="cn", rule="deprecated"):
    """获取基金的最新数据
    ...
    if rule == "ricequant":
        df = pd.DataFrame(result).set_index(["order_book_id", "latest_date"])
        df.index.names = ['order_book_id', 'datetime']
    else:
