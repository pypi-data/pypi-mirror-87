...
import cuml # 0.14.0
import cudf # 0.14.0
import cugraph # 0.14.0
import cupy as cp # 7.2.0
...
def find_neighbors(X, n_neighbors, metric, algorithm, distributed):
    ...
    print(f"Finding {n_neighbors} nearest neighbors using "
    ...
    if distributed:
        ...
        model = cuml.dask.neighbors.NearestNeighbors(n_neighbors=n_neighbors+1,
                                                     client=client)
...
def kneighbors_graph(idx, n_neighbors, n_fit):
    ...
    graph = csr_matrix((weight, idx.values.ravel(), indptr),
                        shape=(n_fit, n_fit))
...
def cluster(X, n_neighbors=30, community='louvain', metric='euclidean', 
            algorithm='brute', similarity='jaccard', min_size=10, distributed=False):
    """
    Clusters
    ...
    print(f'{similarity} graph constructed in {time.time() - subtic} seconds...', flush=True)
    ...
    if community == 'louvain':
        ...
        communities = sort_by_size(cp.asarray(parts.sort_values(by='vertex').partition), min_size)
