...
class disc_t(ctypes.Structure):
	_fields_ = [("num_intervals", ctypes.c_uint64), ("num_moments", ctypes.c_uint64), ("_intervals", ctypes.POINTER(ctypes.c_double)), ("_moments", ctypes.POINTER(ctypes.c_double)), ("mean", ctypes.c_double), ("var", ctypes.c_double)]
	...
	with numpy.nditer(data, flags = ['multi_index']) as it:
		while not it.finished:
			...
			if data[row,col] == MISSING_VALUE:
				R[row, col] = numpy.nan
			elif col < len(M) and M[col] is not None:

				state = it[0]
				
				m = M[col].moments[state][0]
				s = numpy.sqrt(M[col].moments[state][1])
				
				l = M[col].intervals[state][0]
				u = M[col].intervals[state][1]
				
				Y = len(M[col].intervals)
...
def save_discretization(fn, U):
	...
	for i in range(0,len(U)):
		temp["i"+str(i)] = U[i].intervals
		temp["m"+str(i)] = U[i].moments
		temp["M"+str(i)] = U[i].mean
		temp["V"+str(i)] = U[i].var
