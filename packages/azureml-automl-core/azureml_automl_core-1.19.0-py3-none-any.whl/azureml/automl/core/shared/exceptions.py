...
class AutoMLException(AzureMLException):
    ...
    def __init__(self,
        ...
        self._azureml_error_definition = \
            azureml_error.error_definition if azureml_error else None  # Optional[ErrorDefinition]
        ...
        if self._azureml_error:
            ...
            self.message_format = self._azureml_error_definition.message_format
        else:
    ...
    def pii_free_msg(self, scrubbed: bool = True) -> str:
        ...
        if self._azureml_error_definition:
            return cast(str, self._azureml_error_definition.message_format)
    ...
    def error_type(self):
        ...
        if self._azureml_error_definition:
            return self._azureml_error_definition.get_root_error()
    ...
    def error_code(self):
        ...
        if self._azureml_error_definition:
            return self._azureml_error_definition.code

        return getattr(self, "_error_code", self.error_type)

    def _get_all_error_codes(self) -> List[str]:
        if self._azureml_error_definition:
            return cast(List[str], self._azureml_error_definition.code_hierarchy)
...
class DataShapeException(UserException):
    ...
    def __init__(self, exception_message="", target=None, **kwargs):
        warnings.warn("DataShapeException is deprecated. Consumers of this exception should catch "
...
class InvalidValueException(ValidationException):
    ...
    def __init__(self, exception_message="", target=None, **kwargs):
        warnings.warn("InvalidValueException is deprecated. Consumers of this exception should catch "
                      "ValidationException, and producers should raise the same, while including an `azureml_error` "
                      "field for error classification purposes if needed")
        super().__init__(exception_message, target, **kwargs)
