...
class ClientTransport(threading.Thread, QObject):
    ...
    def __init__(self, port, ip_address, database, username, passwd, keys):
        # Вызов конструктора предков
        threading.Thread.__init__(self)
        QObject.__init__(self)

        # Класс базы данных - работа с базой
    ...
    def connection_init(self, port, ip):
        ...
        # Соединение, 5 попыток, при успехе устанавливаем флаг в True
        connected = False
        for i in range(5):
        ...
        # При отсутствии подключения - исключение
        if not connected:
        ...
        with socket_lock:
            ...
            # Отправка серверу приветственное сообщение
            try:
                send_message(self.transport, presense)
                ans = get_message(self.transport)
                # При возврате ошибки бросаем исключение
                if RESPONSE in ans:
                    if ans[RESPONSE] == 400:
    ...
    def process_server_ans(self, message):
        ...
        # При сообшении от пользователя добавляем в базу, даём сигнал о новом сообщении
