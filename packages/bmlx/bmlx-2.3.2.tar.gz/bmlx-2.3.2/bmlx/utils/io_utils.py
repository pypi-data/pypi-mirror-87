...
_REMOTE_FS_PREFIX = ["hdfs://", "s3://", "http://"]
...
def resolve_filesystem_and_path(where, is_uri=False):
    ...
    if parsed_uri.scheme == "hdfs":
        ...
        #s3 get_instance需指定bucket
        bucket = parsed_uri.netloc
        fs = CephFileSystem.get_instance(bucket)
        endpoint = CephFileSystem.get_endpoint_from_bucket(bucket)
        path = "%s/%s" % (endpoint,
            re.sub(r"/+", r"/", "%s/%s" %
                (bucket, parsed_uri.path)
            )
        )
    elif parsed_uri.scheme == "http" and where.find("fs-ceph") > 0:
        #ceph get_instance需指定bucket
        bucket = CephFileSystem.get_bucket_from_parsed_path(parsed_uri.path)
        fs = CephFileSystem.get_instance(bucket)
    else:
...
def copy(src, dst):
    ...
    dst_fs, dst_path = resolve_filesystem_and_path(dst_fs)
