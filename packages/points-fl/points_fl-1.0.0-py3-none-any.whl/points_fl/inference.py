...
class Inference(Base):
    ...
    def horizontal_predict(self, x, id):
            return self._inference_stub.HorizontalPredict(inference_pb2.HorizontalPredictRequest(x=x, id=id, token=self._token))

    def vertical_predict(self, uid, id):
            return self._inference_stub.VerticalPredict(inference_pb2.VerticalPredictRequest(uid=uid, id=id, token=self._token))
