...
class WMLEngine(BaseEngine):
                        ...
                        if status.get('state') == RunStateTypes.FAILED:
                            ...
                            if self._current_run_id in text and 'completed' in text:
                                ...
                                if len(prepared_text) > max_len:
                                    ...
                                    progress_bar.set_description(desc=prepared_text + ' '*(max_len-len(prepared_text)))

                            progress_bar.increment_counter(gen.get_progress(process))
                            progress_bar.update()
                    elif event.name in ['closing', 'disconnected']:
                        end = True
                        break
    ...
    def get_pipeline(self, pipeline_name: str, local_path: str = '.', persist: 'bool' = False) -> 'Pipeline':
        ...
        try:
            # note: get experiment information to retrieve Hybrid SW specs and look which scikit was used
            _, request_json = prepare_auto_ai_model_to_publish_normal_scenario(
                pipeline_model=pipeline_name,
            ...
            hybrid_sw_spec = request_json['hybrid_pipeline_software_specs'][-1]['name']
            # below includes scikit version check
            _, _ = get_sw_spec_and_type_based_on_sklearn(client=self._wml_client, spec=hybrid_sw_spec)
            # --- end note
        
        # do nothing when we cannot find request.json on user COS
