...
class TestOBMAndKB(unittest.TestCase):
    ...
    data_connections = []
    batch_deployment_data_connections = []
    main_conn = None
    customers_conn = None
    transactions_conn = None
    customers_transactions_conn = None
    purchases_conn = None
    products_conn = None
    ...
    def test_01_create_multiple_data_connections__connections_created(self):
        ...
        if self.wml_client.ICP:
            TestOBMAndKB.main_conn = DataConnection(
                ...
                location=FSLocation(path=self.data_location + 'group_customer_main_semicolon_delimiter.csv'))

            main_conn_without_delimiter = DataConnection(
                data_join_node_name="main",
                location=FSLocation(path=self.data_location + 'group_customer_main.csv'))

            TestOBMAndKB.customers_transactions_conn = DataConnection(
                data_join_node_name=["customers", "transactions"],
                location=FSLocation(path=self.data_location + 'group_customer.xlsx'))
            ...
            TestOBMAndKB.main_conn = DataConnection(
                ...
                location=S3Location(bucket=self.bucket_name,
                                    path=self.data_cos_path + 'group_customer_main_semicolon_delimiter.csv'))
            ...
            TestOBMAndKB.customers_transactions_conn = DataConnection(
                ...
                connection=S3Connection(
                    ...
                    secret_access_key=self.cos_credentials['cos_hmac_keys']['secret_access_key']),
                location=S3Location(bucket=self.bucket_name,
                                    path=self.data_cos_path + 'group_customer.xlsx'))
        ...
        TestOBMAndKB.data_connections = [self.main_conn,
                                         self.customers_transactions_conn,
                                         self.purchases_conn,
                                         self.products_conn]

        TestOBMAndKB.batch_deployment_data_connections = [main_conn_without_delimiter,
                                                          self.customers_conn,
    ...
    def test_02_create_data_join_graph__graph_created(self):
        ...
        data_join_graph.node(name="customers", sheet_name="customers")
        data_join_graph.node(name="transactions", sheet_name="transactions")
    ...
    def test_05_save_remote_data_and_DataConnection_setup(self):
        ...
        if is_cp4d():
            ...
            self.main_conn.write(data=self.data_location + 'group_customer_main_semicolon_delimiter.csv',
                                 remote_name=self.data_cos_path + 'group_customer_main_semicolon_delimiter.csv')
            self.customers_transactions_conn.write(data=self.data_location + 'group_customer.xlsx',
                                      remote_name=self.data_cos_path + 'group_customer.xlsx')
    ...
    def test_37a_score_deployed_model_notebook(self):
        scoring_params = self.service.run_job(
            payload=self.batch_deployment_data_connections,
            output_data_reference=DataConnection(
                connection=S3Connection(endpoint_url=self.cos_endpoint,
                                        access_key_id=self.cos_credentials['cos_hmac_keys']['access_key_id'],
                                        secret_access_key=self.cos_credentials['cos_hmac_keys']['secret_access_key']),
