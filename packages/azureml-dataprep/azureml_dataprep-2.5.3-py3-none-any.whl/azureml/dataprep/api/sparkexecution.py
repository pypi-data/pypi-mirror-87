...
def _add_lariat_spark_jar(sc: 'pyspark.SparkContext'):
    ...
    if sc._jvm.scala.util.Properties.versionNumberString().startswith("2.12"):
        print("Unable to load data as Spark with Scala 2.12 is not supported. Only Spark with Scala 2.11 is supported.")
        raise DataPrepSparkVersionError("Unable to load data as Spark with Scala 2.12 is not supported. Only Spark with Scala 2.11 is supported.")
