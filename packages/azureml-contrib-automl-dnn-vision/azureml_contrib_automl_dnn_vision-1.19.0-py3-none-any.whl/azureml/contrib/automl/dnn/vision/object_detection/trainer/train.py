...
from ..common.constants import ArtifactLiterals, TrainingParameters, ValidationMetricType
...
def train(model, optimizer, scheduler,
          ...
          run=None, ignore_data_errors=True):
    """Train a model
    ...
    for epoch in range(train_settings.number_of_epochs):
        ...
        if val_metric_type != ValidationMetricType.NONE:
            map_score = compute_metrics(eval_bounding_boxes, val_metric_type,
                                        ...
                                        coco_metric_time, voc_metric_time, master_process, run)
            ...
            if master_process and run is not None:
                run.log(train_settings.primary_metric, round(map_score, 3))
        else:
