...
def query_dag_exit(rd,debug=False,verbose=False):
    ...
    """
    if "marginalize_intrinsic_parameters_BasicIterationWorkflow.dag.dagman.out" not in os.listdir(rd):
        return 300000 # no .out exists, we'll assum because this hasn't been run yet
    else:
        try:
