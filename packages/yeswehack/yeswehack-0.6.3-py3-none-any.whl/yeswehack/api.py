...
class YesWeHack(YwhApiObject):
    ...
    def raw_call(self, method, url, data=None, headers=None):
    ...
    def post_comment(self, report_id, comment, private=False):
        data = {"message": comment, "attachments": []}
        headers = {**self.session.headers, **self.apps_headers}
        resp = self.raw_call('POST', f'{self.api_url}/reports/{report_id}/comments', data=json.dumps(data), headers=headers)
        try:
            json_resp = resp.json()
        except Exception as e:
            raise InvalidResponse
    ...
    def managed_programs(self, lazy=False):
        if not managed_pgms or not lazy:
