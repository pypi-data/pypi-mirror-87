    ...
    def execute_scalar_query(self, query, text_as_bytes=False) -> NullableValue:
        ...
        with self.execute_query(query, text_as_bytes) as result:
            if len(result.schema.columns) != 1:
                raise HyperException(0xA1B8BBEC6D, 'Query result must have exactly one column')
            if not result.next_row():
                raise HyperException(0xB8BBEC6DA1, 'Query returned zero rows')
            value = result.get_value(0)
            if result.next_row():
                raise HyperException(0xBEC6DA1B8B, 'Query returned more than one row')
            return value
