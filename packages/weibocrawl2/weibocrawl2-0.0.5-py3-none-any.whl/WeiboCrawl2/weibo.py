...
class Weibo(object):
                ...
                for w in weibos:
                    if w['card_type'] == 9:
                        ...
                        if wb:
                            ...
                            if (created_at >= since_date) and (created_at <= end_date):
                                return 1
        except:
            return None
                ...
                for w in weibos:
                    if w['card_type'] == 9:
                        ...
                        if wb:
                            ...
                            #截止时间
                            end_date = datetime.strptime('2020-9-30', '%Y-%m-%d')
                            self.overglag=False if created_at > end_date else True
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            file_dir = 'weibo' + os.sep + self.user['screen_name']
    ...
    def weibo_to_mongodb(self, wrote_count):
        ...
        self.info_to_mongodb(self.user['screen_name'], self.weibo[wrote_count:])
    ...
    #定位起点
    def cal_page_end(self, page_count):
        left=1
        right=page_count
        middle=(left+right)//2
        is_end = self.get_page_pos(middle)

        while abs(middle-left)>3:
            print(left)
            if is_end==2:
                left=middle
                middle = (left + right) // 2
    ...
    def get_pages(self):
        ...
        try:
            ...
            if since_date <= today:
                ...
                # print('page_count')
                # print(page_count)
                # left=self.cal_page_end(page_count)
                # left_page=left if left is not None else 1
    ...
    def load_users(self, userpath):
        ...
        try:
            ...
            id_list = [{
                'user_id': str(user_id),
                'since_date': self.since_date
            } for user_id in df['symbol']]
            # print(id_list)
            self.user_config_list=id_list
    ...
    def start(self, index):
        ...
        try:
            # 程序终止时间
            endstamp = datetime.strptime('2021-12-30', '%Y-%m-%d')
            ...
            print(index)
            if index not in range(0, len(self.user_config_list)):
                print('please input the correct starting point index')
                return
            else:
                self.user_config_list=self.user_config_list[index:]
...
def main(userpath, index=0):
    try:
        config = get_config()
        wb = Weibo(config)
        wb.load_users(userpath)
        wb.start(index)  # 爬取微博信息
    except Exception as e:
        logger.exception(e)


if __name__ == '__main__':
    userpath='D:\Iseuwei\WorkSpace\PyCharm\WeiboCrawl\WeiboCrawl/comments_user.xlsx'
    main(userpath, 0)
