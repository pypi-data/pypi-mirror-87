...
class Text:
    def __init__(text, content, x, y, color, size):
        ...
        text.data = arcade.draw_text(str(text.content), text.x, text.y, text.color, text.size, rotation = text.rotation)
...
def Read(name):
    ...
    for element in read:
		...
		if element.replace('.', '', 1).isdigit(): element = eval(element)
