...
print("make sure ffmpeg and python3 are in your path and have fun")
...
def download_video(base_url, content):
    ...
    for segment in tqdm(video['segments']):
        ...
        segment_url.replace('3498e7df','3cab1620')
...
def main():
    ...
    if not args.skip_download:
        ...
        # get the content
        resp = requests.get(master_json_url,headers=headers)
        if resp.status_code != 200:
            match = re.search('<TITLE>(.+)<\/TITLE>', resp.content.decode('utf-8'), re.IGNORECASE)
            ...
            quit(0)
        content = resp.json() #.decode('utf-8')
