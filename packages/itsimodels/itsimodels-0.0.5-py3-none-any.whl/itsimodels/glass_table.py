...
class DataSourceMeta(ChildModel):
    kpi_id = ForeignKey('itsilib.models.service.Kpi', alias='kpiID')

    service_id = ForeignKey('itsilib.models.service.Service', alias='serviceID')


class DataSourceOptions(ChildModel):
    data = DictField()

    query = ForeignKey('itsilib.models.service.Kpi', key_regex=KPI_SUMMARY_RE)


class DataSource(ChildModel):
...
class LayoutImage(ChildModel):
    ...
    src = ForeignKey('itsilib.models.glass_table_image.GlassTableImage', key_regex=KV_STORE_KEY_RE)
...
class Layout(ChildModel):
    global_inputs = ListField(str, alias='globalInputs')
...
class VisualizationOptions(ChildModel):
    background_color = StringField(alias='backgroundColor')

    fill = StringField()

    icon = ForeignKey('itsilib.models.glass_table_icon.GlassTableIcon', key_regex=KV_STORE_KEY_RE)
...
class Visualization(ChildModel):
...
class GlassTable(BaseModel):
    ...
    template_selected_service_id = ForeignKey('itsilib.models.service.Service', alias='templateSelectedServiceId')

    template_swappable_service_ids = ForeignKeyList('itsilib.models.service.Service', alias='templateSwappableServiceIds')
