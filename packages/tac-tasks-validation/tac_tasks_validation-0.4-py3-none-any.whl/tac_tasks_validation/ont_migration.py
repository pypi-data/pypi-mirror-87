...
from fibrenest_db_models.postgres import ONTMIGRATION
from sqlalchemy import or_
from tac_tasks_validation.base import ValidateOldOnt, ValidateNewOnt
from sqlalchemy.orm import Session
from concurrent.futures import ThreadPoolExecutor
from tac_tasks_validation.exceptions import ActiveSessionError, ValidationError
...
class ONTMigrationValidation(ValidateOldOnt, ValidateNewOnt):
    ...
    def ont_not_already_for_migration(self):
        """ Checks ONT isn't already scheduled for a migration """

        query: List[ONTMIGRATION] = self.new_db_session.query(ONTMIGRATION).filter(
            ONTMIGRATION.sn == self.ont_sn,
            or_(ONTMIGRATION.migration_status == 'pending', ONTMIGRATION.migration_status == 'started')
        ).all()
        if query:
            raise ValidationError(
                f'ONT migration for ONT already scheduled. JOB ID are/is: {[q.id for q in query]}'
            )
        return
