...
__context_options = [
    ...
    click.option(
        "--skip_auth", type=bool, default=False, help="should skip auth or not",
    ),
    click.option(
        "--engine",
    ...
    click.option(
        "--package_uri", type=str, default="", help="pipeline package uri",
    ),
    click.option(
...
def pass_bmlx_context(f):
    ...
    def new_func(
        ...
        **kwargs
        ...
        # 通过env决定package/artifacts这些的存储路径，默认在一个bucket下
        if env == "prod":
            CephFileSystem.specify_storage("bmlx-pipeline")
            pipeline_storage_base = (
                pipeline_storage_base
                or "http://fs-ceph-hk.bigo.sg/bmlx-pipeline"
            )
        else:
            CephFileSystem.specify_storage("mlpipeline-test")
            pipeline_storage_base = (
                pipeline_storage_base
                or "http://fs-ceph-hk.bigo.sg/mlpipeline-test"
            )
