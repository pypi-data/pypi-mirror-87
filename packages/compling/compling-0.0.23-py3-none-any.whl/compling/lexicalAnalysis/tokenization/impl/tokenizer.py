...
#from compling.descriptor import paramtypecheck, paramtypechecktarget
...
class Tokenizer(tokenization_abc.Tokenization):
    ...
    def __init__(self, domain_stopwords: Iterable[str] = None) -> None:
        """
        **\_\_init\_\_**: Creates a new Tokenizer object.
        ...
        if domain_stopwords is not None and not all([isinstance(s, str) for s in domain_stopwords]):
            raise TypeError('domain_stopwords must be Iterable[str], not {}'.format(type(domain_stopwords).__name__))
    ...
    #@paramtypechecktarget(params=['corpus_threshold', 'doc_threshold', 'len_gram', 'replace'])
    def ngrams2tokens(self,
        ...
        if isinstance(n, abciterable):
            ...
            raise TypeError('n must be typing.Union[int, Iterable[int], not {}'.format(type(n).__name__))

        for k, p in {'pos': pos, 'include': include}.items():
            if p is not None and not all([isinstance(i, str) for i in p]):
                raise TypeError('{} must be typing.Iterable[str], not {}'.format(k, type(p).__name__))
    ...
    #@paramtypechecktarget(params=['doc_id', 'token_id', 'sent_id', 'para_id', 'para_size', 'index_doc',
    #                              'index_sent', 'index_para'])
    def run(self,
