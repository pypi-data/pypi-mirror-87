...
class GitHub:
    ...
    def __post_init__(self):
        ...
        with self.lock:
            if self.log:
                self.log.debug(inspect.currentframe().f_code.co_name, f'{self.lock.locked()=}')
            self.init_clone()
    ...
    def add(self, force: Bool = False, write: Bool = True):
        ...
        for file in self.repo.untracked_files:
            ...
            rv.append(added[0][3])
        if self.log:
            self.log.debug(inspect.currentframe().f_code.co_name, f'{self.lock.locked()=}', rv)
        return rv
    ...
    def commit_cmd(self, message: Text = None) -> None:
        ...
        if self.log:
            self.log.debug(inspect.currentframe().f_code.co_name, f'{self.lock.locked()=}', commit)
        return commit
    ...
    def commit_cli(self, message, ctx, extra_args=None):
        ...
        try:
            ...
            if self.log:
                self.log.debug(inspect.currentframe().f_code.co_name, f'{self.lock.locked()=}', err_msg)
            raise exc
        finally:
            os.unlink(f.name)
    ...
    def init_clone(self):
        ...
        try:
            ...
            if self.log:
                self.log.error(inspect.currentframe().f_code.co_name, f'{self.lock.locked()=}', exception)
    ...
    def push(self, remote: Text = 'origin'):
        ...
        if self.log:
            self.log.debug(inspect.currentframe().f_code.co_name, f'{self.lock.locked()=}', f'{rv[0].summary=}',
                           f'{rv[0].flags=}')
