...
class Browser:
    ...
    def resize(self, width, height):
        windll.user32.SetWindowPos(
            self.inner_hwnd, 0, 0, 0, width - 16, height - 38, 0x0002 | 0x0004 | 0x0010
        )
...
def init(window):
    ...
    if not _initialized:
        ...
        all_command_line_switches = dict(
            default_command_line_switches, **command_line_switches
        )
        cef.Initialize(
            settings=all_settings, commandLineSwitches=all_command_line_switches
        )
