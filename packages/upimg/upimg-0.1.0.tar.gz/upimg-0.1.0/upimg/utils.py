...
if SYSTEM == 'Windows':
    from sys_platform.windows import get_clipboard_file_paths, notify

    CTRL_KEY = Key.ctrl

if SYSTEM == 'Darwin':
    from sys_platform.mac import get_clipboard_file_paths, notify
...
class ClipbordFile:
    ...
    def file_objects(self):
        """
        获取图片优先级： 剪贴板图片 > 已复制到剪贴板的文件列表 > 已选择的文件
        """
        ...
        file_paths = self.clipboard_file_paths or clipboard_copy() or self.clipboard_file_paths
        if file_paths:
...
class ListenerOperation:
    ...
    def __init__(self):
        keyboard_listener = keyboard.Listener(on_release=self.listener_operation)
        mouse_listener = mouse.Listener(on_click=self.listener_operation)
        keyboard_listener.start()
        mouse_listener.start()

    def listener_operation(self, value, *args):
        if self.HAS_OPERATED:
            return False
        if value:
            self.HAS_OPERATED = True
            return False


def clipboard_copy():
    """模拟 ctrl + c 复制已选择的文件到剪贴板"""
    keyboard = Controller()
