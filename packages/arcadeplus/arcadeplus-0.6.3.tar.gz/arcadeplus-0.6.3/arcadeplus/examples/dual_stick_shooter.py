...
class MyGame(arcadeplus.View):
    ...
    def on_show(self):
        ...
        if joys:
            ...
            arcadeplus.window_commands.schedule(self.debug_joy_state, 0.1)
        if not self.joy:
            print("No joystick present, using keyboard controls")
        arcadeplus.window_commands.schedule(self.spawn_enemy, ENEMY_SPAWN_INTERVAL)
    ...
    def on_update(self, delta_time):
        ...
        ship_death_hit_list = arcadeplus.check_for_collision_with_list(self.player, self.enemy_list)
        if len(ship_death_hit_list) > 0:
            self.game_over = True
        for bullet in self.bullet_list:
            bullet_killed = False
            enemy_shot_list = arcadeplus.check_for_collision_with_list(bullet, self.enemy_list)
    ...
    def on_key_press(self, key, modifiers):
        ...
        elif key == arcadeplus.key.A:
            self.player.change_x = -MOVEMENT_SPEED
        elif key == arcadeplus.key.S:
            self.player.change_y = -MOVEMENT_SPEED
        elif key == arcadeplus.key.D:
            self.player.change_x = MOVEMENT_SPEED
    ...
    def on_draw(self):
        ...
        # Game over message
        if self.game_over:
            arcadeplus.draw_text("Game Over", SCREEN_WIDTH/2, SCREEN_HEIGHT/2, arcadeplus.color.WHITE, 100, width=SCREEN_WIDTH,
...
class JoyConfigView(arcadeplus.View):
    ...
    def on_draw(self):
        arcadeplus.start_render()
        arcadeplus.draw_text("Configure your joystick", self.width/2, self.height/2+100,
                         arcadeplus.color.BLACK, font_size=32, anchor_x="center")
        arcadeplus.draw_text(self.msg, self.width/2, self.height/2,
                         arcadeplus.color.BLACK, font_size=24, anchor_x="center")
