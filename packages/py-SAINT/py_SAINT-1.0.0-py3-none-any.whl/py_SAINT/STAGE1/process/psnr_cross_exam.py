...
output = []
threshold = 0
path_saint = '/CT_DSI/experiment/3d_refine_4channel_lung/results/raw'
...
for file in test_files:
   if spacing_info[file][2] >= threshold and spacing_info[file][2] <= upper:
        ...
        if factor == 6:
            data = pickle.load(open(os.path.join(path_msr, file+'_cor_sr.pt'),'rb'))
            print(data.shape)
            msr_cor = np.transpose(data[62:446,64:448], (1,0,2)).round().astype(float)/4000
        else:
            msr_cor = np.transpose(pickle.load(open(os.path.join(path_msr, file+'_cor_sr.pt'),'rb'))[128:384,128:384], (1,0,2)).round().astype(float)/4000

        # meta_cor = np.transpose(pickle.load(open(os.path.join(path_meta, file+'_cor_sr.pt'),'rb'))[128:384,128:384], (1,0,2)).round().astype(float)/4000
        saint_cor = np.transpose(pickle.load(open(os.path.join(path_saint, file + '_cor_sr.pt'), 'rb'))[128:384,128:384],
        ...
        # else:
        hr = hr['image'][128:384,128:384].astype(float)/4000
        ...
        total_slice.append(slice_num)
        print(hr.shape, saint_cor.shape, msr_cor.shape)#, meta_cor.shape)
        psnr_saint_cor = compare_psnr(hr, saint_cor)
        psnr_msr_cor = compare_psnr(hr, msr_cor)
        # psnr_meta_cor = compare_psnr(hr, meta_cor)
        print(file, 'thickness: ',  spacing[2], ' saint: ', np.around(psnr_saint_cor,2), ' msr: ', np.around(psnr_msr_cor,2))#' meta: ', np.around(psnr_meta_cor,2)
