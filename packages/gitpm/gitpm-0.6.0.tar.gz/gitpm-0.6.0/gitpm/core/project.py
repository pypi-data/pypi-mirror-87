...
class Project:
    ...
    def list(
        ...
        if id_required:
            projects = [p for p in projects if "-" not in p.getId()]
    ...
    def listSelectors(path=os.getcwd()):
        return [(p.path, p.getName(), p.getId()) for p in Project.list(path)]

    @staticmethod
    def listNonUniqueIds(path=os.getcwd()):
        ids = [s[2] for s in Project.listSelectors(path)]
        return list(set([i for i in ids if ids.count(i) > 1 and "-" not in i]))

    @staticmethod
    def listNonUniqueNames(path=os.getcwd()):
        names = [s[1] for s in Project.listSelectors(path)]
        return list(set([i for i in names if names.count(i) > 1 and "-" not in i]))
    ...
    def setName(self, name, rename_dir=True):
    ...
    def listCommits(self, branch=None):
        ...
        return self.execute(["git", "rev-list", branch, "--count"])
