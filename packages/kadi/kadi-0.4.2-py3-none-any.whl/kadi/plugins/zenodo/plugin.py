...
from kadi.lib.constants import ONE_GB
...
def kadi_publish_record(record, provider, client, token, task):
    ...
    # Zenodo requires at least 3 characters for the description.
    description = (
        record.description if len(record.description) >= 3 else "*No description.*"
    )
    metadata = {
        "metadata": {
            "upload_type": "dataset",
