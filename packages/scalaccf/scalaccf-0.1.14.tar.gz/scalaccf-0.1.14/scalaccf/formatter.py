...
class Formatter:
    ...
    def snake_to_lower_camel_case(naming):
        ...
        return components[0].lower() + ''.join(x[0].capitalize() + x[1:] for x in components[1:])
    ...
    def __fix_namings(self):
        ...
        while i < len(self.__tokens):
            ...
            if token.get_value() == ':':
                ...
                if self.__tokens[i].get_token_type() == TokenType.IDENTIFIER:
                    if Formatter.__is_not_upper_camel_case(self.__tokens[i].get_value()):
                        self.__fix_similar(self.__tokens[i].get_primary_value(),
            ...
            # lower case camel case for annotation
                    ...
                    token.get_value()[1:]):
                token.set_fixed_value('@' + Formatter.snake_to_lower_camel_case(token.get_value()[1:]))
                i += 1
            ...
            # upper case camel case for class, trait & object
                ...
                if Formatter.__is_not_lower_camel_case(self.__tokens[i].get_value()):
                    self.__fix_similar(self.__tokens[i].get_primary_value(),
                                       Formatter.snake_to_lower_camel_case(self.__tokens[i].get_value()))
                i += 1
            elif token.get_token_type() == TokenType.IDENTIFIER and token.get_fixed_value() is None:
                pos = i + 1
                while self.__tokens[pos].get_value() == ' ':
                    pos += 1
                if self.__tokens[pos].get_value() not in (
                        '.', '[', 'extends', '(', '{') and Formatter.__is_not_lower_camel_case(
                    token.get_value()):

                    self.__fix_similar(token.get_primary_value(),
                                       Formatter.snake_to_lower_camel_case(token.get_value()))
                i += 1
            else:
                i += 1
        ...
        for token in self.__tokens:
            if token.get_value() == '[':
                ...
                brace_count -= 1
                    ...
                    token.get_value()):
                self.__fix_similar(token.get_primary_value(), Formatter.snake_to_upper_camel_case(token.get_value()))
    ...
    def __fix_class_trait_object(self, pos):
        ...
        if self.__tokens[pos].get_token_type() == TokenType.IDENTIFIER and Formatter.__is_not_upper_camel_case(
            ...
            self.__fix_similar(self.__tokens[pos].get_primary_value(), fixed_value)
            pos += 1
        return pos
    ...
    def __fix_similar(self, token_before, token_after):
        i = 0
        while i < len(self.__tokens):
            token = self.__tokens[i]
            if token.get_primary_value() == token_before:
                token.set_fixed_value(token_after)
            i += 1
