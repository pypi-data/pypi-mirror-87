...
class TrackerStore:
    ...
    def get_channel_timeout(self, channel):
        """
        Use 20 mins TTL for the voice channels and 2 hours for everything else.
        :param channel:
        :return: timeout in seconds
        """
        timeouts = {"OMNI_VOICE_00001": 1200}
...
class DynamoTrackerStore(TrackerStore):
    ...
    def save(self, tracker, stream_only=False):
        """Saves the current conversation state"""
        if self.event_broker:
            self.stream_events(tracker)
...
class RedisClusterTrackerStore(TrackerStore):
    ...
    def __init__(
        ...
        try:
            logger.info(f"Redis cluster connection info - {host}:{port}. Max connections: {int(max_connections)}")
            self.red_cluster = RedisCluster(
                host=host, port=port, max_connections=int(max_connections)
            )
    ...
    def save(self, tracker: DialogueStateTracker, timeout=None, stream_only=False):
        ...
        serialised_tracker = self.serialise_tracker(tracker)
        start = time.perf_counter()
        self.red_cluster.set(tracker.sender_id, serialised_tracker, ex=timeout * 2)
    ...
    def retrieve(self, sender_id):
        start = time.perf_counter()
        stored = self.red_cluster.get(sender_id)
        duration = time.perf_counter() - start
    ...
    def delete(self, sender_id):
        self.red_cluster.delete(sender_id)

    def ping(self):
        return self.red_cluster.ping()

    def test_conn(self):
        return self.red_cluster.set("ping", "")
        ...
        while True:
            try:
                ...
                if dialect == "postgresql":
                    ...
                    self.engine = create_engine(f'mssql+pymssql://{username}:{password}@{host}/{db}',
                                                pool_size=int(os.environ.get("SQL_POOL_SIZE", "50")),
                                                max_overflow=int(os.environ.get("SQL_MAX_OVERFLOW", "100")),
                                                echo=echo_query)
                else:
...
class FailSafeTrackerStore(TrackerStore):
    ...
    def save(self, tracker: DialogueStateTracker, stream_only=False) -> None:
        try:
