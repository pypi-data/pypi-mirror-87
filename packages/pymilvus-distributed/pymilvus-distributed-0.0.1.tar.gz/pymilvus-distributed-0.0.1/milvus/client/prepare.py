...
class Prepare:
            ...
            for field in fv:
                ...
                if isinstance(type_params, dict):
                    for tk, tv in type_params.items():
                        if tk == "metric_type":
                            if not tv or not isinstance(tv, MetricType) or tv == MetricType.INVALID:
                                raise ParamError("metric_type must be of Milvus.MetricType!")
                        if tk == "dim":
                ...
                if isinstance(index_params, dict):
                    for index_param in index_params:
                        ...
                        kv_pairs = [common_types.KeyValuePair(key=str(ik), value=str(iv))
                                    for ik, iv in index_param.items()]
                        field_schema.index_params.extend(kv_pairs)
    ...
    @classmethod
    def bulk_insert_param(cls, collection_name, entities, partition_tag, ids=None, params=None, fields_info=None, **kwargs):
        ...
        for i in range(fields_len):
            fields_type.append(entities[i]["type"])
