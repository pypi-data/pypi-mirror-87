...
def __make_requests(baseUrl, inputs, source, target, authentication, outputPath):
    requests = []
    for sql in inputs:
...
def __handle_responses(baseUrl, responses, authentication, target, outputPath):
    ...
    while responsesToProcess:
        ...
        for data in responseData:
            ...
            if data["translateCompilation"]["error"] is not None:
                ...
                errors.append({"input": sqlInput.path, "error": data["translateCompilation"]["error"]})
                ...
                for err in data['fatals'] + data['errors']:
                    ...
                    sys.stderr.write("ERROR in file: %s\n" % sqlInput.path)
                    sys.stderr.write("%s\n" % msg)
                    errors.append({"input": sqlInput.path, "error": msg})
            if data["targetStmt"]:
...
def runner(runnerArgs):
    ...
    try:
        ...
        if args.db_url or args.db_schema or args.db_table:
            if args.source != 'oracle':
                raise Exception("Input is database but unsupported db type specified %s as source." % (args.db_type))
            elif not (args.db_url and args.db_schema and args.db_table):
                raise Exception("Input is database but at least one of db-url, db-schema or db-table not specified.")
            ...
            schema_crawler_oracle.oracle_collect_sql(args.db_url, args.db_schema, args.db_table, inputPath) # inputPath is this functions outputPath
        ...
        responses = __make_requests(baseUrl, inputs, args.source, args.target, args.auth, outputPath)
        endRequests = time.time()
        startResponses = time.time()
        errors = __handle_responses(baseUrl, responses, args.auth, args.target, outputPath)
