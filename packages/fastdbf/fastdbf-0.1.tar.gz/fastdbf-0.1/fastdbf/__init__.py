    ...
    with open(file_name, "rb") as f:
        ...
        if HAVE_C_EXT:
            ...
            dtypes = {
                ...
                'D': lambda len: '<M8[D]', # converted from string
                'N': lambda len: '=f4', # atof
                'L': lambda len: '?', # stored as 1-byte, converted from string
                'I': lambda len: '=i4' # memcpy
            }
        else:
            def _parse_c(data):
        ...
        for field in fields:
            ...
            if HAVE_C_EXT:
                ...
                arr = np.require(np.empty(numrecords, dtype=dtypes[field_type](field_length)), requirements='CAW') # C_CONTIGUOUS, ALIGNED, WRITEABLE
