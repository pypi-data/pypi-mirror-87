        ...
        for key, value in class_items:
            if key not in columns:
                if value.__class__.__name__ in _globals:
                    ...
                    cursor = self.__conn.cursor()
                    cursor.execute(f"CREATE TABLE IF NOT EXISTS {table_name}_to_{value[0].__class__.__name__.lower()} ({table_name}_id bigint, {value[0].__class__.__name__.lower()}_id bigint)")
                    self.__conn.commit()
                else:
                    columns_to_add.append((key, Py2SQL.__get_sql_type(value)))
