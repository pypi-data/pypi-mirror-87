...
def validate_classification(y_test: np.ndarray,
    ...
    for metric in metrics:
        if metric not in constants.CLASSIFICATION_SET:
            logger.error("Invalid metric while scoring classification results")
            message = "Metric {} not a valid classification metric".format(metric)
            raise ValidationException._with_error(AzureMLError.create(
                AutoMLInternal, target="metric", reference_code=ReferenceCodes._METRIC_INVALID_CLASSIFICATION_METRIC,
...
def validate_regression(y_test: np.ndarray,
    ...
    for metric in metrics:
        if metric not in constants.REGRESSION_SET:
            logger.error("Invalid metric while scoring regression results")
            message = "Metric {} not a valid regression metric".format(metric)
            raise ValidationException._with_error(AzureMLError.create(
                AutoMLInternal, target="metric", reference_code=ReferenceCodes._METRIC_INVALID_REGRESSION_METRIC,
                error_details=message)
            )
...
def validate_forecasting(y_test: np.ndarray,
    ...
    for metric in metrics:
        if metric not in constants.FORECASTING_SET:
            logger.error("Invalid metric while scoring forecasting results")
            message = "Metric {} not a valid forecasting metric".format(metric)
            raise ValidationException._with_error(AzureMLError.create(
                AutoMLInternal, target="metric", reference_code=ReferenceCodes._METRIC_INVALID_FORECASTING_METRIC,
                error_details=message)
            )
