...
def verify_user_emails(modeladmin, request, queryset):
	...
	for user in queryset:
		if user.email and user.email not in already_verified:
			verified_email_address = EmailAddress()
			verified_email_address.user = user
			verified_email_address.email = user.email
			verified_email_address.verified = True
			verified_email_address.primary = True
