...
`client = AsrClient("ip", "port", enable_flush_data=False, service_name="", app_key="", app_secret="")`初始化时可选传入的参数列表如下：
...
| app_key | string | 无 | access key |
| app_secret | string | 无 | secret_key |
| service_name| string | 无 | 服务名称 例如asrbj |
