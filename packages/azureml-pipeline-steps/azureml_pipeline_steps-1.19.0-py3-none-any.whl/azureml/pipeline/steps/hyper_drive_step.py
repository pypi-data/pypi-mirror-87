...
class HyperDriveStep(PipelineStep):
    ...
    :param estimator_entry_script_arguments: [Required] A list of command-line arguments for the
    ...
    :param metrics_output: Optional value specifying the location to store HyperDrive run metrics as a
                        JSON file.
    :type metrics_output: azureml.pipeline.core.builder.PipelineData
                        or azureml.data.data_reference.DataReference
                        or azureml.pipeline.core.graph.OutputPortBinding
    ...
    def __init__(self, name, hyperdrive_config,
        ...
        if not isinstance(estimator_entry_script_arguments, list):
        ...
        if run_config.arguments:
            raise ValueError('arguments in run_config should not be provided. Please use arguments in the step')
        if run_config.data_references:
            raise ValueError('data_references in run_config should not be provided. Please use inputs in the step')
        ...
        self._params[HyperDriveStep._primary_metric_name] = hyperdrive_config._primary_metric_config['name'].lower()
