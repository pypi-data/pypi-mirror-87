...
class Model(nn.Module):
    ...
    def forward(self, batch_data, eval=False, use_hidden=False):
        ...
        if use_hidden and self.encoder_hidden is not None:
            ...
            while True:
                ...
                for seq in sequences:
                    if tok.tok_sep(self.tokenizer) not in seq[0]:
                        ...
                        # check input exceed
                        if len(feature_dict['target_mask']) > self.maxlen:
                            exceed = True
                            all_candidates.append(seq)
                            continue
                        for k, v in feature_dict.items():
                            feature_dict[k] = [v]
                        predictions = self.forward(feature_dict, eval=True, use_hidden=True)
