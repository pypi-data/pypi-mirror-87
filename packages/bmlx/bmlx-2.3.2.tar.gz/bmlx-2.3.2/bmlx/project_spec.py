...
class Project(object):
    ...
    def _load(self, entry: Optional[Text] = None):
        ...
        self.artifact_storage_base = (
            self._data["settings"]["pipeline"]["artifact_storage_base"].as_str()
            if self._data["settings"]["pipeline"][
                "artifact_storage_base"
            ].exists()
            else ARTIFACT_STORAGE_BASE_PATH
        )

        user_assigned_buckets = {}
        #bmlx.yml添加bucket_name和aksk信息
        if self._data["settings"]["pipeline"]["s3_storage"].exists():
            for name in self._data["settings"]["pipeline"]["s3_storage"]:
                user_assigned_buckets[name] = {
                    "endpoint": self._data["settings"]["pipeline"]["s3_storage"][
                        name]["endpoint"].as_str(),
                    "access_key": self._data["settings"]["pipeline"]["s3_storage"][
                        name]["access_key"].as_str(),
                    "secret_key": self._data["settings"]["pipeline"]["s3_storage"][
                        name]["secret_key"].as_str(),
                }
        CephFileSystem.update_storage_map(user_assigned_buckets)
    ...
    def configurables(self):
        ...
        for file_name, patterns in params_dict.items():
            ...
            for pattern in patterns:
                result.update(resolve_patterns(file_name, pattern))
        return result
