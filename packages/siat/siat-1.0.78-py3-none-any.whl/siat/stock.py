...
def comp_2securities_1measure(df1,df2,measure,twinx=False):
    ...
    if not (measure in dfcols):
        print("... 错误#1(comp_2securities_1measure)，指标类别仅支持选项: ",dfcols)
        return        
...
if __name__ =="__main__":
    ...
    fromdate='2020-1-1'
    todate='2020-3-16'
    compare(tickers,measures,fromdate,todate)            

    tickers2=['000002.SZ','600266.SS']
    measures2=['Close','Adj Close']
    compare(tickers2,measures2,fromdate,todate)

    tickers3=['000002.SZ','600266.SS']
    measures3='Close'
    compare(tickers3,measures3,fromdate,todate)    

    tickers4=['000002.SZ','600606.SS','600266.SS']
    measures4=['Close','Adj Close','Daily Return']
    compare(tickers4,measures4,fromdate,todate)      
...
def candlestick_demo(stkcd,fromdate,todate,colorup='red',colordown='black',width=0.7):
    ...
    titletxt="证券价格走势K线演示图："+codetranslate(str(stkcd))
...
def stock_info(symbol):
    ...
    try:
        ...
        print("#Error(stock_info): failed to get asset profile of",symbol)
        return None
...
def print_companyOfficers(sub_info):
    ...
    for i in alist:
        print(' '*4,i[key1])
        print(' '*8,i[key2],'\b,',i[key4],'\b岁 (生于'+str(i[key3])+')')
        print(' '*8,'总薪酬',currency+str(format(i[key6],',')),'@'+str(i[key7]))
