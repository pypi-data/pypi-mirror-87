...
class Base(object):
    ...
    def delete(self, table, condition, or_cond, is_commit=True, show_sql=False):
    ...
    def query(self, sql, show_sql=False,is_commit=False):
        if self._engine == sqlite:
            sql = sql.replace('binary','')
    ...
    def query_one(self, sql,show_sql=True,commit=True):
        if self._engine != sqlite:
            try:
                self.db.ping(reconnect=True)
            except Exception as e:
