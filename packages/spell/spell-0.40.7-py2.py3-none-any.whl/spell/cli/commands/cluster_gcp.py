...
GKE_DEFAULT_SERVINGGROUP_TYPE = "n1-standard-2"  # Instance type of default GKE serving groups
...
def create_gcp(ctx, name):
    ...
    network_name, subnet_name = get_vpc(compute_service, name, project_id, region)
...
def gke_init(
    ...
    zone = get_zone_from_region(compute_service, gcp_project_name, region)
    if not use_existing:
        create_gke_cluster(
            gke_cluster_name,
            gcp_project_name,
        ...
        if not click.confirm("Are you sure you want to use {}?".format(gke_cluster_name)):
            raise ExitException(
    ...
    return gke_configure_k8s(
        ctx, gcp_project_name, gke_cluster_name, zone, service_account, cluster
    )


def gke_update(
    ctx, project, credentials, cluster, auth_api_url, gke_cluster_name,
):
...
def gke_configure_k8s(ctx, gcp_project_name, gke_cluster_name, zone, service_account, cluster):
    ...
    cluster_utils.echo_delimiter()
    click.echo("Setting up Ambassador Edge Stack...")
    try:
        ...
        click.echo("Setting up Ambassador deployment...")
        with tempfile.NamedTemporaryFile(suffix=".yaml", mode="w+") as f:
            ambassador_yaml = generate_main_ambassador_yaml("GCP", False)
        ...
        with tempfile.NamedTemporaryFile(suffix=".yaml", mode="w+") as f:
            ambassador_yaml = generate_ambassador_host_yaml(
                cluster["serving_cluster_domain"], False
            )
            f.write(ambassador_yaml)
            f.flush()
            subprocess.check_call(("kubectl", "apply", "--filename", f.name))
    ...
    cluster_utils.add_prometheus_stack(ctx.obj["stack"])
...
def get_vpc(compute_service, cluster_name, project, region):
    cluster_utils.echo_delimiter()
    ...
    # Add firewall rules
    click.echo("Creating firewall rule to allow ingress on ports {}...".format(INGRESS_PORTS))
    request = compute_service.firewalls().insert(
        ...
        body={
            ...
            "allowed": [{"IPProtocol": "TCP", "ports": [str(port) for port in INGRESS_PORTS]}],
        },
    )
    response = request.execute()
    global_progress_bar(project, compute_service, response)
...
def gke_scale_nodepool(credentials, serving_group, min_nodes, max_nodes):
    from googleapiclient import discovery
    ...
    config = json.loads(serving_group.config)
    name_path = "{}/nodePools/{}".format(config["parent"], serving_group.name)

    if serving_group.min_nodes is None:
        serving_group.min_nodes = 0
    if serving_group.max_nodes is None:
        serving_group.max_nodes = 2

    new_min_nodes = min_nodes if min_nodes is not None else serving_group.min_nodes
    new_max_nodes = max_nodes if max_nodes is not None else serving_group.max_nodes
    autoscaling_cfg = {
        "autoscaling": {
            "enabled": True,
...
def gke_delete_nodepool(credentials, serving_group):
    from googleapiclient import discovery
    from googleapiclient.errors import HttpError
...
def gke_add_nodepool(serving_group_name, config_contents, gcp_service_account, gcp_creds):
    from googleapiclient import discovery
    from googleapiclient.errors import HttpError
    ...
    config["nodePool"]["config"]["labels"]["spell_serving_group"] = serving_group_name
...
def delete_gcp_cluster(ctx, cluster):
    ...
    if cluster.get("serving_cluster_name"):
        gke_delete_cluster(project_id, cluster["serving_cluster_name"])
...
def gke_delete_cluster(project_id, serving_cluster_name):
    zone, gke_name = extract_gke_cluster_info(project_id, serving_cluster_name)
