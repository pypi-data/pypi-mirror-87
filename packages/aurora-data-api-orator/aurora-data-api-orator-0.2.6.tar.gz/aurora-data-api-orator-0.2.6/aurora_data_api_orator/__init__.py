"""
aurora-data-api-orator - A Python DB-API 2.0 client for the AWS Aurora Serverless Data API
"""
...
class AuroraDataAPIClient:
    def __init__(self, database=None, aurora_cluster_arn=None, secret_arn=None, client=None, charset=None):
        self._client = client
        if client is None:
...
class AuroraDataAPICursor:
    ...
    _data_api_type_hint_map = {
        ...
        pendulum.pendulum.Pendulum: "TIMESTAMP",
        Decimal: "DECIMAL"
    }

    def __init__(self, client=None, database=None, aurora_cluster_arn=None, secret_arn=None, transaction_id=None):
        self.arraysize = 1000
    ...
    def execute(self, operation, parameters=None):
        if parameters:
            ...
            for i in range(len(parameters)):
                param_dict[str(i)] = parameters[i]
                operation = operation.replace('?', f':{i}', 1)
...
class Row(tuple):
    def items(self):
        return ((self._columns[n], self.__getitem__(n)) for n in range(len(self)))
...
def connect(aurora_cluster_arn=None, secret_arn=None, client=None, database=None, host=None, port=None,
            username=None, password=None, charset=None):
    return AuroraDataAPIClient(database=database, aurora_cluster_arn=aurora_cluster_arn,
                               secret_arn=secret_arn, client=client, charset=charset)
