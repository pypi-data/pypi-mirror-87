...
class RScriptStep(_PythonScriptStepBase):
    ...
    _base_image_for_gpu = 'mcr.microsoft.com/azureml/base-gpu:openmpi3.1.2-cuda10.0-cudnn7-ubuntu16.04'
    _base_image_for_cpu = 'mcr.microsoft.com/azureml/base:openmpi3.1.2-ubuntu16.04'
    ...
    def __init__(self, script_name, name=None, arguments=None, compute_target=None, runconfig=None,
        ...
        if runconfig is None:
            raise ValueError("RunConfiguration is missing. It is required with RSection.")

        if runconfig.framework.lower() != 'r':
            raise ValueError("RunConfiguration.framework should be r.")

        if runconfig.environment.r is None:
            raise ValueError("RSection is missing from RunConfiguration.Environment.")

        if not runconfig.environment.docker.enabled:
            raise ValueError("DockerSection.enabled needs to be set True in RunConfiguration.Environment.")

        if runconfig.environment.docker.base_image == DEFAULT_CPU_IMAGE:
            if use_gpu:
                runconfig.environment.docker.base_image = RScriptStep._base_image_for_gpu
            else:
                runconfig.environment.docker.base_image = RScriptStep._base_image_for_cpu
