from agilicus.output.json import output_json
import json
import sys
...
@click.option("--prefix_email_search", default="")
...
@click.option("--previous_email", default=None)
@click.option("--hide_members", type=bool, default=None)
@click.option(
    "--search_direction",
    default="forwards",
    type=click.Choice(["forwards", "backwards"]),
...
@click.option("--icon_url", default=None)
@click.option("--location", type=click.Choice(["hosted", "external"]), default=None)
@click.option("--service_account_id", default=None)
@click.pass_context
...
@click.option(
    "--authentication_document", type=click_extension.JSONFile("r"), default=None
)
@click.option("--scope", type=str, multiple=True)
@click.option("--client_id", default=None)
@click.option("--org_id", default=None)
@click.option("--user_id", default=None)
@click.option("--auth_issuer_url", default=None)
...
def list_secure_agents(ctx, **kwargs):
    ...
    table = apps.format_secure_agent_as_text(ids)
    print(table)
...
@click.option("--connection_uri", default=None)
@click.option("--org_id", default=None)
...
def list_authentication_documents(ctx, **kwargs):
    ...
    table = tokens.format_authentication_document_as_text(ids)
    print(table)
...
def add_authentication_document(ctx, outfile, **kwargs):
    ...
    if outfile:
        with open(outfile, "w", encoding="utf-8") as f:
            json.dump(result, f, ensure_ascii=False, default=str, indent=4)
    else:
        output_entry(ctx, result)
...
def list_service_accounts(ctx, **kwargs):
    ...
    table = users.format_service_account_as_text(ids)
    print(table)
...
def list_api_keys(ctx, **kwargs):
    ...
    table = tokens.format_api_keys_as_text(api_keys)
    print(table)
