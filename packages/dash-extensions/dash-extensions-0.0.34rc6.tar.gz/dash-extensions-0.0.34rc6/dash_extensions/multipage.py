...
from .enrich import Output, Input, Dash, PrefixIdTransform
...
class Page:
    def __init__(self, id, label, layout=None, callbacks=None, transforms=None, proxy=None):
        self.id = id
        self.label = label
        self._layout = layout
        ...
        self._proxy = proxy
        if proxy is None:
            transforms = [PrefixIdTransform(id)] if transforms is None else transforms
            self._proxy = Dash(transforms=transforms)
