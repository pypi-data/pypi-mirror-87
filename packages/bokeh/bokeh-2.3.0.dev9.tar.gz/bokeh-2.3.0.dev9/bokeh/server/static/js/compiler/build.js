...
async function build(base_dir, bokehjs_dir, base_setup) {
    ...
    const bokeh_ext = sys_1.read_json(bokeh_ext_json_path);
    ...
    const license = (() => {
        if (isPlainObject(bokeh_ext.license)) {
            if (isString(bokeh_ext.license.file)) {
                const license_path = path_1.join(base_dir, bokeh_ext.license.file);
                const text = sys_1.read(license_path);
                if (text != null)
                    return text;
                else
    ...
    print("All done.");
    return !compiler_1.is_failed(tsoutput) && status;
}
exports.build = build;
