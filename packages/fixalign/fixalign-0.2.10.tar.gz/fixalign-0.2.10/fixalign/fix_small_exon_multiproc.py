...
def parse_bed(bed_path):
    ...
    def parse_row(row):
        row["blockSizes"] = [np.int64(x) for x in row["blockSizes"].split(',') if len(x) > 0]
        row["blockStarts"] = [np.int64(x)
                              for x in row["blockStarts"].split(',') if len(x) > 0]
...
def run_multiprocess(
    ...
    proc_list[-1]["end_read_i"] = read_count  # reset the end
