...
class XdlReader(object):
    ...
    def _resolve_sample_files(
        ...
        if not sample_artifacts:
            return []
        if sample_artifacts[0].meta.uri.startwith("kafka"):
        ...
        def sample_file_in_slots(file_list, files_sample_rate):
            ...
            for timestr in timestr_2_file_lst.keys():
                res = random.sample(
                    ...
                    int(len(timestr_2_file_lst[timestr]) * files_sample_rate),
                )
                res_lst.extend(res)
            return res_lst
        ...
        if sampling_rate != 1.0:
            file_list = sample_file_in_slots(file_list)
