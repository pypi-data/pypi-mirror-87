...
def execute_sort(
    ...
    # update the step to save the variables needed to reconstruct the sort
    wsc.curr_step['sheet_index'] = sheet_index
    wsc.curr_step['column_header'] = column_header
    wsc.curr_step['sort_direction'] = sort_direction
...
def _execute_sort(
    ...
    try: 
        return df.sort_values(by=column_header, ascending=(sort_direction == ASCENDING), na_position='first')
...
def transpile_sort(
    ...
    """
    # sort the dataframe
    sort_line = f'{widget_state_container.df_names[sheet_index]} = {widget_state_container.df_names[sheet_index]}.sort_values(by=\'{column_header}\', ascending={sort_direction == ASCENDING}, na_position=\'first\')'
