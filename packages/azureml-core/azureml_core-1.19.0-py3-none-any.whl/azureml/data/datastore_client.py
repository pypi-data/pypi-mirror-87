...
from azureml._restclient.models.rest_client_enums import ServiceDataAccessAuthIdentity
...
from .constants import CONFLICT_MESSAGE
...
class _DatastoreClient:
    ...
    @staticmethod
    def _grant_msi_workspace_access(workspace, role, datastore_url, datastore_type):
        ...
        properties = {"roleDefinitionId": "/subscriptions/" + workspace.subscription_id +
                                          ...
                                          role,
                      "principalId": workspace_resource.identity_principal_id}
        ...
        if response.status_code not in [200, 201, 409]:
            error_message = ("Please contact an owner of the storage account to manually assign the "
                             "{} role to the Workspace Managed Identity."
                             "Trying to grant Workspace MSI access to the {} "
                             "resource failed with error status '{}' and error message '{}'").format(
                role, datastore_type, response.status_code,
    ...
    def _check_or_grant_access(subscription_id, resource_group, workspace, account_name, custom_warning,
        ...
        url = 'https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{' \
              ...
              'account_name}/blobServices/default/providers/Microsoft.Authorization/roleAssignments'

        if datastore_type is "Blob" or datastore_type is "AzureDataLakeGen2":
            url = 'https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{' \
                  'resource_group}/providers/Microsoft.Storage/storageAccounts/{' \
                  'account_name}/blobServices/default/providers/Microsoft.Authorization/roleAssignments'
        elif datastore_type is "AzureDataLake":
            url = 'https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{' \
                  'resource_group}/providers/Microsoft.DataLakeStore/Accounts/{account_name}/' \
                  'providers/Microsoft.Authorization/roleAssignments'
        elif datastore_type is "AzureSqlDatabase":
            url = 'https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{' \
        ...
        workspace_resource = _commands.get_workspace(workspace._auth, workspace.subscription_id,
                                                     workspace.resource_group, workspace.name)
        query_parameters = {}
        query_parameters['$filter'] = 'principalId%20eq%20%27' + workspace_resource.identity_principal_id + '%27'
        ...
        for role in missing_roles:
            _DatastoreClient._grant_msi_workspace_access(workspace, role, url, datastore_type)
    ...
    def _register_azure_storage(ws, datastore_name, storage_type, container_name, account_name,
        ...
        service_data_access_auth_identity = ServiceDataAccessAuthIdentity.workspace_system_assigned_identity \
            if grant_workspace_access else ServiceDataAccessAuthIdentity.none
    ...
    def _register_azure_sql_database(ws, datastore_name, server_name, database_name, tenant_id=None, client_id=None,
        ...
        # Although auth_type and service_data_access_auth_identity are not directly related,
        # but in the absense of one the other is sufficient to service the request.
        # But the absense of both is sure case of error.
        elif service_data_access_auth_identity == ServiceDataAccessAuthIdentity.none:
            raise ValueError("Please provide either a username and password or the credential of a service principal.")
