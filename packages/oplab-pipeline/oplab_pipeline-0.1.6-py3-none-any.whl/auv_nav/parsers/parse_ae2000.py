        ...
        if float(df["Height"][row_index]) < altitude_limit:
            if ftype == "oplab":
                ...
                if category == "depth":
                    ...
                    depth_std = (
                        depth * mission.depth.std_factor + mission.depth.std_offset
                    )
            ...
            if ftype == "acfr":
                if category == "velocity":
                    ...
                    # DVL convention is +ve aft to forward
                    xx_velocity = float(df["vx"][row_index])
                    # DVL convention is +ve port to starboard
                    yy_velocity = float(df["vy"][row_index])
                    # DVL convention is bottom to top +ve
                    zz_velocity = float(df["vz"][row_index])
