...
class CCF:
    ...
    def verify(self):
        ...
        for tokens in tokens_list:
            ...
            expected_path, found_path = CCF.get_fixed_path(file, formatter.find_package())
    ...
    def fix(self):
        ...
        i = 0
        for tokens in tokens_list:
            logging.basicConfig(filename=f'verification/{file.split(".")[0]}_fixing.log', level=logging.WARN)
            formatter = Formatter(tokens)
    ...
    def fix_path(expected_path, found_path, file):
        ...
        if len(expected_path) != len(found_path) and len(found_path) == 2:
            current_path = abspath(file).split('\\')[:-1]
        else:
            current_path = abspath(file).split('\\')[:-len(found_path)]

        os.chdir('\\'.join(current_path))
        path = current_path
        path.extend(expected_path[:-1])
        if not os.path.exists('\\'.join(expected_path[:-1])):
            os.makedirs('\\'.join(expected_path[:-1]))

        with open(os.path.join('\\'.join(path), expected_path[-1]), 'w+'):
            pass
        return os.path.join('\\'.join(path), expected_path[-1])

    @staticmethod
    def get_fixed_path(file, formatter):
        package = formatter.find_package()
        path = abspath(file).split('\\')
        file_name = path[-1]
