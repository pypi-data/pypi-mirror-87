...
class ParamDict(dict):
    ...
    The main part is a single value; the list part is a list containing additional information;
        they type is a single character: S for string type, F for float, L for length on the canvas (a float).
        ...
        if not isdeprec:
            if key not in self:
                ...
                if isinstance(data, list):
                    ...
                    # The input was not a list, and the parameter is defined as containing float values
                    if isinstance(data, str) and data.strip() in ['','-']:
                        ...
                        try:
                            ...
                            hlp.addlogwarn('Cannot convert parameter {} to number: {}'. format(key, data))
                else:
