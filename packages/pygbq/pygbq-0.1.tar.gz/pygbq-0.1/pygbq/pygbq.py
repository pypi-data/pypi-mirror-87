...
if SERVICE_KEY_PATH:
    CREDENTIALS, PROJECT_ID = load_credentials_from_file(SERVICE_KEY_PATH,
                                                         scopes=[
...
class MyDataError(MyError):
    ...
    def __init__(self, row, error_info):
        super(MyDataError, self).__init__(msg=f"Row {row} caused the following errors:\n{error_info}")
...
def update_table_using_temp(data, table_id, how, schema,
    ...
    def _gen_schema(schema_dict):
        ...
        for field in schema_dict:
            if field['type'] != 'RECORD':
                schema_list.append(bigquery.schema.SchemaField(field['name'],
                                                               field['type'],
                                                               mode=field['mode']))
            else:
                schema_list.append(bigquery.schema.SchemaField(field['name'],
...
@parametrized
def gbq(function, table_id: str = None, how: str = None, save_dir: str = None, after: str = None,
        test: str = None, service: bool = False):
    """
    ...
    if table_id:
        if not table_id.replace('_', '').replace('.', '').isalnum():
            raise MyNameError('Bad table name: only letters, numbers and underscores allowed')
        ...
        if num_dots == 0:
            ...
            _, dataset, table_name = table_id.split('.')  # don't need to store project as we use the default
        else:
        ...
        if dataset:
            ...
            raise MyNameError(f"Either provide full name of the table or set dataset with pygbq.set_dataset()")
        ...
        if isinstance(returned, DataFrame):
            ...
            if after:
                ...
                try:
                    ...
                    after_status = False
                    logging.exception(f"Your query '{after}' is incorrect")  # this way we get E but not MyError
