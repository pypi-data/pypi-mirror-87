...
async def make_browser(self):
    ...
    utlis.post('chrome/site', self.project.site)
    utlis.post('chrome/os', system)
    if system == "Linux":
        if self.project.site == 'amp':
            utlis.post('chrome/headless', options.Headless)
...
async def scan_plan_view(parent, platform, pause=True):
    """
    ...
    """
    for target_child in range(0, 500):
