...
from CodeConventionFixer.class_fix import ClassFix
from CodeConventionFixer.declaration import Declaration
from CodeConventionFixer.declaration_type import DeclarationType
from CodeConventionFixer.interface_fix import InterfaceFix
from CodeConventionFixer.naming_convention_fix import NamingConventionFix
from LexicalAnalysis.lexem_type import LexemType
...
class CodeConventionFixer:
    ...
    def verify_file_conventions(file_path):
        ...
        f = open(file_path + '_verification.log', 'w', encoding='utf-8')
    ...
    def internal_find_convention_violations_in_file(file_path):
        ...
        interfaces = []
        namespaces = CodeConventionFixer.find_namespace_declarations(lexems)
        if len(namespaces) == 0: #AssemblyInfo.cs and other special files, they have no need to import anything
            print(file_path)
            return
    ...
    def internal_fix_file_conventions(file_path):
        ...
        fixing_log = open(file_path + '_fixing.log', 'w', encoding='utf-8')
        fixing_log.write(file_path + ' FIXING LOG\n')

        filename = ntpath.basename(file_path)
        dir_path = os.path.dirname(os.path.realpath(file_path))
        filename_wo_ext = filename[0:(len(filename) - len(CodeConventionFixer.cs_file_extension))]
        if not CodeConventionFixer.is_pascal_case(filename_wo_ext):
            new_name = CodeConventionFixer.to_pascal_case(filename_wo_ext) + CodeConventionFixer.cs_file_extension
            slash = "\\" if platform.system() in ['Windows', 'windows', 'Win', 'win'] else "/"
            file_path = dir_path + slash + new_name
            os.rename(dir_path + slash + filename, file_path)
            fixing_log.write(CodeConventionFixer.get_error_msg(file_path, -1,
                                                      'Filename is not in pascal case: ' + filename + '. Fixed to: ' + \
                                                      new_name))
    ...
    def fix_directory_conventions(directory_path):
        ...
        old_name = os.path.basename(directory_path)
        if not CodeConventionFixer.is_pascal_case(old_name):
            new_name = CodeConventionFixer.to_pascal_case(old_name)
            slash = "\\" if platform.system() in ['Windows', 'windows', 'Win', 'win'] else "/"
            path = Path(directory_path)
            new_path = str(path.parent) + slash + new_name
        ...
        for root, dirs, files in os.walk(project_path):
            for dir in dirs:
                ...
                if not CodeConventionFixer.is_pascal_case(old_name):
                    new_name = CodeConventionFixer.to_pascal_case(old_name)
                    slash = "\\" if platform.system() in ['Windows', 'windows', 'Win', 'win'] else "/"
                    os.rename(root + slash + dir, root + slash + new_name)
    ...
    def fix_file_conventions_old(file_path):
        ...
        f = open(file_path + '_fixing.log', 'w', encoding='utf-8')
