...
for img in imgs:
    if 'liver' in img:
        files.append(img)
out = random.sample(files,5)
print(out)
result = {}
for item in out:
    data = pickle.load(open(os.path.join(sys.argv[1], item),'rb'))['image']
    data = data[...,data.shape[2]%6:]
    print(data.shape)
    slices= data.shape[2]-1
    slice_range = list(range(max(slices//2-20,0), min(slices//2+20,data.shape[2]-1)))
    for num in slice_range:
        if num%6 == 0:
            slice_range.remove(num)
    print(slice_range)
    rand_slices = random.sample(slice_range, 5)
    result[item] = rand_slices
print(result)
