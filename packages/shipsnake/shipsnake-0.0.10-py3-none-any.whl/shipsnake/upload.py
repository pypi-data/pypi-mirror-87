...
def main(arguments,):
	...
	f = f'git config user.name "{data["author"]}";git config user.email "{data["email"]}";git add .;git commit -a v{version} -m "Release v{version}" >> build/log.txt;git remote add origin git@github.com:{data["build"]["github"]}.git >> build/log.txt;git push -u origin master;'
	os.system(f)