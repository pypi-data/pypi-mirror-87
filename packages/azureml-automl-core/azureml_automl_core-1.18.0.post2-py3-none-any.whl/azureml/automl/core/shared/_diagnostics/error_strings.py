...
class AutoMLErrorStrings:
    ...
    LOCAL_MANAGED_SCRIPT_ERROR = "Local managed run failed with an internal error: {error_type}"
    ...
    TIMESERIES_DF_INV_VAL_ALL_GRAINS_CONTAIN_SINGLE_VAL = \
        "All time series in the data frame contain less than 2 values."
    ...
    TIMESERIES_DF_UNIQUE_TARGET_VALUE_GRAIN = "The time series for {grain} only contains one unique value. To " \
                                              "continue, ensure that each series contains multiple values to train."
    ...
    TIMESERIES_GRAIN_ABSENT_NO_GRAIN_IN_TRAIN = \
        "One of time series [{grain}] was not present in the training data set. " \
    ...
    TIMESERIES_INVALID_SHORT_GRAIN_HANDLING = "The {short_series_handling} parameter may take value of {values}."
    ...
    TIMESERIES_ONE_POINT_PER_GRAIN = "Could not determine the data set time frequency. "\
                                     "All series in the data set have one row and no freq parameter was provided. " \
                                     "Please provide the freq (forecast frequency) parameter or review the " \
                                     "time_series_id_column_names setting to decrease the number of time series."
    ...
    UNEXPECTED_IMPORT = "Unable to import the {package} package. " \
                        "Please install the {package} and run AutoML again."
    # endregion
