...
def vnl_complex_to_complex_fft_image_filter(*args, **kwargs):
    """Procedural interface for VnlComplexToComplexFFTImageFilter"""
    import itk
...
def vnl_complex_to_complex_fft_image_filter_init_docstring():
    ...
    import itkTemplate
    import itkHelpers
    if isinstance(itk.VnlComplexToComplexFFTImageFilter, itkTemplate.itkTemplate):
        filter_object = itk.VnlComplexToComplexFFTImageFilter.values()[0]
    else:
        filter_object = itk.VnlComplexToComplexFFTImageFilter

    vnl_complex_to_complex_fft_image_filter.__doc__ = filter_object.__doc__
    vnl_complex_to_complex_fft_image_filter.__doc__ += "\n Args are Input(s) to the filter.\n"
    vnl_complex_to_complex_fft_image_filter.__doc__ += "\n Available Keyword Arguments:\n"
    if isinstance(itk.VnlComplexToComplexFFTImageFilter, itkTemplate.itkTemplate):
        vnl_complex_to_complex_fft_image_filter.__doc__ += itkHelpers.filter_args(filter_object)[0]
        vnl_complex_to_complex_fft_image_filter.__doc__ += "\n"
        vnl_complex_to_complex_fft_image_filter.__doc__ += itkHelpers.filter_args(filter_object)[1]
    else:
        vnl_complex_to_complex_fft_image_filter.__doc__ += "".join([
