...
class PyJAMAS(QtCore.QObject):
    ...
    def setupUI(self):
        ...
        self.gView.setMouseTracking(True)  # This here is necessary so that MouseMove events are triggered when no mouse buttons are pressed.
        ...
        self.addMenuItem(self.menuIO, 'Load annotations (additive) ...', QtCore.Qt.ALT + QtCore.Qt.SHIFT + QtCore.Qt.Key_L,
        ...
        # Save binary image roi.
        self.addMenuItem(self.menuIO, 'Save current ROIs as binary image ...', QtCore.Qt.Key_B,
                         self.io.cbSaveCurrentAnnotationsBinaryImage)

        # Save XML roi.
        self.addMenuItem(self.menuIO, 'Save all ROIs as XML ...', QtCore.Qt.Key_X,
                         self.io.cbSaveAnnotationsXML)
        ...
        # Undo
        self.addMenuItem(self.menuImage, 'Undo image operation ...', QtCore.Qt.ControlModifier + QtCore.Qt.Key_Z, self.image.cbUndo)

        self.menuImage.addSeparator()

        # Adjust contrast
        self.addMenuItem(self.menuImage, 'Adjust contrast ...', QtCore.Qt.SHIFT + QtCore.Qt.Key_C, self.image.cbAdjustContrast)
        ...
        self.addMenuItem(self.menuImage, 'Register image', QtCore.Qt.SHIFT + QtCore.Qt.Key_R, self.image.cbRegisterImage)
        ...
        # Crop action.
        self.addMenuItem(self.menuImage, 'Crop', QtCore.Qt.SHIFT + QtCore.Qt.Key_P, partial(self.image.cbCrop, polyline=None))
        ...
        self.addMenuItem(self.menuClassifiers, 'Segment detected objects ...', None, self.image.cbSegmentDetectedObjects)
        ...
        # Display info.
        self.addMenuItem(self.menuImage, 'Display info', QtCore.Qt.ControlModifier + QtCore.Qt.Key_I, self.image.cbDisplayInfo)
    ...
    def initData(self) -> bool:
        ...
        self.undo_stack: SizedStack = SizedStack(PyJAMAS.undo_stack_size)  # Stores images modified by "irreversible" operations (MIP, kymograph, registration, etc.).
    ...
    def drawPolyline(self, aQPolygonF):
        ...
        if aQPolygonF.isClosed():
            ...
            theItem = self.gScene.addPath(aPath, pen, PyJAMAS.polyline_brush_style)
    ...
    def file_dropped(self, l: list, kb: QtCore.Qt.KeyboardModifier) -> bool:
        ...
        replace_flag = False if kb == QtCore.Qt.AltModifier else True
