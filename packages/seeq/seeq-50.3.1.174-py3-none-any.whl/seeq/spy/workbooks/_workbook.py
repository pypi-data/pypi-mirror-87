...
class Workbook(ItemWithOwnerAndAcl):
    ...
    def path(self):
        ...
        folders = (self.item_inventory.get(folder_id) or self.item_inventory.get(folder_id.upper())
                   for folder_id in self['Ancestors'])
        return _common.path_list_to_string(folder.name for folder in folders if folder is not None)

    @property
    def item_errors_str(self):
        return 'Errors encountered:\n' + \
               '\n----------------------------------------\n'.join(self.item_errors)
    ...
    def _scrape_datasource_inventory(self):
        ...
        for datasource_class, datasource_id in referenced_datasources:
            if datasource_class is None or datasource_id is None:
    ...
    def _scrape_folder_inventory(self):
        ...
        for ancestor_id in self['Ancestors']:
            ...
            # Don't include the user's home folder
            if (item.definition.get(SeeqNames.Properties.unmodifiable)
                    and item.definition.get(SeeqNames.Properties.unsearchable)):
                continue
