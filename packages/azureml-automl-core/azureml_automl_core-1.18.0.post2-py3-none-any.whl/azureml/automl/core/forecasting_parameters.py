...
from azureml.automl.core.shared._diagnostics.automl_error_definitions import (
    ...
    TimeseriesInvalidShortGrainHandlingConfig)
...
class ForecastingParameters:
        ...
        if val is not None and not isinstance(val, str):
            raise ConfigException._with_error(
                AzureMLError.create(
                    ...
                    pandas_url=TimeSeriesWebLinks.PANDAS_DO_URL,
    ...
    def _validate_short_series_handling_config(self, val: Any) -> None:
        ...
        :raises: ConfigException
        """
        if val is not None and val not in ShortSeriesHandlingValues.ALL:
            possible_values = ", ".join(ShortSeriesHandlingValues.ALL)
            possible_values += ' and None'
            raise ConfigException._with_error(
                AzureMLError.create(
                    TimeseriesInvalidShortGrainHandlingConfig, target=TimeSeries.SHORT_SERIES_HANDLING_CONFIG,
                    short_series_handling=TimeSeries.SHORT_SERIES_HANDLING_CONFIG,
                    values=possible_values,
                    reference_code=ReferenceCodes._FORECASTING_PARAM_SHORT_GRAIN_CFG_INVALID)
            )
