...
from lib.method_list import DirectString, TopString, LastString, Int, WordSegment, FExxhash 
...
class FeatureExtractor():
    ...
    def update_proto(self, proto_file_path):
        ...
        ret = os.system("cp {} {}".format(proto_pyfile_name, 
            os.path.join(module_path, "proto")))
        if ret != 0:
            print("update proto failed")
        os.chdir(cur_path)
        sys.path.append(module_path)
        self.proto = "proto.{}".format(proto_pyfile_name.split(".")[0])
    ...
    def _get_all_origin_feature_value(self, ins, idx):
        ...
        '''
        for origin_feature_name in self.origin_feature_name_list:
            self.feature_value_map[origin_feature_name] = self._get_origin_feature_value_by_name(origin_feature_name, ins, idx)
