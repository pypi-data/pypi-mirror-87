...
class ArraySerializer(ObjectSerializer):
    ...
    @classmethod
    @inject_metadata
    def serialize(cls, value: numpy.ndarray, names: List[str] = None):
        arrow_arrays = []
        if value.ndim == 1:
            c = 1
        ...
        if not names:
            # logger.warning(f'Names not passed, setting them to default')
            print(f"Names not passed, setting them to default")
...
class BinarySerializer(ObjectSerializer):
    @classmethod
    def deserialize(cls, value: pyarrow.Table):
        val = value.to_pandas()[DEFAULT_SERIALIZED_COLUMN_NAME].values[0]
        mem_file = BytesIO(val)
        return numpy.load(mem_file)  # noqa
