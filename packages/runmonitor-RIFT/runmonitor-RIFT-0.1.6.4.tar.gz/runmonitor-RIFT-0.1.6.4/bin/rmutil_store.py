...
parser.add_argument("--envsh",type=str,default=None,help="a path to a single script that sets up the environment, to allow for self healing. If not passed this will not exist, and self-healing cannot be used. If 'generate' is passed as the argument, then it will generate an envsh itself")
...
sto.store(opts.event,rd,level=opts.level,rmbase=opts.rmbase,cluster=cluster,envsh=opts.envsh)
